...
Suppose there is a $p$, equal to $1$ or to a prime number, such that the
characteristic exponent of every residue field of $S$ is $p$. Then
$$
G^\sigma = \{x \in G^\tau \mid (\nu(x),p)=1\}, \qquad
G^\rho = \{x \in G^\tau \mid \nu(x)\text{ is a power of }p\}
$$
are locally constructible. If $G^0 = \bigcup G_s^0$ is closed in $G$, then
$G^\tau$ is open and closed. Under the equal-characteristic-exponent
assumption, $G^\sigma$ and $G^\rho$ are closed as well.
\end{lemma}
...
$$
which is constructible. This proves the first assertion and also the local
constructibility of $G^\sigma$ and $G^\rho$.
...
\ref{topology-lemma-characterize-closed-Noetherian}. If $\nu(x)$ is prime
to $p$, the integer $n$ can be chosen prime to $p$, and if $\nu(x)$ is a
power of $p$, the integer $n$ can be chosen to be a power of $p$. The same
argument proves that $G^\sigma$ and $G^\rho$ are stable under specialization.
Their local constructibility proves that they are closed.
\end{proof}
...
morphism of finite type and let $e : S \to X$ be a section. Let $s \in S$.
...
\begin{proof}
We may replace $S$ by an affine open neighbourhood of $s$. Let $A$ be the
henselization of $\mathcal{O}_{S, s}$. Apply
