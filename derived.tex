...
Lemma \ref{lemma-representable-homological}
there exists a morphism $i : Q \to Y$ such that $i \circ u = a$.
Finally, using TR1 again we can choose a triangle
$(Q, Y, W, i, j, k)$. Here is a picture
$$
\xymatrix{
...
\medskip\noindent
This proves the first statement of the proposition. If $\mathcal{D}$ is also
...
Hence $S'$ is a saturated multiplicative system compatible with the
...
$(X, Y, Q_2)$ we finally conclude that $g \in S$. (It also
...
\medskip\noindent
Next, suppose that $S$ is a multiplicative system in $\mathcal{D}$ compatible
...
\begin{enumerate}
\item We let $\text{Comp}(\mathcal{A}) = \text{CoCh}(\mathcal{A})$
...
$$
A matrix computation shows that $c$ is a morphism of complexes.
...
the triangles $(K^\bullet, L^\bullet, C(f), f, i, p)$ and
...
$K^\bullet \to L^\bullet$ is isomorphic to a termwise split injection
...
a short exact sequence of complexes. For
...
defined above has the natural structure of a $\delta$-functor,
with
$$
...
Lemma \ref{lemma-derived-canonical-delta-functor}
above maps the short exact sequence
$0 \to A^\bullet \to B^\bullet \to C^\bullet \to 0$
...
for the first two. For the last one we have to do a little
...
$F|_{X/S} : X/S \to \mathcal{D}'$ (resp.\ $F|_{Y/S} : Y/S \to \mathcal{D}'$)
...
By
Lemma \ref{lemma-complex-cohomology-bounded}
there exists a quasi-isomorphism $X' \to X''$ with $X''$ bounded below.
...
then implies that $\Im(d^0) = \Ker(d^1)$ is an element of
...
We let $D_\mathcal{B}(\mathcal{A})$ be the full subcategory of
...
both $H^n(X)$ and $H^n(Y)$ are kernels of maps between objects
...
This proves faithfulness. Fullness is proved in the
exact same manner.
\end{proof}
...
$I^0/A \to I^1$ into an injective object of $\mathcal{A}$.
...
& =
\alpha^n \circ d^{n - 1} - d^{n - 1} \circ h^n \circ d^{n - 1} \\
& =
...
Hence we can think of $\alpha^n -  d^{n - 1} \circ h^n$
...
$\tilde \alpha : K^\bullet \to \tilde L^\bullet$, $\pi$, $s$ be as in
...
\medskip\noindent
Let $\tilde \alpha : K^\bullet \to \tilde L^\bullet$, $\pi$, $s$ be as in
...
\item we can combine (1) and (2) if also $I^n = 0$ for $n < 0$.
\end{enumerate}
\end{lemma}
...
\begin{proof}
Part (2) is a direct consequence of part (1) and
...
\begin{enumerate}
\item There exists an $i \ll 0$ such that $I^{p, q} = 0$ for all $p < i$
and all $q$.
...
t : R(G \circ F) \longrightarrow RG \circ RF
$$
is an isomorphism of functors from $D^{+}(\mathcal{A})$ to $D^{+}(\mathcal{C})$.
\end{enumerate}
\end{lemma}
...
the proof of the lemma does show that any two resolution functors are
...
in $K^{+}(\mathcal{I})$ such that $i' = a \circ i$ as morphisms in
$K^{+}(\mathcal{A})$. Hence the only issue is existence, and we will see how
...
an abelian category. Let $(j, i)$ be a resolution functor, see
Definition \ref{definition-localization-functor}.
...
$\text{Fil}^f(\mathcal{A})$. Denote $d^0$ the induced map $I^0 \to I^1$.
Next, choose a strict monomorphism $u_2 : \Coker(u_1) \to I^2$ into
a filtered injective object of $\text{Fil}^f(\mathcal{A})$. Denote $d^1$ the induced
...
\begin{proof}
As $A[0] \to I^\bullet$ and $B[0] \to J^\bullet$ are filtered
...
$$
the map $d_{I_a}^b$. Denote by $\text{Tot}(I^{\bullet, \bullet})$
...
This essentially means that if we use a set's worth of diagrams, etc
...
could just as well have developed the whole theory of filtered complexes
...
$H^i(F(\alpha)) : H^i(F(P^\bullet)) \to H^i(F(Q^\bullet))$ is an isomorphism
...
such that $a \circ i_n = j_n \circ a_n$ and $d \circ a = (a_n[1]) \circ c$.
...
exist and are exact. Then countable direct sums exist and are exact.
...
consist of all objects $F(A)$ with $A \in \Ob(\mathcal{A})$. We have
$$
...
on $b - a$. If $b - a = 0$, then $K$ is isomorphic to $H^a(K)[-a]$
...
of $\mathcal{D}$. Denote by $\langle E \rangle_1$ the strictly full subcategory
...
object of $\langle E \rangle$ and the proposition is proven.
\end{proof}
...
$\text{hocolim} X_n = X \to Y$ such that
...
triangulated subcategories of $\mathcal{D}$.
\end{lemma}
...
\medskip\noindent
Proof of (2). The composition $Y_n[n] \to Y'_n[n] \to X'_n[n]$ is
