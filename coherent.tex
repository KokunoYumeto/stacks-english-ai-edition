...
\end{reference}
\begin{history}
The cited source proves the result for a coherent subsheaf of a finite free
sheaf on an irreducible affine classical variety by clearing denominators and
writing an explicit contracting cochain. In the final displayed coboundary
sum, the print has $(-1)^q$ although the summation omits the index $j$; the
alternating differential and the immediately following cocycle identity
require $(-1)^j$. The proof below has that alternating sign and proves the
stronger result for every quasi-coherent module over an arbitrary affine
scheme.
\end{history}
Let $X$ be a scheme.
...
\end{reference}
\begin{history}
The cited corollary passes from vanishing for finite principal-open coverings
to sheaf cohomology using their cofinality. The proof below makes that passage
through a basis criterion and removes the source's irreducibility, coherence,
and finite-type restrictions.
\end{history}
\begin{slogan}
