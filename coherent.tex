...
\begin{reference}
\cite[Chapter II, \S 3, no. 44, Proposition 7, pp. 236--237]{FAC}
The extension to arbitrary coherent algebraic sheaves is
\cite[Chapter II, \S 3, no. 46, Theorem 3, pp. 238--239]{FAC}.
\end{reference}
\begin{history}
The no. 44 proposition proves the result for a coherent subsheaf of a finite
free sheaf on an irreducible affine classical variety by clearing denominators
and writing an explicit contracting cochain. In the final displayed
coboundary sum, the print has $(-1)^q$ although the summation omits the index
$j$; the alternating differential and the immediately following cocycle
identity require $(-1)^j$. The no. 46 theorem resolves an arbitrary coherent
sheaf by a finite free sheaf and removes irreducibility by a closed embedding
into affine space. The proof below has the corrected alternating sign and
proves the stronger result for every quasi-coherent module over an arbitrary
affine scheme.
\end{history}
Let $X$ be a scheme.
...
\begin{reference}
\cite[Chapter II, \S 3, no. 44, Proposition 7, Corollary 1,
p. 237]{FAC}
The unrestricted classical affine-variety statement is
\cite[Chapter II, \S 3, no. 46, Theorem 3, Corollary 1,
p. 239]{FAC}.
\end{reference}
\begin{history}
The no. 44 corollary passes from vanishing for finite principal-open coverings
to sheaf cohomology using their cofinality in the irreducible case. The no. 46
corollary first removes irreducibility by the preceding closed-embedding
argument. The proof below makes the cofinality passage through a basis
criterion and removes the source's coherence and finite-type restrictions as
well.
\end{history}
\begin{slogan}
...
\begin{reference}
\cite[Chapter II, \S 3, no. 47, Proposition 8 and Theorem 4,
pp. 239--240]{FAC}
\cite[Chapter III, \S 1, no. 52, Proposition 2, p. 244]{FAC}
\cite[Chapter III, \S 3, no. 64, Corollary, p. 258]{FAC}
\end{reference}
\begin{history}
The cited proposition proves Cech vanishing for a finite affine-open covering
of an affine classical variety by refinement to principal opens. The cited
theorem then proves that a finite affine-open covering of an arbitrary
classical variety computes sheaf cohomology. The lemma below obtains the
comparison directly whenever all finite intersections are affine, allows an
arbitrary index set, and applies to every quasi-coherent module. The later
application in no. 52 uses the $r + 1$ standard affine charts of classical
projective space; in the scheme formulation their finite intersections are
affine because projective space is separated.

The no. 64 corollary first identifies the source's algebraic colimit complex
with the alternating {\v C}ech complex of that standard covering, passes to
the usual {\v C}ech complex using its no. 20 comparison, and then invokes
no. 52 to reach sheaf cohomology. The lemma below gives the last comparison
directly for every quasi-coherent module whenever the finite intersections
are affine.
\end{history}
Let $X$ be a scheme.
...
\begin{reference}
\cite[Chapter III, \S 1, no. 52, Proposition 2 and Corollary,
p. 244]{FAC}
\end{reference}
\begin{history}
The cited corollary uses the $r + 1$ standard affine charts of
$\mathbf{P}_r(K)$ and the alternating Cech complex to obtain vanishing in
degrees greater than $r$ for coherent algebraic sheaves. The lemma below
isolates the covering argument for every quasi-coherent module on a
quasi-compact scheme with affine diagonal and for any finite affine covering.
\end{history}
\begin{slogan}
...
in degree $-n - 1$ and is free of rank $\binom{-d - 1}{n}$ for
$d \leq -n - 1$.
...
\cite[Chapter III, \S 2, no. 54, observation following Proposition 1,
p. 247]{FAC}
\cite[Chapter III, \S 3, no. 62, Proposition 2 and Corollaries,
pp. 255--256]{FAC}
\cite[Chapter III, \S 3, no. 65, Proposition 8,
pp. 258--259]{FAC}
\cite[Chapter III, \S 4, no. 71, definition of $\Omega$ and the
free-module case of Proposition 2, pp. 264--265]{FAC}
\cite[Chapter III, \S 6, no. 80, proof of Proposition 3, p. 275]{FAC}
\end{reference}
\begin{history}
For $r \geq 1$ over a field, the cited FAC observation identifies global
sections of $\mathcal{O}(d)$ with homogeneous polynomials of degree $d$ for
$d \geq 0$ and proves that they vanish for $d < 0$ by using homogeneous
regular functions on the punctured affine cone. The lemma below computes all
cohomology groups over an arbitrary base ring. It also retains the exceptional
zero-dimensional projective-space behaviour excluded by the source's
$r \geq 1$ hypothesis. FAC no. 62 computes the same field case from finite
power Koszul complexes: for positive-dimensional projective space it
identifies degree-zero cohomology, kills the intermediate groups, describes
top cohomology by monomials with strictly negative exponents, and proves that
the finite systems stabilize once $k \geq -d-r$.

The top-cohomology quotient and final dimension sentence in that source
require $r \geq 1$. When $r = 0$, its positive complex has only degree zero,
so there is no incoming differential by which to form the displayed top
quotient; moreover, degree zero and top degree coincide. The piecewise formula
below resolves the overlap and gives
$H^0(\mathbf{P}^0_R, \mathcal{O}(d)) = R$ for every $d$.

No. 65, Proposition 8 transports the no. 62 computation through the no. 64
comparison with sheaf cohomology. It records the intermediate vanishing, the
top-degree basis of reciprocal monomials, and the class of
$1/(t_0\cdots t_r)$ in degree $-r-1$. The formula below gives the same
description over an arbitrary base ring and retains the separate
zero-dimensional case where degree zero and top degree coincide.

No. 71 fixes this last class as a basis of the top cohomology of
$S(-r-1)$ and pairs the reciprocal monomials above with the ordinary
polynomial monomial basis. This is the free-summand calculation underlying
More on Algebra, Lemma
\ref{more-algebra-lemma-top-graded-duality-polynomial-ring}; that lemma
separately handles arbitrary direct sums and free presentations.

No. 80 takes the alternating sum of this calculation and obtains
$\chi(\mathcal{O}(d)) = \binom{d + n}{n}$ for every integer $d$, where the
right hand side is the generalized binomial polynomial. For
$d \leq -n - 1$, the rank of top cohomology is
$\binom{-d - 1}{n} = (-1)^n\binom{d + n}{n}$. An earlier rank sentence
instead put the negative integer $d$ in the lower binomial index; the
corrected expression counts reciprocal monomials directly.
\end{history}
Let $R$ be a ring.
...
\begin{reference}
\cite[Chapter I, \S 2, no. 15, Example 2, p. 210]{FAC}
\cite[Chapter II, \S 2, no. 37, Lemma 1, Proposition 1, and Remark,
pp. 230--231]{FAC}
\cite[Chapter II, \S 4, no. 48, Proposition 3, p. 241]{FAC}
\end{reference}
\begin{history}
For classical algebraic varieties over an algebraically closed field,
\cite{FAC} first proves coherence of the local-ring sheaf on affine space
from Noetherianity of the polynomial ring. It then passes to a closed
subvariety by its vanishing-ideal sheaf and quotient structure sheaf, and
uses locality to treat algebraic and prealgebraic varieties. Under the
reduced induced scheme comparison of
Schemes, Remark \ref{schemes-remark-reduced-induced-locally-closed},
these are instances of the locally Noetherian scheme statement below.
In no. 48 the source presents a finite module over the Noetherian coordinate
ring as a cokernel of a map between finite free modules and applies exactness
of the associated-sheaf functor. This is the affine case of the equivalence
between (4) and coherence below; finite generation alone would not suffice
...
\begin{reference}
\cite[Chapter II, \S 2, no. 37, Lemma 2 and Proposition 1,
pp. 230--231]{FAC}
\cite[Chapter II, \S 2, no. 39, Proposition 2, pp. 231--232]{FAC}
\end{reference}
Let $X$ be a locally Noetherian scheme.
...
\begin{reference}
\cite[Chapter II, \S 2, no. 39, Proposition 3, p. 232]{FAC}
\cite[Chapter III, \S 3, no. 66, opening paragraph, p. 259]{FAC}
\end{reference}
\begin{history}
In the classical setting of a closed subvariety $W \subset V$,
\cite{FAC} writes $\mathcal{F}^V$ for the direct image of a sheaf on $W$
and describes it as extension by zero. It identifies
$i_*\mathcal{O}_W$ with $\mathcal{O}_V/\mathcal{J}(W)$ and characterizes
the coherent sheaves obtained in this way by annihilation by
$\mathcal{J}(W)$. The formulation below retains the scheme structure and
allows an arbitrary closed immersion of locally Noetherian schemes.

No. 66 applies this construction to a closed projective subvariety
$i:V\to\mathbf{P}_r(K)$, writes $\mathcal{F}^X$ for $i_*\mathcal{F}$,
and uses equality of cohomology on $V$ and $X$. In scheme language the
coherence assertion is the equivalence below, while the cohomology equality
comes from the affineness of a closed immersion and relative affine
cohomology.
\end{history}
...
\begin{reference}
\cite[Chapter II, \S 2, no. 39, Remark, p. 232]{FAC}
\end{reference}
Let $X$ be a Noetherian scheme.
...
\begin{reference}
\cite[Chapter III, \S 4, no. 69, definition of $B^0(M)$, the map
$\alpha$, and Remark, pp. 262--263]{FAC}
\end{reference}
\begin{history}
Take $X=\operatorname{Spec}(K[t_0,\ldots,t_r])$,
$\mathcal I=(t_0,\ldots,t_r)$, $\mathcal G=\mathcal O_X$, and let
$\mathcal F$ be associated to a finite graded module $M$. The ideals
$(t_0^k,\ldots,t_r^k)$ are cofinal with the powers of $\mathcal I$.
The final isomorphism below therefore identifies the source's
$B^0(M)$ with sections on the punctured affine cone, and its map $\alpha$
with ordinary restriction. The source additionally retains the homogeneous
decomposition of this module.
\end{history}
Let $X$ be a Noetherian scheme.
Let $\mathcal{F}$ be a quasi-coherent $\mathcal{O}_X$-module.
...
\cite[Chapter III, \S 1, no. 52, Proposition 2 and Corollary,
p. 244]{FAC}
\cite[Chapter III, \S 2, no. 55, Theorem 1 and Corollary,
pp. 247--248]{FAC}
\cite[Chapter III, \S 3, no. 63, Proposition 3(b) and (c),
pp. 256--257]{FAC}
\cite[Chapter III, \S 3, no. 65, Proposition 7, p. 258]{FAC}
\cite[Chapter III, \S 5, no. 77, proof of Proposition 4(b),
p. 272]{FAC}
\cite[Chapter III, \S 4, no. 70, biduality observation for
$M=\Gamma(\mathcal F)$, p. 264]{FAC}
\cite[Chapter III, \S 4, no. 72, Corollaries 1 and 2,
p. 267]{FAC}
\end{reference}
\begin{history}
The no. 52 result proves that the standard affine covering computes the
cohomology of a coherent algebraic sheaf on classical projective space and
deduces vanishing above its dimension. No. 55 proves eventual global
generation after twisting and deduces a finite quotient by copies of one
twist of the structure sheaf. Part (1) below retains that quotient conclusion
over an arbitrary Noetherian base ring, while the remaining parts add
finiteness and eventual higher-cohomology vanishing to the source results.

No. 63 proves its finiteness and vanishing statements over a field for the
algebraic colimit groups attached to a graded module satisfying condition
(TF). It resolves a finite graded representative by finite sums of shifts of
the polynomial ring and inducts through the resulting long exact sequences.
The source compares those groups with standard-cover cohomology only in
no. 64. Parts (3) and (4) below state the corresponding sheaf results directly
over an arbitrary Noetherian base and use a finite quotient by twists and
descending induction on cohomological degree instead.

No. 65, Proposition 7 applies the no. 64 comparison to convert the no. 63
algebraic finiteness and eventual-vanishing statements into the corresponding
claims for every coherent algebraic sheaf on classical projective space.
Parts (3) and (4) below state that conclusion directly and strengthen its
field base to an arbitrary Noetherian ring.

No. 77, Proposition 4(b) uses the same eventual vanishing simultaneously
for the finitely many degrees $0<q<k$ and then restricts to all positive
multiples of one sufficiently large twist.

No. 70 uses the field case of part (3) for every twist to observe that all
homogeneous pieces of $\Gamma(\mathcal F)$ are finite dimensional. Algebra,
Lemma \ref{algebra-lemma-graded-K-dual} then makes the canonical evaluation
map from that graded module to its graded bidual an isomorphism. This is a
componentwise finiteness statement; it does not assert that the whole
graded direct sum is finite dimensional.

No. 72 uses this componentwise finiteness to dualize once more in degree
zero and obtain the projective-space duality pairing. It also replaces a
module satisfying condition (TF) by a finite graded representative, applies
finiteness of internal graded Ext, and concludes that the positive
dual-cohomology modules are finite while the degree-zero one has a finite
high-degree tail. More on Algebra, Lemma
\ref{more-algebra-lemma-graded-local-duality-polynomial-ring} retains those
graded conclusions; parts (3) and (5) below supply the sheaf-cohomology
finiteness and finite twisted-section module used in the passage.
\end{history}
...
\begin{reference}
\cite[Chapter III, \S 2, no. 60, Proposition 9 and Theorem 2,
pp. 252--253]{FAC}
\end{reference}
\begin{history}
Over the polynomial ring of classical projective space, Proposition 9 of
the cited source proves that the twisted-section module of a finite direct
sum of twists satisfies the source's high-degree finiteness condition.
Theorem 2 then chooses a finite presentation of a coherent algebraic sheaf
by such sums and sheafifies the cokernel of the induced graded-module map.
It thereby obtains a graded representative without yet identifying that
chosen cokernel with the full module of twisted sections. Parts (1) and (4)
below retain the two finiteness ingredients in a form valid for every
degree-one generated graded ring over a Noetherian base.
\end{history}
...
\begin{reference}
\cite[Chapter III, \S 2, no. 58, Proposition 5, pp. 250--251]{FAC}
\cite[Chapter III, \S 3, no. 63, Proposition 3(c),
pp. 256--257]{FAC}
\cite[Chapter III, \S 3, no. 65, Proposition 5, p. 258]{FAC}
\cite[Chapter III, \S 3, no. 67, proof of Proposition 9,
pp. 260--261]{FAC}
\cite[Chapter III, \S 5, no. 77, proof of Proposition 4(a),
p. 272]{FAC}
\end{reference}
\begin{history}
The cited source proves over $K[t_0, \ldots, t_r]$ that a finite graded
module whose associated sheaf vanishes has $M_n=0$ for all sufficiently
large $n$. Its proof applies the Nullstellensatz to the homogeneous
annihilator of each of finitely many generators. The proof below uses the
same finite-generator mechanism with a degree-one generating set, avoids
closed-point language, and places the vanishing step inside the stronger
eventual recovery of $M_n$ from $\Gamma(X, \widetilde M(n))$.

The cited no. 63 proves eventual bijectivity of
$M_n \to H^0(M(n))$ first for its algebraically defined colimit group. Its
finite-free-resolution argument couples this with eventual higher-degree
vanishing; the comparison with cover cochains follows in no. 64. The lemma
below lands directly in global sections and works over a Noetherian degree-one
generated base rather than only over a polynomial ring over a field.

No. 65, Proposition 5 transports the source's eventual comparison map from
the algebraic group $H^0(M(n))$ to
$\Gamma(X, \mathcal{A}(M(n)))$ using no. 64. The lemma below states the
result directly for a finite graded module and identifies the map with the
canonical twisted-section map.

No. 67 uses this eventual isomorphism as the starting range for a descending
induction that detects when the comparison is an isomorphism in every degree.
Constructions, Lemma
\ref{constructions-lemma-comparison-proj-isomorphism} isolates that exact
criterion; the lemma below supplies its eventual-bijectivity hypothesis in
the present Noetherian finite-module setting.

No. 77, Proposition 4(a) chooses one sufficiently large Veronese degree so
that this eventual isomorphism holds in every positive multiple of that
degree.
\end{history}
...
\begin{reference}
\cite[Chapter II, \S 4, no. 49, Remark 2, p. 242]{FAC}
\cite[Chapter III, \S 2, no. 56, class $\mathcal{C}$ and condition (TF),
p. 249]{FAC}
\cite[Chapter III, \S 2, no. 58, Propositions 5 and 6,
pp. 250--251]{FAC}
\cite[Chapter III, \S 2, no. 59, comparison maps and forecast,
pp. 251--252]{FAC}
\cite[Chapter III, \S 2, no. 60, Proposition 9 and Theorem 2,
pp. 252--253]{FAC}
\cite[Chapter III, \S 3, no. 65, Propositions 5 and 6,
p. 258]{FAC}
\end{reference}
\begin{history}
The no. 49 remark announces that the affine module--sheaf correspondence
must be modified for coherent sheaves on projective space. The no. 56
preliminaries call the graded modules vanishing in all sufficiently large
degrees the class $\mathcal{C}$, call a morphism with kernel and cokernel in
$\mathcal{C}$ $\mathcal{C}$-bijective, and impose condition (TF) when the
high-degree tail is finitely generated. Over the polynomial ring in the
source, (TF) is equivalently the existence of a finite graded representative
up to a $\mathcal{C}$-bijective morphism. The proposition below makes this
modification precise for coherent sheaves: it starts with finitely generated
graded modules, quotients by those invisible in high degree, and recovers a
sheaf from the graded family of twisted global sections rather than from
ordinary global sections alone.

Proposition 5 of no. 58 proves that the associated sheaf of a module
satisfying (TF) is coherent and that its kernel consists exactly of
$\mathcal{C}$; Proposition 6 detects injectivity, surjectivity, and
isomorphy after applying the functor by the corresponding kernel and
cokernel conditions. The quotient equivalence below is the categorical
form of those assertions and supplies the quasi-inverse announced there.

No. 59 constructs the canonical maps from a graded module to its twisted
section module and from the sheaf associated to twisted sections back to
the original sheaf. Its Propositions 7 and 8 give the two identity
composites, and its final sentence forecasts the coherent equivalence.
The proposition below realizes that forecast as an equivalence of the
Serre quotient with coherent sheaves.

Theorem 2 of no. 60 supplies the source's essential-surjectivity argument.
It presents a coherent algebraic sheaf by finite sums of twists, takes the
cokernel of the induced map between their twisted-section modules, and uses
exactness together with the two comparison isomorphisms from Proposition 9
to recover the sheaf. The proposition below strengthens this existence
statement to an equivalence and identifies a functorial quasi-inverse.

No. 65 returns to the comparison maps forecast in no. 59. Proposition 5
shows that the module-to-twisted-sections map is an isomorphism in high
degree, and Proposition 6 uses essential surjectivity together with the
first triangle identity to prove that the associated-sheaf comparison is an
isomorphism. The proposition below packages these two conclusions as the
unit and counit of the Serre-quotient equivalence over any Noetherian
degree-one generated base.
\end{history}
Let $A$ be a graded ring such that $A_0$ is Noetherian and $A$ is generated
...
\begin{lemma}
\label{lemma-graded-projective-dimension-cohomology}
\begin{reference}
\cite[Chapter III, \S 5, no. 77, Propositions 2 and 3,
pp. 271--272]{FAC}
\end{reference}
\begin{history}
The source calls projective dimension the dimension of a module. It combines
Lemma 3 with graded duality to characterize this dimension by the canonical
maps from a graded module to all twisted global sections and by vanishing of
all intermediate twisted cohomology groups.

For the homogeneous coordinate ring of a closed subvariety, the source calls
the bound in part (2) the property of being $k$-fold of the first kind. The
statement below retains the exact reusable criterion without adopting that
historical terminology as a general Stacks definition.
\end{history}
Let $K$ be a field, let $r\geq1$, put
$S=K[t_0,\ldots,t_r]$, and set $X=\mathbf P^r_K$. Let $M$ be a finite
graded $S$-module and let $\mathcal F=\widetilde M$. Denote the canonical
maps by
$$
\alpha_n:M_n\longrightarrow H^0(X,\mathcal F(n)).
$$
Then:
\begin{enumerate}
\item $M$ has projective dimension at most $r$ if and only if $\alpha_n$
is injective for every $n\in\mathbf Z$.
\item If $1\leq k\leq r$, then $M$ has projective dimension at most
$r-k$ if and only if every $\alpha_n$ is an isomorphism and
$$
H^q(X,\mathcal F(n))=0
$$
for every $n\in\mathbf Z$ and every $0<q<k$.
\end{enumerate}
\end{lemma}

\begin{proof}
Put $\Omega=S(-r-1)$. On the standard affine covering,
Constructions, Lemma \ref{constructions-lemma-proj-sheaves} and Lemma
\ref{lemma-cech-cohomology-quasi-coherent} identify the module $B^q(M)$ of
More on Algebra, Lemma
\ref{more-algebra-lemma-graded-local-duality-polynomial-ring} with
$\bigoplus_n H^q(X,\mathcal F(n))$. For $q=0$, its canonical map
$M\to B^0(M)$ is the sum of the maps $\alpha_n$. Every homogeneous piece
in sight is finite dimensional over $K$ by Lemma
\ref{lemma-coherent-projective}.

Algebra, Lemmas \ref{algebra-lemma-graded-ext-properties} and
\ref{algebra-lemma-projective-dimension-ext-finite-module} show
that a projective-dimension bound is equivalent to the corresponding
vanishing of $\text{GrExt}^i_S(M,\Omega)$. Graded local duality identifies
these Ext modules with $B^{r-i}(M)^\vee$ for $0\leq i<r$ and gives the
exact sequence
$$
0\to\text{GrExt}^r_S(M,\Omega)\to B^0(M)^\vee
\xrightarrow{\alpha^\vee}M^\vee\to
\text{GrExt}^{r+1}_S(M,\Omega)\to0.
$$
Because all homogeneous pieces are finite dimensional, the last Ext module
vanishes exactly when every $\alpha_n$ is injective. This proves (1).
Vanishing of both exceptional Ext modules says that every $\alpha_n$ is an
isomorphism, while vanishing in degrees
$r-k<i<r$ says precisely that $B^q(M)=0$ for $0<q<k$. This proves (2).
\end{proof}

\begin{lemma}
\label{lemma-negative-twists-local-ext}
\begin{reference}
\cite[Chapter III, \S 5, no. 74, Theorem 1 and proof,
p. 268]{FAC}
\end{reference}
\begin{history}
For a coherent algebraic sheaf on classical projective $r$-space, the
cited theorem characterizes eventual vanishing after increasingly negative
twists by vanishing of one local Ext module at every point. Its proof
passes from standard-cover cohomology to the finite high-degree tail of the
graded dual, applies the graded duality theorem of no. 72, and then uses
the local graded Ext comparison of no. 73.

The proof below retains each of those transitions. In particular, the
exceptional degree $q=0$ uses the high-degree part of the five-term
sequence rather than an unavailable direct graded-duality isomorphism.
The statement is made for all scheme points of projective space. The
source writes $q\geq0$; we display the substantive range
$0\leq q\leq r$, since cohomology above $r$ vanishes and negative-indexed
Ext is not part of the convention used here.
\end{history}
Let $k$ be a field, let $r\geq1$, and set
$X=\mathbf{P}^r_k$. Let $\mathcal{F}$ be a coherent
$\mathcal{O}_X$-module and let $0\leq q\leq r$. The following are
equivalent:
\begin{enumerate}
\item $H^q(X,\mathcal{F}(-n))=0$ for all sufficiently large $n$,
\item
$$
\Ext^{r-q}_{\mathcal{O}_{X,x}}
(\mathcal{F}_x,\mathcal{O}_{X,x})=0
$$
for every $x\in X$.
\end{enumerate}
\end{lemma}
...
\begin{proof}
Put $S=k[T_0,\ldots,T_r]$ with its standard grading and
$\Omega=S(-r-1)$. By Proposition
\ref{proposition-coherent-modules-on-proj}, choose a finite graded
$S$-module $M$ and an isomorphism $\widetilde M\cong\mathcal{F}$.
Use the notation $B^q(M)$ and $T^q(M)=B^q(M)^\vee$ of More on Algebra,
Lemma \ref{more-algebra-lemma-graded-local-duality-polynomial-ring}.

On the standard affine covering, Constructions, Lemma
\ref{constructions-lemma-proj-sheaves} identifies the degree $-n$ part
of the positive localization complex of $M$ with the alternating
{\v C}ech complex of $\mathcal{F}(-n)$. Lemma
\ref{lemma-cech-cohomology-quasi-coherent} therefore gives
$$
H^q(X,\mathcal{F}(-n))=B^q(M)_{-n}.
$$
By the definition of the graded $k$-dual, condition (1) is equivalent to
$T^q(M)_n=0$ for all sufficiently large $n$.

Set
$$
E=\text{GrExt}^{r-q}_S(M,\Omega).
$$
If $q>0$, More on Algebra, Lemma
\ref{more-algebra-lemma-graded-local-duality-polynomial-ring}
identifies $E$ with $T^q(M)$. If $q=0$, the five-term sequence in that
lemma and the equality $M^\vee_n=0$ for $n\gg0$ identify
$E_n$ with $T^0(M)_n$ for all sufficiently large $n$. Thus condition
(1) is equivalent to $E_n=0$ for $n\gg0$.

The graded module $E$ is finite by Algebra, Lemma
\ref{algebra-lemma-graded-ext-properties}. For a finite graded module,
$E_n=0$ for $n\gg0$ is equivalent to $\widetilde E=0$. Indeed, one
direction follows from Lemma \ref{lemma-recover-tail-graded-module};
for the other, high-degree vanishing and finite generation give
$S_+^aE=0$ for some $a$, so every homogeneous localization defining
$\widetilde E$ is zero.

It remains to test $\widetilde E$ on stalks. If $x$ corresponds to the
homogeneous prime $\mathfrak p$, Constructions, Lemma
\ref{constructions-lemma-proj-internal-hom-ext} gives
$$
(\widetilde E)_x
=
\Ext^{r-q}_{S_{(\mathfrak p)}}
\left(M_{(\mathfrak p)},\Omega_{(\mathfrak p)}\right).
$$
Here $S_{(\mathfrak p)}=\mathcal{O}_{X,x}$,
$M_{(\mathfrak p)}=\mathcal{F}_x$, and
$\widetilde\Omega=\mathcal{O}_X(-r-1)$ is invertible. After choosing a
local basis of this invertible module, the last display is the Ext module
in condition (2). Hence $\widetilde E=0$ if and only if condition (2)
holds at every point.
\end{proof}

\begin{lemma}
\label{lemma-negative-twists-projective-dimension}
\begin{reference}
\cite[Chapter III, \S 5, no. 74, Theorem 2,
p. 269]{FAC}
\cite[Chapter III, \S 5, no. 77, local corollary and
proof of Proposition 4(c), pp. 272--273]{FAC}
\end{reference}
\begin{history}
The cited theorem combines its preceding local Ext criterion with
Lemma 1 of no. 74. It calls the length of a finite free resolution the
dimension of the module; in current terminology this is projective
dimension. The statement below makes the local ring explicit and uses
Algebra, Lemma \ref{algebra-lemma-projective-dimension-ext-local-ring}
to test projective dimension against the ring itself. We state the
nonvacuous range $0\leq p\leq r$ and retain all scheme points.

No. 77 first specializes the criterion at twist zero, and then uses its
converse direction uniformly for all sufficiently negative multiples of a
fixed Veronese degree. The statement below supplies both steps without any
closed-point restriction.
\end{history}
Let $k$ be a field, let $r\geq1$, and set
$X=\mathbf{P}^r_k$. Let $\mathcal{F}$ be a coherent
$\mathcal{O}_X$-module and let $0\leq p\leq r$. The following are
equivalent:
\begin{enumerate}
\item $H^q(X,\mathcal{F}(-n))=0$ for all sufficiently large $n$ and
every $0\leq q<p$,
\item for every $x\in X$, the
$\mathcal{O}_{X,x}$-module $\mathcal{F}_x$ has projective dimension
at most $r-p$.
\end{enumerate}
\end{lemma}

\begin{proof}
Every point $x$ lies in a standard affine chart of $X$, whose coordinate
ring is a polynomial ring in $r$ variables over $k$. Algebra, Proposition
\ref{algebra-proposition-finite-gl-dim-polynomial-ring} and Lemma
\ref{algebra-lemma-localize-finite-gl-dim} show that
$\mathcal{O}_{X,x}$ has global dimension at most $r$. Thus the finite
module $\mathcal{F}_x$ has finite projective dimension at most $r$.

By Algebra, Lemma
\ref{algebra-lemma-projective-dimension-ext-local-ring}, condition (2)
is equivalent to
$$
\Ext^i_{\mathcal{O}_{X,x}}
(\mathcal{F}_x,\mathcal{O}_{X,x})=0
\quad\text{for }r-p<i\leq r
$$
at every point $x$. Writing $i=r-q$, this is exactly the collection of
local conditions for $0\leq q<p$ in Lemma
\ref{lemma-negative-twists-local-ext}, and the result follows.
\end{proof}

\begin{lemma}
\label{lemma-negative-twists-complete-intersection}
\begin{reference}
\cite[Chapter III, \S 5, no. 75, Theorem 3, Corollary, and Remark,
pp. 269--270]{FAC}
\end{reference}
\begin{history}
The cited theorem starts with a nonsingular pure $p$-dimensional closed
subvariety of projective $r$-space and a coherent sheaf whose stalks are
free over the local rings of the subvariety. Its proof constructs a
length $r-p$ Koszul resolution and applies the negative-twist criterion of
no. 74. The corollary takes the structure sheaf, while the remark observes
that the proof only needs the subvariety to be locally a complete
intersection.

The statement below uses exactly that local input, expressed as a regular
sequence of fixed length $c$. This both retains the source's remark and
avoids identifying its historical nonsingularity terminology with a
stronger modern smoothness condition. In the source's theorem one has
$c=r-p$.
\end{history}
Let $k$ be a field, let $r\geq1$, and set $X=\mathbf{P}^r_k$.
Let $0\leq c\leq r$ and let $i:Z\to X$ be a closed immersion such that,
for every $z\in Z$, the kernel of
$$
\mathcal{O}_{X,i(z)}\longrightarrow\mathcal{O}_{Z,z}
$$
can be generated by a regular sequence of length $c$.
If $\mathcal{E}$ is a finite locally free $\mathcal{O}_Z$-module, then
$$
H^q\left(Z,\mathcal{E}\otimes_{\mathcal{O}_Z}
i^*\mathcal{O}_X(-n)\right)=0
$$
for all sufficiently large $n$ and every $0\leq q<r-c$.
\end{lemma}

\begin{proof}
Set $\mathcal{G}=i_*\mathcal{E}$. A closed immersion is finite by
Morphisms, Lemma \ref{morphisms-lemma-closed-immersion-finite}, and $X$ is
Noetherian. Hence $\mathcal{G}$ is coherent by Lemma
\ref{lemma-finite-pushforward-coherent}.

We claim that every stalk of $\mathcal{G}$ has projective dimension at
most $c$ over the corresponding local ring of $X$. This is immediate away
from $i(Z)$. If $x=i(z)$, put
$$
R=\mathcal{O}_{X,x},\qquad A=\mathcal{O}_{Z,z},\qquad
I=\Ker(R\longrightarrow A).
$$
Choose a regular sequence $f_1,\ldots,f_c$ generating $I$. By More on
Algebra, Definition \ref{more-algebra-definition-koszul-complex} and
Lemma \ref{more-algebra-lemma-regular-koszul-regular}, the augmented
Koszul complex on this sequence is a finite free $R$-resolution of $A$ of
length $c$. Since $\mathcal{E}_z$ is a finite free $A$-module, a finite
direct sum of this complex resolves
$\mathcal{G}_x=\mathcal{E}_z$. This proves the claim.

Apply Lemma \ref{lemma-negative-twists-projective-dimension} with
$p=r-c$. We obtain
$$
H^q(X,\mathcal{G}(-n))=0
$$
for all sufficiently large $n$ and every $0\leq q<r-c$.
The projection formula of Cohomology, Lemma
\ref{cohomology-lemma-projection-formula} identifies
$$
\mathcal{G}(-n)=
i_*\left(\mathcal{E}\otimes_{\mathcal{O}_Z}i^*\mathcal{O}_X(-n)\right).
$$
Finally, Lemma \ref{lemma-finite-pushforward-coherent} and Cohomology,
Lemma \ref{cohomology-lemma-apply-Leray} identify the cohomology of the
right hand side on $X$ with the cohomology of its argument on $Z$.
This gives the asserted vanishing.
\end{proof}

\begin{lemma}
\label{lemma-veronese-projective-dimension}
\begin{reference}
\cite[Chapter III, \S 5, no. 77, Proposition 4 and proof,
pp. 272--273]{FAC}
\end{reference}
\begin{history}
The cited proposition starts with a connected closed subvariety of classical
projective space whose structure-sheaf stalks have a fixed ambient
projective-dimension bound. It proves that every sufficiently high Veronese
embedding has a homogeneous coordinate ring satisfying the corresponding
global bound. Its proof treats positive, zero, and negative twists separately.

The statement below makes both polynomial rings and the new ambient dimension
explicit. It also records the source's implicit conventions that the base
field is algebraically closed and a subvariety is reduced. No tag is assigned
to this new statement here.
\end{history}
Let $K$ be an algebraically closed field, let $r\geq1$, and let
$1\leq k\leq r$. Put $S=K[t_0,\ldots,t_r]$. Let
$X\subset\mathbf P^r_K$ be a connected reduced closed subscheme, let $I$
be its defining saturated homogeneous ideal, and put $A=S/I$. Assume that
\begin{enumerate}
\item $H^q(X,\mathcal O_X)=0$ for every $0<q<k$, and
\item for every $x\in X$, the $\mathcal O_{\mathbf P^r_K,x}$-module
$\mathcal O_{X,x}$ has projective dimension at most $r-k$.
\end{enumerate}
For $d\geq1$, put
$$
P_d=\operatorname{Sym}_K(S_d),\qquad
N_d=\dim_K(S_d)-1,
$$
where $S_d$ is placed in degree one. The natural graded map
$P_d\to A^{(d)}$ is surjective. For every sufficiently large $d$,
$$
\operatorname{pd}_{P_d}(A^{(d)})\leq N_d-k.
$$
\end{lemma}

\begin{proof}
The scheme $X$ is proper over $K$. Since it is connected and reduced,
Varieties, Lemma
\ref{varieties-lemma-proper-geometrically-reduced-global-sections} shows
that $H^0(X,\mathcal O_X)$ is a finite field extension of $K$, hence is
$K$.

Let $Y$ be an irreducible component of dimension $p$, and let $\eta$ be its
generic point. The ambient local ring $\mathcal O_{\mathbf P^r_K,\eta}$ is
regular of dimension $r-p$, while $\mathcal O_{X,\eta}$ is a field. The
Auslander--Buchsbaum formula and hypothesis (2) therefore give
$r-p\leq r-k$, so every component has dimension at least $k$. In particular,
$\operatorname{Ass}(\mathcal O_X)$ contains no closed point. Varieties,
Lemma \ref{varieties-lemma-vanishin-h0-negative}, applied to the ample
invertible sheaf $\mathcal O_X(1)$, gives an integer $d_0$ such that
$$
H^0(X,\mathcal O_X(-m))=0\quad(m\geq d_0).
$$

Let $i:X\to\mathbf P^r_K$ be the closed immersion. Lemma
\ref{lemma-recover-tail-graded-module} gives an integer $d_1$ such that
$$
A_m\longrightarrow H^0(X,\mathcal O_X(m))
$$
is an isomorphism for $m\geq d_1$. Lemma
\ref{lemma-coherent-projective}, applied to $i_*\mathcal O_X$, gives an
integer $d_2$ such that
$$
H^q(X,\mathcal O_X(m))=0
$$
for $m\geq d_2$ and $0<q<k$. Finally, Lemma
\ref{lemma-negative-twists-projective-dimension} and hypothesis (2) give
an integer $d_3$ such that the same vanishing holds for $m\leq-d_3$ and
$0<q<k$.

Choose $d\geq\max(d_0,d_1,d_2,d_3)$ large enough that $N_d\geq k$.
Constructions, Lemma \ref{constructions-lemma-d-uple} identifies
$\operatorname{Proj}(A^{(d)})$ with $X$ and its twist $n$ with
$\mathcal O_X(nd)$. The canonical map for the finite graded
$P_d$-module $A^{(d)}$ is an isomorphism in every degree: this follows from
the choice of $d_1$ for $n>0$, from $H^0(X,\mathcal O_X)=K$ for $n=0$,
and from the choice of $d_0$ for $n<0$. Its cohomology in degrees
$0<q<k$ vanishes for every twist by the choices of $d_2,d_3$ and hypothesis
(1) at $n=0$. Lemma
\ref{lemma-graded-projective-dimension-cohomology}, applied in
$\mathbf P^{N_d}_K$, proves the assertion.
\end{proof}

\begin{lemma}
\label{lemma-smooth-projective-coordinate-ring-projective-dimension}
\begin{reference}
\cite[Chapter III, \S 5, no. 77, final Corollary, p. 273]{FAC}
\end{reference}
\begin{history}
The source expresses the conclusion by saying that a nonsingular projective
variety admits an embedding $k$-fold of the first kind. The equivalent modern
form below states the projective-dimension bound on the homogeneous coordinate
ring and uses smoothness over the algebraically closed base field.
\end{history}
Let $K$ be an algebraically closed field, let $k\geq1$, and let $X$ be a
smooth projective $K$-scheme of pure dimension $p\geq k$. The following are
equivalent:
\begin{enumerate}
\item $X$ is connected and $H^q(X,\mathcal O_X)=0$ for every $0<q<k$,
\item there is a closed immersion $X\to\mathbf P^N_K$ whose homogeneous
coordinate ring $B$ satisfies
$\operatorname{pd}_{K[x_0,\ldots,x_N]}(B)\leq N-k$.
\end{enumerate}
\end{lemma}

\begin{proof}
If (2) holds, Lemma \ref{lemma-graded-projective-dimension-cohomology}
gives the asserted cohomology vanishing at twist zero and identifies
$K=B_0$ with $H^0(X,\mathcal O_X)$. The latter equality implies that $X$
is connected, since a decomposition would give a nontrivial idempotent in
its ring of global functions.

Conversely, choose a projective embedding $X\to\mathbf P^r_K$. Both schemes
are smooth over $K$, so Divisors, Lemma
\ref{divisors-lemma-immersion-smooth-into-smooth-regular-immersion} makes
this a regular immersion of codimension $r-p$. The Koszul resolution gives
projective dimension $r-p\leq r-k$ at every stalk. Lemma
\ref{lemma-veronese-projective-dimension} now gives (2) after any
sufficiently high Veronese embedding.
\end{proof}

\begin{lemma}
\label{lemma-projective-complete-intersection-cohomology}
\begin{reference}
\cite[Chapter III, \S 5, no. 78, Proposition 5 and remark,
pp. 273--274]{FAC}
\end{reference}
\begin{history}
The cited proposition starts with a pure $p$-dimensional projective
subvariety whose homogeneous ideal has $r-p$ generators. The dimension
condition makes those equations a regular sequence in the polynomial ring:
after localization at the homogeneous maximal ideal this follows from
Algebra, Proposition \ref{algebra-proposition-CM-module}, and regularity of
a homogeneous sequence is detected by that localization. The statement
below exposes the resulting exact algebraic hypothesis and does not require
the base field to be algebraically closed.

The source states the canonical section isomorphism for every integer twist
without excluding $p=0$. That exclusion is essential: if
$S=K[t_0,t_1]$ and $A=S/(t_1)$, then $\text{Proj}(A)$ is a point,
$A_{-1}=0$, and $H^0(\text{Proj}(A),\mathcal O(-1))=K$. We therefore state
the three conclusions in their valid positive-dimensional range. The exact
zero-dimensional replacement is recorded in the remark following the proof.
\end{history}
Let $K$ be a field, let $r\geq1$, and let $0\leq c\leq r-1$. Set
$$
S=K[t_0,\ldots,t_r],\qquad p=r-c.
$$
Let $f_1,\ldots,f_c\in S$ be a homogeneous regular sequence of positive
degrees $m_1,\ldots,m_c$, and put
$$
A=S/(f_1,\ldots,f_c),\qquad X=\text{Proj}(A),\qquad
N=m_1+\ldots+m_c-r-1.
$$
Then for every $n\in\mathbf Z$ the following hold:
\begin{enumerate}
\item the canonical map
$$
A_n\longrightarrow H^0(X,\mathcal O_X(n))
$$
is an isomorphism;
\item $H^q(X,\mathcal O_X(n))=0$ for $0<q<p$;
\item after fixing the displayed order of the equations, there are
isomorphisms
$$
H^p(X,\mathcal O_X(n))\cong
\Hom_K(A_{N-n},K)\cong
\Hom_K(H^0(X,\mathcal O_X(N-n)),K).
$$
\end{enumerate}
Moreover, if $H^p(X,\mathcal O_X)=0$, then $N<0$.
\end{lemma}

\begin{proof}
The Koszul resolution in More on Algebra, Lemma
\ref{more-algebra-lemma-graded-ext-complete-intersection} shows that
$A$ has projective dimension $c=r-p$ over $S$. Since $p\geq1$, Lemma
\ref{lemma-graded-projective-dimension-cohomology}, applied with $k=p$,
proves (1) and (2).

Put $\Omega=S(-r-1)$. The same Koszul--Ext lemma gives
$$
\text{GrExt}^c_S(A,\Omega)\cong A(N).
$$
Use the notation $B^q(A)$ and $T^q(A)=B^q(A)^\vee$ of More on Algebra,
Lemma \ref{more-algebra-lemma-graded-local-duality-polynomial-ring}.
Because $c<r$, that lemma identifies
$$
T^p(A)\cong\text{GrExt}^{r-p}_S(A,\Omega)\cong A(N).
$$
The degree $n$ part of $B^p(A)$ is
$H^p(X,\mathcal O_X(n))$: this follows from the standard affine covering
and Lemma \ref{lemma-cech-cohomology-quasi-coherent}. Hence the degree
$-n$ part of the last display is
$$
\Hom_K(H^p(X,\mathcal O_X(n)),K)\cong A_{N-n}.
$$
Both vector spaces are finite dimensional by Lemma
\ref{lemma-coherent-projective}. Dualizing and using (1) with twist $N-n$
proves (3).

Finally, if $H^p(X,\mathcal O_X)=0$, then (3) gives $A_N=0$. This is
impossible for $N=0$ because $A_0=K$. If $N>0$, generation of $A$ by
$A_1$ would make $A_+$ nilpotent. On the other hand, applying Algebra,
Lemma \ref{algebra-lemma-one-equation} successively to the $c$ equations
gives $\dim(A)\geq r+1-c=p+1>0$, whereas a nilpotent $A_+$ would give
$\dim(A)=\dim(A/A_+)=0$. Thus $N<0$.
\end{proof}

\begin{remark}
\label{remark-projective-complete-intersection-dimension-zero}
Keep the notation above but take $c=r$, so that $X$ has dimension zero.
The canonical section map need not be an isomorphism in negative degree.
More on Algebra, Lemmas
\ref{more-algebra-lemma-graded-ext-complete-intersection} and
\ref{more-algebra-lemma-graded-local-duality-polynomial-ring} instead give,
for every $n\in\mathbf Z$, an exact sequence
$$
0\longrightarrow A_n\longrightarrow H^0(X,\mathcal O_X(n))
\longrightarrow\Hom_K(A_{N-n},K)\longrightarrow0.
$$
Thus this sequence, rather than an all-degree identification of its first
two terms, is the graded-local-duality statement in dimension zero.

The top-duality assertion itself remains valid in this case. Indeed,
$X\to\mathbf P^r_K$ is a regular immersion with normal determinant
$\mathcal O_X(m_1+\ldots+m_r)$. Duality, Lemmas
\ref{duality-lemma-regular-immersion},
\ref{duality-lemma-upper-shriek-P1},
\ref{duality-lemma-upper-shriek-composition}, and
\ref{duality-lemma-perfect-comparison-shriek} identify the dualizing module
of $X$ with $\mathcal O_X(N)$. Duality, Lemma
\ref{duality-lemma-duality-proper-over-field-CM}, applied to
$\mathcal O_X(n)$, therefore gives
$$
H^0(X,\mathcal O_X(n))\cong
\Hom_K(H^0(X,\mathcal O_X(N-n)),K).
$$
Thus part (c) of the source survives in dimension zero even though part (a)
does not.
\end{remark}
...
\begin{reference}
\cite[Chapter III, \S 3, no. 66, Theorems 1 and 2,
pp. 259--260]{FAC}
\end{reference}
\begin{history}
For a closed projective variety over a field with its hyperplane bundle, the
cited source proves finite-dimensional coherent cohomology and eventual
vanishing after positive twists. The lemma below gives the corresponding
finite-module and vanishing statements over an arbitrary Noetherian base for
every proper scheme equipped with an ample invertible sheaf. The source's
eventual global-generation assertion is the ample-sheaf characterization in
Properties, Proposition \ref{properties-proposition-characterize-ample}.
\end{history}
...
\begin{reference}
\cite[Chapter III, \S 3, no. 66, Theorem 1 and following conjecture,
p. 259]{FAC}
\end{reference}
\begin{history}
The cited theorem proves finite-dimensional coherent cohomology for a
projective variety over a field and conjectures the same conclusion for every
complete variety in Weil's sense. The lemma below supplies the modern proper
scheme form over an arbitrary Noetherian affine base, without requiring a
chosen projective embedding. Vanishing above dimension is supplied separately
by Cohomology, Proposition
...
\begin{lemma}
\label{lemma-algebraize-flat-formal-scheme-H2}
\begin{reference}
\cite[Expos\'e 182, Th\'eor\`eme 4, pp. 198--199]{FGA}
\end{reference}
Let $(A, \mathfrak m, k)$ be a complete Noetherian local ring. Set
$S = \Spec(A)$ and $S_n = \Spec(A/\mathfrak m^n)$. Consider a commutative
diagram
$$
\xymatrix{
X_1 \ar[r] \ar[d] & X_2 \ar[r] \ar[d] & X_3 \ar[r] \ar[d] & \ldots \\
S_1 \ar[r] & S_2 \ar[r] & S_3 \ar[r] & \ldots
}
$$
with cartesian squares. Assume that
\begin{enumerate}
\item $X_n \to S_n$ is flat for every $n \geq 1$,
\item $X_1$ is projective over $k$, and
\item $H^2(X_1, \mathcal{O}_{X_1}) = 0$.
\end{enumerate}
Then there exists a projective flat morphism $X \to S$ and compatible
isomorphisms $X_n \cong X \times_S S_n$.
\end{lemma}

\begin{proof}
Choose an ample invertible sheaf $\mathcal{L}_1$ on $X_1$. Suppose
$\mathcal{L}_n$ has been constructed on $X_n$. Let $\mathcal{I}_n$ be the
ideal sheaf of $X_n$ in $X_{n + 1}$. By More on Morphisms, Lemma
\ref{more-morphisms-lemma-deform}, flatness gives
$$
\mathcal{I}_n =
\mathcal{O}_{X_n} \otimes_{A/\mathfrak m^n}
\mathfrak m^n/\mathfrak m^{n + 1}.
$$
This is the pushforward from $X_1$ of the tensor product of
$\mathcal{O}_{X_1}$ with the finite dimensional $k$-vector space
$\mathfrak m^n/\mathfrak m^{n + 1}$. Hence
$$
H^2(X_n, \mathcal{I}_n) =
H^2(X_1, \mathcal{O}_{X_1}) \otimes_k
\mathfrak m^n/\mathfrak m^{n + 1} = 0.
$$
The exact sequence of More on Morphisms, Lemma
\ref{more-morphisms-lemma-picard-group-first-order-thickening} shows that
$\mathcal{L}_n$ lifts to an invertible sheaf $\mathcal{L}_{n + 1}$ on
$X_{n + 1}$. This constructs a compatible system $(\mathcal{L}_n)$.

Theorem \ref{theorem-algebraization} gives a proper morphism $X \to S$,
an ample invertible sheaf on $X$, and compatible isomorphisms of its
reductions with the $X_n$. Thus $X \to S$ is projective by Morphisms,
Lemma \ref{morphisms-lemma-finite-type-over-affine-ample-very-ample}.

It remains to prove flatness. Let $x$ be a point of the closed fibre and
choose an affine open neighbourhood $\Spec(B) \subset X$ of $x$. The ring
$B$ is Noetherian and $B/\mathfrak m^nB$ is flat over
$A/\mathfrak m^n$ for every $n$. Algebra, Lemma
\ref{algebra-lemma-flat-module-powers} shows that $X \to S$ is flat at
$x$. Thus the flat locus is an open neighbourhood of the closed fibre by
More on Morphisms, Theorem
\ref{more-morphisms-theorem-openness-flatness}. Its closed complement has
closed image in $S$ because $X$ is proper. A nonempty closed subset of the
spectrum of a local ring contains the closed point. The complement must
therefore be empty, and $X$ is flat over $A$.
\end{proof}
