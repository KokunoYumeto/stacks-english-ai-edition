...
\begin{reference}
\cite[Chapter II, \S 2, no. 41, Proposition 5, p. 233]{FAC}
The finite-projective affine correspondence is
\cite[Chapter II, \S 4, no. 50, Corollary, pp. 242--243]{FAC}.
\end{reference}
\begin{history}
For a classical algebraic vector bundle $E$, the cited source associates the
locally free sheaf $\mathcal{S}(E)$ of regular sections and recovers $E$ from
that sheaf. With Definition \ref{definition-vector-bundle}, the same
geometric bundle is $\mathbf{V}(\mathcal{S}(E)^\vee)$: the degree $1$ part
of its symmetric algebra is the dual of the sheaf of sections. This dual
convention accounts for the anti-equivalence below. The later cited
corollary restricts this correspondence to finite projective modules over a
classical affine coordinate ring and finite-rank fibre spaces; under the same
dual convention, free modules correspond exactly to trivial bundles.
\end{history}
...
\begin{reference}
\cite[Chapter III, \S 1, no. 51, construction of the structure sheaf,
pp. 243--244]{FAC}
\cite[Chapter III, \S 2, no. 57, pp. 249--250]{FAC}
\end{reference}
\begin{history}
The cited source takes scalar-invariant regular functions on inverse images
of opens in the punctured affine cone. It describes the local functions as
fractions $P/Q$ of homogeneous polynomials of the same degree. On a standard
open, the construction below encodes precisely this degree-zero condition in
the ring $S_{(f)}$ and then extends the resulting sheaf from the standard-open
basis to every open of the homogeneous prime spectrum.

For a graded module over the polynomial ring, no. 57 applies the same
same-degree fraction construction to numerators in $M$, gives its restriction
maps and stalk fractions, and records functoriality in $M$. The construction
below first proves the sheaf condition on the standard-open basis before
extending to all opens; this step is essential for a general graded module.
\end{history}
Let $S$ be a graded ring.
\begin{enumerate}
...
\begin{reference}
\cite[Chapter III, \S 2, no. 54, Proposition 1,
pp. 246--247]{FAC}
\cite[Chapter III, \S 2, no. 57, pp. 249--250]{FAC}
\cite[Chapter III, \S 2, no. 58, Proposition 3, p. 250]{FAC}
\cite[Chapter III, \S 3, no. 64, Proposition 4 and proof,
pp. 257--258]{FAC}
\end{reference}
\begin{history}
For the polynomial ring and $M = S(n)$, the cited source describes the same
local sections as homogeneous fractions $P/Q$ with
$\deg(P)-\deg(Q)=n$, or equivalently as degree-$n$ regular functions on the
punctured affine cone with the appropriate scalar transformation law. The
construction below gives the standard-open section and stalk formulas for
every graded module over every graded ring; its degree-zero localization
also retains nonclosed scheme points absent from the classical description.

For arbitrary graded $M$, no. 57 writes the equality relation with the extra
homogeneous annihilator required by localization, the module operations and
restriction maps, the closed-point stalk formula, and the additive covariant
functor. Items (1)--(5) below recover these formulas on standard opens and at
all homogeneous primes. The exactness assertion is proved independently
below. Proposition 3 of no. 58 proves its classical case by clearing one
more homogeneous denominator in a stalk; the proof below uses the same
localization exactness at every homogeneous prime.

No. 64 applies the same standard-open section formula on every intersection
$D_+(t_{i_0}\cdots t_{i_q})$ of the projective-space covering. A section is
represented by a homogeneous numerator over a power of the displayed
product; finiteness of the covering gives one common denominator power for
an entire cochain. Item (2) below makes this formula valid for every graded
module, removing the source's condition (TF) from the resulting cochain-level
comparison.
\end{history}
...
\begin{lemma}
\label{lemma-proj-internal-hom-ext}
\begin{reference}
\cite[Chapter III, \S 5, no. 73, Proposition 1 and proof,
pp. 267--268]{FAC}
\end{reference}
\begin{history}
For $S=K[t_0,\ldots,t_r]$, the cited proposition constructs the first map
below on a stalk: a homogeneous fraction $\varphi/P$ sends $m/P'$ to
$\varphi(m)/(PP')$. It first checks finite graded free modules, then uses a
resolution finite graded free in every degree and the two exact sequences of
cocycles and coboundaries. Algebra, Lemma
\ref{algebra-lemma-graded-ext-proj-prime} isolates that complex-level
comparison and preserves the source's internal grading.

The formulation below uses all homogeneous primes of Proj rather than only
classical closed points. It keeps the associated graded Ext module distinct
from ordinary Ext over $S$ and from localization at the prime before taking
degree zero.
\end{history}
Let $S$ be a Noetherian graded ring generated by $S_1$ over $S_0$, set
$X=\operatorname{Proj}(S)$, let $M$ be a finite graded $S$-module, and let
$N$ be a graded $S$-module. There is a canonical isomorphism
$$
\widetilde{\text{GrHom}(M,N)}
\longrightarrow
\SheafHom_{\mathcal O_X}(\widetilde M,\widetilde N).
$$
Moreover, if $x\in X$ corresponds to the homogeneous prime $\mathfrak p$,
then for every $q\geq0$ there is a canonical isomorphism
$$
\left(\widetilde{\text{GrExt}^q_S(M,N)}\right)_x
\longrightarrow
\Ext^q_{\mathcal O_{X,x}}(\widetilde M_x,\widetilde N_x).
$$
For $q=0$ this is the map on stalks induced by the first isomorphism.
\end{lemma}

\begin{proof}
On a homogeneous standard open, a degree-zero fraction represented by
$\varphi/g$ in the localization of $\text{GrHom}(M,N)$ acts by
$$
\frac{m}{h}\longmapsto\frac{\varphi(m)}{gh}.
$$
The fraction relations show that this is well defined, and these maps commute
with restriction to smaller standard opens. They therefore define the first
canonical morphism.

Because $S$ is Noetherian and $M$ is finite, there is a presentation of $M$
by two finite graded free modules. Applying the exact associated-sheaf
functor of Lemma \ref{lemma-proj-sheaves} gives a finite presentation of
$\widetilde M$ on every degree-one standard open; twists are free there.
Thus $\widetilde M$ is finitely presented. At the point $x$, Lemma
\ref{lemma-proj-sheaves} and Algebra, Lemma
\ref{algebra-lemma-graded-ext-proj-prime} for $q=0$ identify the source stalk
with
$$
\Hom_{S_{(\mathfrak p)}}
\left(M_{(\mathfrak p)},N_{(\mathfrak p)}\right).
$$
The target stalk is canonically the same module by Modules, Lemma
\ref{modules-lemma-stalk-internal-hom}, and the two identifications commute
with the fraction formula above. Hence the first morphism is an isomorphism
on every stalk and therefore an isomorphism.

For arbitrary $q$, Lemma \ref{lemma-proj-sheaves} identifies the source
stalk with $\text{GrExt}^q_S(M,N)_{(\mathfrak p)}$, while Algebra, Lemma
\ref{algebra-lemma-graded-ext-proj-prime} identifies this with the displayed
local Ext group.
\end{proof}
...
\end{reference}
\begin{history}
The cited source assigns to every nonempty classical open $U$ the degree-zero
fractions having one denominator nonvanishing everywhere on $U$, and says
that these groups themselves form a sheaf. For arbitrary graded modules that
last assertion is false: the example below gives a raw global section whose
restrictions to a standard-open covering all vanish. Passing through the
standard-open sheaf proved above, rather than using the raw all-open
assignment, is the required correction.
\end{history}
The map from $M_0$ to the global sections of $\widetilde M$
is generally far from being an isomorphism. A trivial
example is to take an algebraically closed field $k$ and
$S = k[x, y, z]$ with $1 = \deg(x) = \deg(y) = \deg(z)$
(or any number of variables) and to take $M = S/(x^{100}, y^{100}, z^{100})$.
It is easy to see that $\widetilde M = 0$, but $M_0 = k$.
In particular, the presheaf which assigns to an arbitrary open $U$
the degree-zero fractions with a single homogeneous denominator
nonvanishing everywhere on $U$ is not even separated in this example:
its value on $\text{Proj}(S)$ contains $M_0 = k$, whereas its restrictions
to the covering by $D_{+}(x)$, $D_{+}(y)$, and $D_{+}(z)$ all vanish.
\end{remark}
...
\begin{reference}
\cite[Chapter III, \S 1, no. 51, Proposition 1, p. 244]{FAC}
\cite[Chapter III, \S 1, no. 52, Lemma 2, p. 245]{FAC}
\end{reference}
\begin{history}
The cited proposition verifies the classical variety axioms for projective
space by its affine coordinate charts and the closed graphs on chart
overlaps. The statement below proves the scheme property for $\operatorname{Proj}$
of every graded ring by identifying each standard open with
$\operatorname{Spec}(S_{(f)})$; separatedness is proved separately. The cited
lemma identifies the nonvanishing locus of a positive-degree homogeneous
polynomial on classical projective space as affine by a Veronese embedding.
Here the same affineness is obtained directly from degree-zero localization
and holds for every graded ring.
\end{history}
Let $S$ be a graded ring.
...
\begin{reference}
\cite[Chapter III, \S 2, no. 54 and Propositions 1 and 2,
pp. 246--247]{FAC}
\cite[Chapter III, \S 3, no. 66, construction of $\mathcal{F}(n)$ and
final tensor formula, pp. 259--260]{FAC}
\end{reference}
\begin{history}
The cited source constructs the twist of a sheaf on classical projective
space by gluing its restrictions to the standard charts with transition
functions $(t_j/t_i)^n$. It then identifies the resulting $\mathcal{O}(n)$
with degree-$n$ homogeneous regular functions on the punctured affine cone
and proves $\mathcal{F}(n) \cong \mathcal{F} \otimes \mathcal{O}(n)$.
The definition below packages the construction as the sheaf associated to a
shifted graded module. For a general graded ring the multiplication maps need
not be isomorphisms and $\mathcal{O}_X(n)$ need not be invertible; the
degree-one generated case of the source is recovered by Lemma
\ref{lemma-apply-modules}.

No. 66 restricts the projective-space transition functions to a closed
subvariety $V$ and identifies the resulting operation with
$\mathcal{F}\otimes\mathcal{O}_V(n)$. Thus the embedding contributes the
invertible sheaf $\mathcal{O}_V(1)$, while the definition below records the
twist intrinsically once that sheaf has been fixed.
\end{history}
...
\begin{reference}
\cite[Chapter III, \S 2, no. 58, Proposition 4, p. 250]{FAC}
\end{reference}
\begin{history}
For the degree-one standard chart $D_{+}(t_i)$, the cited source trivializes
the shifted-module sheaf by the localized fraction $m/(t_i^nQ)$. When
$n < 0$ this notation must be interpreted after $t_i$ has been inverted
(equivalently as $t_i^{-n}m/Q$), since $t_i^nQ$ is then not a polynomial.
The proof below makes this valid for every integer $n$ and every positive
degree homogeneous $f$ by performing multiplication by $f^n$ in $S_f$.
\end{history}
...
\begin{proof}
After inverting $f$, multiplication by $f^n$ defines isomorphisms of
graded modules $S_f \to S(nd)_f$ and $M_f \to M(nd)_f$ for every
$n \in \mathbf{Z}$. The first shows that
...
\cite[Chapter III, \S 2, no. 54 and Propositions 1 and 2,
pp. 246--247]{FAC}
\cite[Chapter III, \S 2, no. 58, Proposition 4 and Corollary,
pp. 250--251]{FAC}
\cite[Chapter III, \S 2, no. 59, definition of $\alpha$,
p. 252]{FAC}
\cite[Chapter III, \S 3, no. 67, definition of $M^\natural$ and $\alpha$,
p. 260]{FAC}
\end{reference}
\begin{history}
For $S = K[t_0, \ldots, t_r]$, the standard opens $D_{+}(t_i)$ cover
$\operatorname{Proj}(S)$ and $t_i^n$ trivializes $\mathcal{O}_X(n)$ there.
On an overlap, changing from the $j$th to the $i$th trivialization multiplies
the coefficient by $(t_j/t_i)^n$, exactly the cocycle used in the cited
construction. Thus the canonical multiplication and tensor identifications
below recover the source's formulas
$\mathcal{F}(n)(m) \cong \mathcal{F}(n+m)$ and
$\mathcal{F}(n) \cong \mathcal{F} \otimes \mathcal{O}(n)$, while applying
to every graded ring generated in degree one.

Proposition 4 of no. 58 applies this chart calculation to an arbitrary
graded module and checks that the local trivializations differ on overlaps
by $(t_j/t_i)^n$. Its corollary specializes to $M=S$. The formulation below
records these as the canonical multiplication maps and keeps the
degree-one-cover hypothesis under which they are isomorphisms.

The definition of $\alpha$ in no. 59 sends a homogeneous $m \in M_n$
to the global section represented locally by $m/1$ after viewing $m$ in
degree zero of $M(n)$. This is the degree-$n$ component of the canonical
graded map in the final display below.

No. 67 denotes the target of this graded map by $M^\natural$ and again
writes $\alpha:M\to M^\natural$. Thus the source's notation is exactly the
direct sum over all integer twists in the final display, not ordinary global
sections of one associated sheaf.
\end{history}
Let $S$ be a graded ring. Let $M$ be a graded $S$-module.
...
\begin{reference}
\cite[Chapter III, \S 2, no. 59, definition of $\beta$, p. 252]{FAC}
\end{reference}
\begin{history}
For $S=K[t_0, \ldots, t_r]$, the cited source constructs the comparison
map on a stalk by sending a same-degree fraction $s/Q$ to the germ of
$Q^{-1}s$. On the chart $D_{+}(t_i)$ it writes this as
$(t_i^n/Q)s_i$ for $s \in \Gamma(X, \mathcal{F}(n))$. The construction
below gives the same map on every homogeneous standard open and records
its functoriality for quasi-coherent modules over an arbitrary graded ring.
\end{history}
Let $S$ be a graded ring. Set $X = \text{Proj}(S)$.
...
\begin{lemma}
\label{lemma-comparison-proj-triangle-identities}
\begin{reference}
\cite[Chapter III, \S 2, no. 59, Propositions 7 and 8,
p. 252]{FAC}
\cite[Chapter III, \S 3, no. 65, Proposition 6 and proof,
p. 258]{FAC}
\end{reference}
\begin{history}
The cited source states the two identities for graded modules and
algebraic sheaves on classical projective space and says that they follow
by direct calculation. The proof below supplies that calculation on the
standard-open basis and retains precisely the degree-one-cover hypothesis
used by the source.

No. 65, Proposition 6 uses the first identity after the module comparison
has become an isomorphism on associated sheaves: the composite with the
sheaf comparison is the identity, forcing the latter comparison to be an
isomorphism as well. The statement below keeps this application tied to the
two functorial triangle identities from which it follows.
\end{history}
Let $S$ be a graded ring and set $X=\operatorname{Proj}(S)$. Assume that
$X$ is covered by standard opens $D_{+}(f)$ with $f \in S_1$.
For a graded $S$-module $M$, let
$$
a_M:M\longrightarrow
N_M=\bigoplus_{n\in\mathbf{Z}}\Gamma(X,\widetilde M(n))
$$
be the map of Equation
(\ref{equation-global-sections-more-generally-simplified}).
For a quasi-coherent $\mathcal{O}_X$-module $\mathcal{F}$, set
$$
N_{\mathcal{F}}=
\bigoplus_{n\in\mathbf{Z}}\Gamma(X,\mathcal{F}(n))
$$
and let $b_{\mathcal{F}}:\widetilde{N_{\mathcal{F}}}\to\mathcal{F}$
be the map of Lemma \ref{lemma-comparison-proj-quasi-coherent}. Then
the composites
$$
\widetilde M\longrightarrow\widetilde{N_M}
\xrightarrow{b_{\widetilde M}}\widetilde M
\quad\text{and}\quad
N_{\mathcal{F}}\longrightarrow
N_{\widetilde{N_{\mathcal{F}}}}\longrightarrow N_{\mathcal{F}}
$$
are the identity maps. In the second composite the first arrow is
$a_{N_{\mathcal{F}}}$ and the second is induced by
$b_{\mathcal{F}}$ in every degree.
\end{lemma}

\begin{proof}
It is enough to check the first composite over the standard opens in the
given covering. Let $f\in S_1$ and represent a section of $\widetilde M$
over $D_{+}(f)$ by a degree-zero fraction $m/f^r$, with $r\geq 0$ and
$m\in M_r$.
The first arrow sends it to $a_M(m)/f^r$. By the definition of the
comparison map, the second arrow multiplies the section represented by
$a_M(m)$ by $f^{-r}$, and hence returns $m/f^r$.

For the second composite take a homogeneous element
$s\in (N_{\mathcal{F}})_n=\Gamma(X,\mathcal{F}(n))$. On each
$D_{+}(f)$ in the covering, the first arrow represents $s$ by $s/1$;
the degree-$n$ twist of $b_{\mathcal{F}}$ sends this section back to
$s|_{D_{+}(f)}$. These opens cover $X$, so the composite returns $s$.
This proves the identity in every degree.
\end{proof}

\begin{lemma}
\label{lemma-comparison-proj-isomorphism}
\begin{reference}
\cite[Chapter III, \S 3, no. 67, Proposition 9 and Remarks,
pp. 260--261]{FAC}
\end{reference}
\begin{history}
For $S=K[t_0,\ldots,t_r]$, the cited source writes $M^\natural$ for the
graded module of twisted global sections and assumes condition (TF), using
no. 65 to know that $\alpha:M\to M^\natural$ is already an isomorphism in
all sufficiently large degrees. Proposition 9 characterizes when it is an
isomorphism in every degree by the two degree-one conditions below and proves
the result by descending induction. Its first remark observes that the first
condition alone characterizes injectivity.

The formulation below isolates the exact inputs used in that proof: a finite
standard-open cover in degree one and eventual bijectivity of the comparison
map. It therefore applies without imposing the source's polynomial-ring or
(TF) presentation once those two inputs are known. The second source remark
packages the two conditions as bijectivity of the degree-zero augmentation
into its first finite Koszul complex; this is distinct from silently replacing
that finite-stage group by a local-cohomology group or by a colimit.
\end{history}
Let $S$ be a graded ring and set $X=\operatorname{Proj}(S)$. Let
$f_1,\ldots,f_r\in S_1$ be homogeneous elements such that
$X=\bigcup_{i=1}^rD_+(f_i)$. Let $M$ be a graded $S$-module. Set
$$
N_M=\bigoplus_{n\in\mathbf{Z}}\Gamma(X,\widetilde M(n))
$$
and let
$$
a_M:M\longrightarrow N_M
$$
be the map of Equation
(\ref{equation-global-sections-more-generally-simplified}). Assume that
$(a_M)_n$ is an isomorphism for all sufficiently large $n$. Then $a_M$ is
an isomorphism if and only if the following conditions hold:
\begin{enumerate}
\item
\label{item-comparison-proj-injective}
If $m\in M$ and $f_i m=0$ for every $i$, then $m=0$.
\item
\label{item-comparison-proj-glue}
For every $n\in\mathbf{Z}$ and elements $m_i\in M_{n+1}$ such that
$f_jm_i=f_im_j$ for all $i,j$, there exists an $m\in M_n$ such that
$m_i=f_im$ for every $i$.
\end{enumerate}
Moreover, $a_M$ is injective if and only if condition
(\ref{item-comparison-proj-injective}) holds.
\end{lemma}

\begin{proof}
We first verify both conditions for $N_M$. It is enough to work degreewise.
If $s\in (N_M)_n$ and $f_is=0$ for all $i$, then $s$ restricts to zero on
$D_+(f_i)$ because $f_i$ trivializes $\mathcal{O}_X(1)$ there. These opens
cover $X$, hence $s=0$.

Suppose that $s_i\in (N_M)_{n+1}$ and $f_js_i=f_is_j$ for all $i,j$. On
$D_+(f_i)$ the quotient $s_i/f_i$ is a section of $\widetilde M(n)$. The
displayed relations show that these sections agree on overlaps, so they glue
to an $s\in (N_M)_n$. To see that $f_is=s_i$, restrict to $D_+(f_j)$ and
observe that
$$
f_j(f_is-s_i)=f_i(f_js)-f_js_i=f_is_j-f_js_i=0.
$$
Since $f_j$ is invertible on $D_+(f_j)$, the desired equality holds there,
and hence globally. Thus $N_M$ satisfies both conditions. Consequently, if
$a_M$ is an isomorphism, then $M$ satisfies both of them. If $a_M$ is merely
injective, the first condition for $N_M$ still shows that the first condition
holds for $M$.

Conversely, assume condition (\ref{item-comparison-proj-injective}). We prove
that $(a_M)_n$ is injective by descending induction on $n$, starting in the
range where it is an isomorphism. If $a_M(m)=0$ for $m\in M_n$, then
$a_M(f_im)=f_ia_M(m)=0$. The induction hypothesis gives $f_im=0$ for every
$i$, and condition (\ref{item-comparison-proj-injective}) gives $m=0$. This
proves both the injectivity assertion and one implication of the final
statement.

Now assume condition (\ref{item-comparison-proj-glue}) as well. We prove
surjectivity by the same descending induction. Given $s\in (N_M)_n$, choose
$m_i\in M_{n+1}$ with $a_M(m_i)=f_is$, using the induction hypothesis. The
injectivity just proved and the equalities
$$
a_M(f_jm_i-f_im_j)=f_jf_is-f_if_js=0
$$
show that $f_jm_i=f_im_j$ for all $i,j$. The second condition gives an
$m\in M_n$ with $m_i=f_im$. Hence $f_i(s-a_M(m))=0$ for every $i$. The first
condition for $N_M$ gives $s=a_M(m)$, as desired.
\end{proof}
...
\begin{reference}
\cite[Chapter III, \S 1, no. 52, Lemma 2, p. 245]{FAC}
\cite[Chapter III, \S 5, no. 77, proof of Proposition 4,
pp. 272--273]{FAC}
\end{reference}
\begin{history}
The cited proof sends a classical projective point to all monomials of a
fixed positive degree and identifies the image with the closed Veronese
variety. The statement below records the scheme-theoretic isomorphism induced
by passage to the Veronese subring, together with its effect on associated
sheaves and twists; the closed immersion into the larger projective space is
not being inferred from this statement alone.

No. 77 applies the same construction to a closed subvariety and writes the
twist identity as $\mathcal O_{V_d}(n)=\mathcal O_V(nd)$. Part (2) is the
scheme-theoretic form of that identity and is used there simultaneously for
positive, zero, and negative twists.

The proof below also corrects two defects in its prior form. The inverse map
on homogeneous primes must conclude membership in $\mathfrak p$, not in
$\mathfrak p'$. Moreover, the natural map
$M^{(d)}\otimes_{S^{(d)}}N^{(d)}\to(M\otimes_SN)^{(d)}$ need not be an
isomorphism; compatibility is proved after sheafification on standard opens.
\end{history}
...
we define a homogeneous ideal $\mathfrak p\subset S$ by declaring, for
homogeneous $g\in S$, that $g\in\mathfrak p$ if and only if
$g^d\in\mathfrak p'$. If homogeneous $g,h\in\mathfrak p$ have the same
degree, then $(g+h)^{2d}\in\mathfrak p'$ by the binomial formula. Hence
$(g+h)^d\in\mathfrak p'$ as $\mathfrak p'$ is prime, and therefore
$g+h\in\mathfrak p$. Closure under multiplication by homogeneous elements
is immediate. If homogeneous $gh\in\mathfrak p$, then
$g^dh^d\in\mathfrak p'$, so $g\in\mathfrak p$ or $h\in\mathfrak p$.
Thus $\mathfrak p$ is a graded prime ideal of $S$, it does not contain $f$,
and its image under $j$ is $\mathfrak p'$.
...
be graded $S$-modules. There is a natural graded map
$$
\beta:M'\otimes_{S'}N'\longrightarrow(M\otimes_SN)'
$$
induced by the tensor product. This map need not be an isomorphism of graded
modules. The compatibility with the multiplication maps of
...
i^*(\widetilde{(M \otimes_S N)'})
}
$$
where the lower arrow is the multiplication map followed by
$i^*\widetilde\beta$.
...
$M'_{(f^d)} \times N'_{(f^d)} \to ((M \otimes_S N)')_{(f^d)}$.
...
\begin{reference}
\cite[Chapter III, \S 1, no. 51, definition of projective space,
p. 243]{FAC}
\end{reference}
\begin{history}
The cited source writes $\mathbf{P}_n(K)$ for scalar orbits in
$K^{n + 1} \setminus \{0\}$. The definition below constructs the full scheme
as the homogeneous spectrum of a polynomial ring over $\mathbf{Z}$ and then
base changes to an arbitrary scheme. The orbit description recovers its
classical $K$-points, not its entire scheme-valued point functor.
\end{history}
The scheme
...
\begin{reference}
\cite[Chapter III, \S 1, no. 51 and Proposition 1,
pp. 243--244]{FAC}
\end{reference}
\begin{history}
The source denotes the chart where the $i$th homogeneous coordinate is
nonzero by $U_i$ and uses the ratios $t_j/t_i$, $j \neq i$, as affine
coordinates. These are the standard opens $D_{+}(T_i)$ below. The modern
form works over $\mathbf{Z}$, hence after base change over every scheme, and
identifies the chart ring with the degree-zero localization.
\end{history}
...
\begin{lemma}
\label{lemma-grassmannian-pluecker}
Let $0 < k < n$ and set $d = n - k$ and $N = \binom{n}{d}$.
There is a closed immersion
$$
\mathbf{G}(k, n) \longrightarrow \mathbf{P}^{N - 1}_{\mathbf{Z}}
$$
which sends a quotient
$q : \mathcal{O}_S^{\oplus n} \to \mathcal{Q}$ to the quotient
$$
\wedge^d q : \wedge^d\mathcal{O}_S^{\oplus n}
\longrightarrow \det(\mathcal{Q}).
$$
The pullback of $\mathcal{O}_{\mathbf{P}^{N - 1}_{\mathbf{Z}}}(1)$
is the determinant of the universal quotient on $\mathbf{G}(k, n)$.
\end{lemma}

\begin{proof}
The displayed quotient defines the morphism by
Lemma \ref{lemma-projective-space}. The assertion about the invertible
module follows from the same description of projective space.

Let $I \subset \{1, \ldots, n\}$ have cardinality $d$. The inverse
image of the standard open where the Pl\"ucker coordinate indexed by
$I$ is invertible is the open $F_I$ used in the proof of
Lemma \ref{lemma-gkn-representable}. On this open we may identify the
quotient with a matrix whose columns indexed by $I$ form the identity
matrix. The Pl\"ucker coordinates obtained by replacing one element of
$I$ by one element of its complement are, up to sign, the entries of
the remaining columns. Consequently, the map from the coordinate ring
of the standard open of projective space to the coordinate ring of
$F_I \cong \mathbf{A}^{kd}_{\mathbf{Z}}$ is surjective. Thus the
morphism is a closed immersion over each of these standard opens.
Since these opens cover projective space, the result follows.
\end{proof}
