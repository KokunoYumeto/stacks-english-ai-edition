...
\begin{definition}
\label{definition-abelian-scheme}
Let $S$ be a scheme. An {\it abelian scheme} over $S$ is a group
scheme $A \to S$ which is flat, proper, and of finite presentation and
whose fibre $A_s$ is an abelian variety over $\kappa(s)$ for every
$s \in S$; see Groupoids, Definition
\ref{groupoids-definition-abelian-variety}.
\end{definition}

\noindent
An abelian scheme is smooth: its fibres are smooth by Groupoids, Lemma
\ref{groupoids-lemma-abelian-variety-smooth}, and one applies Morphisms,
Lemma \ref{morphisms-lemma-smooth-flat-smooth-fibres}.
...
\begin{proposition}
\label{proposition-abelian-model-finite-etale-descent}
\begin{reference}
\cite[Expos\'e 190, Proposition 5.1, p. 324]{FGA}
\end{reference}
Let $S$ be an integral Noetherian regular scheme with function field $K$.
Let $S' \to S$ be a surjective finite \`etale morphism with $S'$ integral,
and let $K'$ be the function field of $S'$. Let $A$ be an abelian variety
over $K$. If there exist
\begin{enumerate}
\item a projective abelian scheme $A' \to S'$, and
\item an isomorphism $A \times_{\Spec(K)} \Spec(K') \to A'_{K'}$
of abelian varieties,
\end{enumerate}
then there is a projective abelian scheme $A_0 \to S$ whose generic fibre
is isomorphic to $A$. The pair consisting of $A_0$ and this generic-fibre
isomorphism is unique up to unique isomorphism.
\end{proposition}

\begin{proof}
Let $\eta = \Spec(K)$. Set $S'' = S' \times_S S'$ and denote its
projections to $S'$ by
$p_1$ and $p_2$. Write $A_i = p_i^*A'$ for $i = 1, 2$.
Over the inverse image of the generic point of $S$, the given isomorphism
with $A$ determines an isomorphism
$$
\varphi_\eta : (A_1)_\eta \longrightarrow (A_2)_\eta.
$$
We claim that $\varphi_\eta$ extends uniquely to an isomorphism
$\varphi : A_1 \to A_2$ over $S''$.

The scheme $S''$ is Noetherian and regular. The scheme $A_1$ is regular
because it is smooth over $S''$. View $\varphi_\eta$ as a rational section
of the abelian scheme
$$
A_2 \times_{S''} A_1 \longrightarrow A_1.
$$
Applying Remark \ref{remark-torsors-over-regular} to the connected
components of $A_1$ extends this rational section everywhere. Applying the
same argument to $\varphi_\eta^{-1}$ gives an inverse. The two compositions
are the identity because this holds on a dense open and rational maps from
a reduced scheme to a separated scheme have unique representatives; see
Morphisms, Lemma
\ref{morphisms-lemma-rational-map-from-reduced-to-separated}.

Over $S' \times_S S' \times_S S'$ the two possible composites formed from
$\varphi$ agree over the inverse image of the generic point of $S$. They
agree everywhere by the same uniqueness lemma. Thus $\varphi$ is a descent
datum. It is effective by Groupoids, Lemma
\ref{groupoids-lemma-descend-along-finite-quasi-projective}, giving a scheme
$A_0 \to S$ and an isomorphism $S' \times_S A_0 \cong A'$ compatible with
the descent datum. Moreover, $A_0 \to S$ is projective by Groupoids, Lemma
\ref{groupoids-lemma-projective-after-finite-locally-free-base-change}.

The multiplication, inverse, and unit of $A'$ are compatible with
$\varphi$. Indeed, the required equalities hold over the generic point
because $\varphi_\eta$ comes from the abelian variety $A$, and hence hold
everywhere by uniqueness. Morphisms of schemes satisfy fpqc descent
(Descent, Remark
\ref{descent-remark-morphisms-of-schemes-satisfy-fpqc-descent}), so these
maps descend and make $A_0$ a group scheme. The morphism $A_0 \to S$ is
flat and of finite presentation by Descent, Lemmas
\ref{descent-lemma-descending-property-flat} and
\ref{descent-lemma-descending-property-finite-presentation}; it is already
proper because it is projective. Every geometric point of $S$ lifts to
$S'$, and after such a lift the corresponding fibre of $A_0$ is a fibre of
$A'$. Hence all fibres of $A_0$ are abelian varieties, so $A_0$ is an
abelian scheme. The original generic-fibre isomorphism is compatible with
the descent datum and therefore descends to $A \cong (A_0)_K$.

Finally, suppose $B_0 \to S$ is another abelian scheme with generic fibre
identified with $A$. The resulting generic isomorphism
$(A_0)_K \to (B_0)_K$ is a rational section of the abelian scheme
$B_0 \times_S A_0 \to A_0$. Remark
\ref{remark-torsors-over-regular} extends it uniquely over $A_0$, and the
same argument applied to its inverse shows that the extension is an
isomorphism. This proves uniqueness.
\end{proof}

\begin{proposition}
\label{proposition-abelian-good-reduction-quotient}
\begin{reference}
\cite[Expos\'e 212, Proposition 7.1, pp. 114--115]{FGA}.
\end{reference}
Let $V$ be a discrete valuation ring with fraction field $K$, and let
$A \to \Spec(V)$ be an abelian scheme. If an abelian variety $B'$ over
$K$ is isogenous to a quotient of $A_K$, then there exist an abelian
scheme $B \to \Spec(V)$ and an isomorphism $B_K \to B'$. The pair
consisting of $B$ and this isomorphism is unique up to unique isomorphism.
\end{proposition}

\begin{proof}
Set $S = \Spec(V)$. By symmetry of isogeny for abelian varieties, we may
choose a quotient $A_K \to Q$ and an isogeny $Q \to B'$. Their composite
is a faithfully flat homomorphism
$$
f : A_K \longrightarrow B'.
$$
Let $C' = \Ker(f)$, a closed subgroup scheme of $A_K$, and let
$C \subset A$ be its scheme theoretic closure. Semistable Reduction,
Lemma \ref{models-lemma-closure-is-model} shows that $C \to S$ is flat
and of finite type and that $C_K=C'$. It is proper because it is closed
in $A$. Moreover, $C$ is a subgroup scheme of $A$. Indeed, inversion
preserves $C$ by minimality of the scheme theoretic closure. The unit and
multiplication factor through $C$ because they do so on the generic fibre
and the generic fibres of the flat schemes $S$ and $C \times_S C$ are
scheme theoretically dense.

Consider the translation equivalence relation
$$
R=C\times_S A,\qquad s,t:R\longrightarrow A,
\qquad s(c,a)=a,\quad t(c,a)=c+a.
$$
Both projections are proper and flat. The morphism
$(t,s):R\to A\times_S A$ is the base change of the closed immersion
$C\to A$ by the difference morphism $A\times_S A\to A$; hence it is a
closed immersion and therefore finite. Since $A\to S$ is proper and of
finite presentation, More on Morphisms of Stacks, Proposition
\ref{stacks-more-morphisms-proposition-proper-flat-groupoid-moduli}
gives an algebraic space $B$ and a proper, faithfully flat morphism
of finite presentation
$$
q:A\longrightarrow B
$$
such that $C\times_S A=A\times_B A$. Thus $B$ represents the fppf
quotient sheaf $A/C$. Since $A$ is commutative, this quotient sheaf is a
group sheaf, and its operations make $B$ a group algebraic space over $S$.

We record the properties needed to apply Raynaud's theorem. The morphism
$B\to S$ is locally of finite type by the proposition just cited, and it
is quasi-compact because $q$ is surjective and $A$ is quasi-compact. It is
separated: after the fpqc base change
$q\times q:A\times_S A\to B\times_S B$, its diagonal becomes the closed
immersion $C\times_S A\to A\times_S A$; use Descent on Spaces, Lemma
\ref{spaces-descent-lemma-descending-property-closed-immersion}.
Consequently More on Groupoids in Spaces, Theorem
\ref{spaces-more-groupoids-theorem-raynaud-group-space-scheme-dimension-one}
shows that $B$ is a scheme.

The morphism $B\to S$ is flat by Descent on Spaces, Lemma
\ref{spaces-descent-lemma-flat-fpqc-local-source}, and it is of finite
presentation because it is quasi-compact and is locally of finite
presentation by Descent on Spaces, Lemma
\ref{spaces-descent-lemma-locally-finite-presentation-fppf-local-source}.
It is proper by Morphisms of Spaces, Lemma
\ref{spaces-morphisms-lemma-image-proper-is-proper}: it is separated and
of finite type, while $A\to S$ is proper and $q$ is surjective.

Let $\overline{s}\to S$ be a geometric point. The scheme
$B_{\overline{s}}$ is connected because it is the image of the connected
abelian variety $A_{\overline{s}}$. It is geometrically reduced. To see
this, let $k'/\kappa(\overline{s})$ be a field extension, let
$b\in |B_{k'}|$, and choose $a\in |A_{k'}|$ above $b$. The local homomorphism
$\mathcal{O}_{B_{k'},b}\to\mathcal{O}_{A_{k'},a}$ is faithfully flat.
The second local ring is geometrically regular over $k'$ because
$A_{k'}$ is smooth, so Algebra,
Lemma \ref{algebra-lemma-geometrically-regular-descent} shows that the
first is regular. Thus $B_{\overline{s}}$ is geometrically regular. Hence
$B_{\overline{s}}$ is a proper, connected, geometrically reduced group
scheme over a field, and hence is an abelian variety by Groupoids, Remark
\ref{groupoids-remark-16-equivalent}. Therefore $B\to S$ is an abelian
scheme.

On the generic fibre, both $q_K:A_K\to B_K$ and $f:A_K\to B'$ are fppf
quotients by the same effective equivalence relation with arrow scheme
$C'\times_K A_K$ and object scheme $A_K$. Their universal properties give a
unique isomorphism $B_K\to B'$ compatible with $q_K$ and $f$.

Finally, let two such models be given. Their generic-fibre isomorphism is a
rational section of one abelian scheme after base change to the other.
Remark \ref{remark-torsors-over-regular} extends that section uniquely;
applying the same argument to the inverse shows that the extension is an
isomorphism. This proves uniqueness.
\end{proof}
