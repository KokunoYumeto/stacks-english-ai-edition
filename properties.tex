...
Lemma \ref{lemma-locally-Noetherian-closed-point}
...
(Lemma \ref{lemma-locally-Noetherian}) ring $\mathcal{O}_X(U)$ is a G-ring.
...
\begin{proof}
Combine Lemmas \ref{lemma-locally-Noetherian}
and \ref{lemma-characterize-G-scheme}.
\end{proof}
...
\begin{reference}
\cite[Chapter II, \S 3, no. 45, Theorem 2, Corollary 1,
p. 238]{FAC}
The finite-generation statement for global sections of a coherent sheaf is
\cite[Chapter II, \S 4, no. 49, opening paragraph, p. 241]{FAC}.
\end{reference}
\begin{history}
The cited corollary proves that a coherent algebraic sheaf on an affine
classical variety is a quotient of a finite free sheaf by combining stalkwise
generation by global sections with quasi-compactness. The lemma below
identifies finite type quasi-coherent modules on affine schemes with finite
modules. Choosing finitely many generators of the corresponding module gives
the same quotient, without irreducibility or reducedness hypotheses. In
no. 49 the source applies affine exactness of global sections to such a
finite free quotient and concludes that the global-section module is finite.
The equivalence below packages both directions and needs only finite type and
quasi-coherence on an arbitrary affine scheme.
\end{history}
...
\begin{reference}
\cite[Chapter II, \S 3, no. 43, Propositions 5 and 6,
pp. 235--236]{FAC}
The section-extension form is used in
\cite[Chapter II, \S 3, no. 45, Lemma 1, pp. 237--238]{FAC}.
The coherent-module form is repeated in
\cite[Chapter III, \S 2, no. 55, Lemma 1, p. 247]{FAC}.
\end{reference}
\begin{history}
The cited source proves two denominator-clearing statements on an
irreducible classical algebraic variety: a regular function on $X_f$ becomes
global after multiplication by a high power of $f$, and a global section of
a coherent sheaf that vanishes on $X_f$ is killed by a high power of $f$.
The later lemma applies these mechanisms over a finite principal-open cover
to extend a local section after multiplication by a power of the defining
function. The no. 55 lemma applies the same statement to a coherent module
on an affine chart as the first step in the projective twisting argument.
The isomorphism below gives both conclusions at once for every quasi-coherent
module on a quasi-compact and quasi-separated scheme; neither integrality nor
coherence is needed.
\end{history}
\begin{slogan}
...
\cite[Chapter III, \S 2, no. 55, Lemma 2,
pp. 247--248]{FAC}
\end{reference}
\begin{history}
The cited FAC lemma starts with a section on one standard chart of classical
projective space. It first clears denominators on every other chart and then
kills the finitely many overlap defects by a second common power, producing a
global section of a sufficiently high twist with the prescribed local
component. The lemma below expresses the same mechanism as localization at a
section of an invertible sheaf and proves it for every quasi-coherent module
on a quasi-compact and quasi-separated scheme.
\end{history}
...
\begin{lemma}
\label{lemma-characterize-quasi-coherent-quasi-affine}
\begin{reference}
\cite[I, Theorem 1.4.1]{EGA1}
\end{reference}
Let $A$ be a ring and let $U \subset \Spec(A)$ be a quasi-compact open
subscheme. Let $\mathcal{F}$ be an $\mathcal{O}_U$-module and set
$M = \Gamma(U, \mathcal{F})$. The following are equivalent:
\begin{enumerate}
\item $\mathcal{F}$ is quasi-coherent,
\item there are $f_1, \ldots, f_n \in A$ such that
$U = \bigcup D(f_i)$ and
$\mathcal{F}|_{D(f_i)} \cong \widetilde{M_i}$ for some
$A_{f_i}$-module $M_i$,
\item there is an $A$-module $N$ such that
$\mathcal{F} \cong \widetilde N|_U$, and
\item for every $f \in A$ such that $D(f) \subset U$ the canonical map
$$
M_f \longrightarrow \Gamma(D(f), \mathcal{F})
$$
is an isomorphism.
\end{enumerate}
If these conditions hold, then in (3) one can take $N = M$, and the
resulting isomorphism is canonical.
\end{lemma}

\begin{proof}
The standard opens contained in $U$ form a basis for its topology and,
as $U$ is quasi-compact, a finite subcollection covers $U$.
Thus (3) implies (2) by
Schemes, Lemma \ref{schemes-lemma-widetilde-pullback}, and (2) implies
(1) because being quasi-coherent is local. Lemma
\ref{lemma-quasi-coherent-quasi-affine} proves that (1) implies (3),
with $N = M$ and with the canonical isomorphism asserted above. The same
lemma, followed by taking sections over $D(f)$, proves that (1) implies
(4).

Assume (4), and write $j : U \to \Spec(A)$ for the inclusion. By
Schemes, Lemma \ref{schemes-lemma-compare-constructions}, the identity
map of $M$ gives a map
$$
\widetilde M \longrightarrow j_*\mathcal{F}.
$$
After restriction to $U$, this gives a canonical map
$\widetilde M|_U \to \mathcal{F}$. On a standard open
$D(f) \subset U$, the induced map on sections is the map in (4).
Consequently this map is an isomorphism on a basis for the topology of
$U$, and hence it is an isomorphism. This proves (3) and the final
assertion.
\end{proof}
...
\end{reference}
\begin{history}
The cited corollary treats coherent algebraic sheaves on a connected
classical affine variety and obtains a locally free sheaf from a finite
projective module. The statement below separates the affine module criterion
from the sheaf criterion and works on an arbitrary scheme. Its rank is
locally constant; connectedness is needed only to replace that function by
...
Follows from
Algebra, Lemma \ref{algebra-lemma-ascend-properties-modules}
and Lemma \ref{lemma-locally-projective}.
\end{proof}
...
By Lemma \ref{lemma-finite-type-module}
...
pp. 247--248]{FAC}
\cite[Chapter III, \S 3, no. 66, Theorem 2(a), p. 259]{FAC}
\end{reference}
\begin{history}
For $\mathcal{O}_{\mathbf{P}_r(K)}(1)$, the cited theorem proves that every
coherent algebraic sheaf becomes generated by global sections after every
sufficiently large twist. Its corollary rewrites finite global generation as
a quotient by a finite direct sum of one twist of the structure sheaf. These
are respectively the projective-space cases of
(\ref{item-c-gg}) and (\ref{item-c-q}) below. The proposition makes them
part of the characterization of an arbitrary ample invertible sheaf and
replaces the chart-by-chart argument by a scheme-theoretic equivalence.

No. 66 applies the same conclusion to a closed projective variety using the
restricted hyperplane bundle: every coherent algebraic sheaf becomes
globally generated after every sufficiently large twist. Condition
(\ref{item-c-gg}) below gives exactly this assertion for every finite-type
quasi-coherent sheaf and every ample invertible sheaf on a quasi-compact
scheme.
\end{history}
Let $X$ be a quasi-compact scheme.
...
\begin{reference}
\cite[Chapter III, \S 2, no. 59, definition of $\beta$ and
Propositions 7 and 8, p. 252]{FAC}
\cite[Chapter III, \S 2, no. 60, Proposition 9 and Theorem 2,
pp. 252--253]{FAC}
\end{reference}
\begin{history}
The cited source constructs the canonical map
$\widetilde{\bigoplus_n\Gamma(X,\mathcal{F}(n))}\to\mathcal{F}$ on
classical projective space and records its two compatibility identities.
It postpones bijectivity for coherent $\mathcal{F}$ to no. 65. The lemma
below strengthens that conclusion to every quasi-coherent
$\mathcal{F}$ on a quasi-compact $\operatorname{Proj}(S)$ by localizing
twisted global sections on finitely many homogeneous standard opens.

Proposition 9 of no. 60 proves the comparison is an isomorphism for finite
direct sums of twists. Its Theorem 2 then recovers an arbitrary coherent
algebraic sheaf indirectly from a cokernel presentation by such sums. The
lemma below removes both the coherence hypothesis and the presentation
choice and gives direct recovery from all twisted global sections.
\end{history}
