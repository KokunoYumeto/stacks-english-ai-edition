...
\end{align*}
The penultimate equality holds because $H^{-n}(K)$ is a finite $A$-module, see
...
structure on $D(\mathcal{O})$ given by derived tensor product. In
Cohomology on Sites, Lemma \ref{sites-cohomology-lemma-invertible-derived}
we have seen this means $L$ is perfect and if $(\mathcal{C}, \mathcal{O})$
...
Let $\omega_X^\bullet$ be a dualizing complex on $X$. Then the function
...
the map (\ref{equation-sheafy-trace}) induces a bijection
$$
...
The second equality holds by Cohomology on Sites, Lemma
\ref{sites-cohomology-lemma-internal-hom}.
The third equality holds by construction of $a$.
The fourth equality holds by Derived Categories of Spaces, Lemma
\ref{spaces-perfect-lemma-cohomology-base-change} (this is the important step).
The fifth equality holds by Cohomology on Sites, Lemma
\ref{sites-cohomology-lemma-internal-hom}.
\end{proof}
...
$$
where again the second arrow is invertible. If $M \in D_\QCoh(\mathcal{O}_X)$
...
\Hom_X(M, Rg'_*a'(Lg^*K))
\end{align*}
(where the arrow pointing left is invertible by the base
...
\vcenter{
\xymatrix{
a \circ l_* \circ l^* \circ b \circ m_* & a \circ b \circ m_* \ar[l] \\
...
$$
k_* \circ a' \circ b' \xrightarrow{\xi_{bot}} a \circ l_* \circ b
...
instead of $- \otimes_A^\mathbf{L} A'$, and we will abbreviate
...
to add as long as the source and target of the transformation are known.
...
$$
In this diagram the two squares commute by 
Categories, Lemma \ref{categories-lemma-properties-2-cat-cats}
or more simply the discussion preceding
Categories, Definition \ref{categories-definition-horizontal-composition}.
...
$\text{Tr}_{f'} \circ \alpha \circ \beta$ by definition this proves the lemma.
\end{proof}
...
Lf^*K \otimes^\mathbf{L}_{\mathcal{O}_X} a(L))
\end{align*}
The second equality follows from the definition of $a$ and the projection formula
(Cohomology on Sites, Lemma
\ref{sites-cohomology-lemma-projection-formula})
or the more general Derived Categories of Spaces, Lemma
\ref{spaces-perfect-lemma-cohomology-base-change}.
Hence the result follows from the Yoneda lemma.
\end{proof}
...
\begin{proof}
Set $\omega_{X/Y}^\bullet = a(\mathcal{O}_Y)$.
...
The first equality holds by Cohomology on Sites, Lemma
\ref{sites-cohomology-lemma-adjoint}.
The second equality holds by construction of $a$.
The third equality holds by Derived Categories of Spaces, Lemma
\ref{spaces-perfect-lemma-cohomology-base-change}.
...
(Cohomology on Sites, Lemma
\ref{sites-cohomology-lemma-dual-perfect-complex})
and the fact that $Rf_*E^\vee$ is perfect by
Derived Categories of Spaces, Lemma
\ref{spaces-perfect-lemma-flat-proper-perfect-direct-image-general}.
...
Let $S$ be a scheme. Let $f : X \to Y$ be a proper, flat morphism of
...
of $k_j$ by $f_{i_j}$). Let $a'_j$ denote the right adjoint of $R(f'_j)_*$. Restricting the given isomorphisms to $Y'_j$
via the morphism $k'_j$ we get isomorphisms of pairs
$((\omega')^\bullet|_{X'_j}, \tau'|_{Y'_j})
\to (a'_j(\mathcal{O}_{Y'_j}), \text{Tr}_{f'_j, \mathcal{O}_{Y'_j}})$.
...
$g_{\varphi, i} : Y_{n, i} \to Y_{m, \alpha(\varphi)(i)}$ and
$g'_{\varphi, i} : X_{n, i} \to X_{m, \alpha(\varphi)(i)}$
...
$$
\iota_{n, i, \varphi} :
(L(g'_{\varphi, i})^*\omega_{n, i}^\bullet, Lg_{\varphi, i}^*\tau_{n, i})
\longrightarrow
...
$\omega_0^\bullet$ and the two maps $\delta_1^1, \delta_0^1 : [0] \to [1]$.
...
be the right adjoint of $Rf_{0*}$ from
