...
\begin{reference}
\cite[Expos\'e 212, Th\'eor\`eme 5.1 (iii), p. 111]{FGA}.
\end{reference}
...
\begin{proposition}
\label{proposition-proper-flat-groupoid-moduli}
\begin{reference}
\cite[Expos\'e 212, Th\'eor\`eme 6.1, pp. 112--113, and erratum,
p. 301]{FGA}.
\end{reference}
Let $S$ be a locally Noetherian scheme and let
$(U, R, s, t, c)$ be a groupoid scheme over $S$. Assume
\begin{enumerate}
\item $U \to S$ is separated and locally of finite type,
\item $s, t : R \to U$ are proper and flat, and
\item $(t, s) : R \to U \times_S U$ is finite.
\end{enumerate}
Then there exist an algebraic space $M$ locally of finite type over $S$
and a morphism $q : U \to M$ with the following properties:
\begin{enumerate}
\item $q$ is surjective, proper, and open,
\item $|M|$ is the quotient of $|U|$ by the orbit equivalence relation,
and the sets of geometric points of the fibres of $q$ are the orbits, and
...
is surjective.
\end{enumerate}
If $(t, s)$ is an equivalence relation, then it is effective and
$q$ is faithfully flat and locally of finite presentation.
\end{proposition}

\begin{proof}
The morphisms $s$ and $t$ are locally of finite presentation because
their targets are locally Noetherian. Hence
$\mathcal{X} = [U/R]$ is an algebraic stack by Criteria for
Representability, Theorem
\ref{criteria-theorem-flat-groupoid-gives-algebraic-stack}.
Let $G \to U$ be the stabilizer group scheme. By Morphisms of Stacks,
Lemma \ref{stacks-morphisms-lemma-presentation-inertia}, it is the
pullback of the inertia of $\mathcal{X}$. On the other hand,
$G \to U$ is the base change of $(t, s)$ by the diagonal
$U \to U \times_S U$. Since being finite is fppf local on the target,
the inertia of $\mathcal{X}$ is finite.

Apply Theorem \ref{theorem-keel-mori} to obtain a uniform categorical
moduli space $f : \mathcal{X} \to M$, and let
$q : U \to \mathcal{X} \to M$ be the composition. The algebraic stack
$\mathcal{X}$ is locally of finite type over $S$, and hence
$M \to S$ is locally of finite type by
Lemma \ref{lemma-keel-mori-finite-type}.

Write $a : U \to \mathcal{X}$ for the presentation. It is surjective,
flat, and locally of finite presentation. It is also proper: after the
surjective fppf base change $U \to \mathcal{X}$ this assertion becomes
the properness of $s : R \to U$. Since $f$ is quasi-compact and a
universal homeomorphism, the composition $q = f \circ a$ is
quasi-compact and universally closed. It is locally of finite type by
Morphisms of Spaces, Lemma
\ref{spaces-morphisms-lemma-permanence-finite-type}. It is separated
because $U \to S$ is separated, by Morphisms of Spaces, Lemma
\ref{spaces-morphisms-lemma-compose-after-separated}. Consequently
$q$ is proper. The morphism $a$ is open, and $|f|$ is a homeomorphism;
hence $q$ is open as well.

Morphisms of Stacks, Lemma
\ref{stacks-morphisms-lemma-points-presentation} identifies
$|\mathcal{X}|$ with the orbit space of $|U|$. Since $|f|$ is a
homeomorphism, this proves the assertion about topological spaces and
geometric fibres. Finally, let $\Spec(k) \to U \times_M U$ be a
geometric point. The description of geometric points in the proof of
Theorem \ref{theorem-keel-mori} says that two objects over an
algebraically closed field have the same image in $M(k)$ exactly when
they are isomorphic. Such an isomorphism is a $k$-valued point of $R$.
This proves the surjectivity of $R \to U \times_M U$.

Suppose now that $(t, s)$ is an equivalence relation. Then
$\mathcal{X}$ has no nontrivial automorphisms, so it is an algebraic
space by Algebraic Stacks, Proposition
\ref{algebraic-proposition-algebraic-stack-no-automorphisms}.
The identity of $\mathcal{X}$ is a uniform categorical moduli space;
the universal properties of it and of $f$ show that
$f : \mathcal{X} \to M$ is an isomorphism. Thus $q=a$ is faithfully
flat and locally of finite presentation, and
$R = U \times_{\mathcal{X}} U = U \times_M U$.
\end{proof}

\begin{remark}
\label{remark-proper-flat-groupoid-scheme-refinement}
The historical theorem cited above asserts in addition that $M$ is a
scheme and, when $S$ is Noetherian and $(t, s)$ is an equivalence
relation, that $M$ is quasi-projective over $S$. Its published erratum
specifies that the proof of the quasi-projectivity assertion applies
only in the equivalence-relation case. Proposition
\ref{proposition-proper-flat-groupoid-moduli} records the conclusions
that follow from the algebraic-stack and moduli-space argument. Notice
that this argument only needs $U \to S$ to be separated and locally of
finite type, whereas the historical statement assumes that $U \to S$
is quasi-projective. The proposition does not assert either of the two
scheme-theoretic refinements. Compare
\cite[Remark 3.3.2]{lieblich_remarks}, where the general relative
conclusion for an algebraic space admitting such a proper flat
quasi-projective cover is described as a tempting expectation; the
argument given there applies over a field, or when a finite flat
quasi-projective cover is already available.
\end{remark}

\begin{proposition}
\label{proposition-dense-scheme-quotient-quasi-finite-stabilizer}
\begin{reference}
\cite[Expos\'e 212, Th\'eor\`eme 6.2, pp. 113--114]{FGA}.
\end{reference}
Let $S$ be a Noetherian scheme and let
$(U, R, s, t, c)$ be a groupoid scheme over $S$. Assume
\begin{enumerate}
\item $U \to S$ is of finite type,
\item $s, t : R \to U$ are flat and of finite type, and
\item $(t, s) : R \to U \times_S U$ is quasi-finite.
\end{enumerate}
There exist a dense invariant open $V \subset U$, a scheme $M$ of
finite type over $S$, and a morphism $q : V \to M$ such that
\begin{enumerate}
\item $q$ is surjective and open,
\item $|M|$ is the quotient of $|V|$ by the orbit equivalence relation,
and the sets of geometric points of the fibres of $q$ are the orbits, and
\item the canonical morphism
$$
(t, s) : R|_V \longrightarrow V \times_M V
$$
is surjective.
\end{enumerate}
If $(t, s)$ is an equivalence relation, then $q$ may be chosen
faithfully flat and of finite presentation, and the relation is
effective on $V$.
\end{proposition}

\begin{proof}
The schemes $U$ and $R$ are Noetherian, and $s$ and $t$ are locally of
finite presentation. Let $G \to U$ be the stabilizer group scheme.
It is quasi-finite because it is the base change of $(t, s)$ by the
diagonal $U \to U \times_S U$. By More on Morphisms, Lemma
\ref{more-morphisms-lemma-quasi-finite-finite-over-dense-open}, there
is a dense open over which $G \to U$ is finite. Let $U_0 \subset U$
be the largest such open. Since $s$ and $t$ are fppf coverings,
More on Groupoids, Lemma
\ref{more-groupoids-lemma-property-G-invariant} shows that $U_0$ is
invariant. In particular $U_0$ is dense in $U$.

Set $R_0 = R|_{U_0}$ and $\mathcal{X}_0 = [U_0/R_0]$. By Criteria for
Representability, Theorem
\ref{criteria-theorem-flat-groupoid-gives-algebraic-stack}, this is an
algebraic stack. Its inertia is finite by Morphisms of Stacks, Lemma
\ref{stacks-morphisms-lemma-presentation-inertia}. Theorem
\ref{theorem-keel-mori} gives a uniform categorical moduli space
$f : \mathcal{X}_0 \to M_0$. Lemma
\ref{lemma-keel-mori-finite-type} shows that $M_0 \to S$ is locally of
finite type. Moreover $\mathcal{X}_0$ is quasi-separated, because its
diagonal is presented by the quasi-finite morphism
$R_0 \to U_0 \times_S U_0$. Hence $M_0$ is quasi-separated by
Lemma \ref{lemma-keel-mori-diagonal}. The space $M_0$ is quasi-compact:
its underlying space is the image of the Noetherian scheme $U_0$.
It is also locally Noetherian, and hence it is Noetherian. Thus
$M_0 \to S$ is of finite type.

By Properties of Spaces, Proposition
\ref{spaces-properties-proposition-locally-quasi-separated-open-dense-scheme},
there is a dense open subspace $M \subset M_0$ which is a scheme. Let
$V \subset U_0$ be its inverse image. Since $M_0$ is Noetherian, the
scheme $M$ is still of finite type over $S$. The morphism
$U_0 \to \mathcal{X}_0 \to M_0$ is surjective and open: the first
map is flat and locally of finite presentation, and the second is a
homeomorphism on underlying spaces. It follows that $V$ is dense in
$U_0$, hence in $U$, and it is invariant. The same argument as in
Proposition \ref{proposition-proper-flat-groupoid-moduli} now gives
the orbit, quotient-topology, geometric-fibre, and kernel-pair
surjectivity assertions for $q : V \to M$.

If $(t, s)$ is an equivalence relation, then $G \to U$ is an
isomorphism, so $U_0=U$. As in the final paragraph of the proof of
Proposition \ref{proposition-proper-flat-groupoid-moduli}, the quotient
stack is an algebraic space and its moduli-space morphism is an
isomorphism. Consequently $q$ is faithfully flat and of finite
presentation and $R|_V = V \times_M V$.
\end{proof}
...
\begin{proposition}
\label{proposition-artinian-group-action-quotient}
\begin{reference}
\cite[Expos\'e 212, Th\'eor\`eme 7.2, p. 115]{FGA} and
\cite[Expos\'e VI$_A$, Th\'eor\`emes 3.2 and 3.3.2, and 4.7]{SGA3}.
\end{reference}
Let $A$ be an Artinian ring and set $S = \Spec(A)$. Let $F$ and $G$
be group schemes of finite type over $S$, and let $u : F \to G$ be a
homomorphism. Assume that $F \to S$ is flat and that $\Ker(u) \to S$
is finite. Assume in addition that either
\begin{enumerate}
\item $G \to S$ is flat, or
\item $u$ is a monomorphism.
\end{enumerate}
Then there are a scheme $Q$ of finite type over $S$ and a morphism
$q : G \to Q$ which is the quotient $G/F$ for the right action
$(g, f) \mapsto gu(f)$. More precisely,
\begin{enumerate}
\item $q$ is surjective and open,
\item its geometric fibres are the orbits, and
\item the canonical morphism
$$
G \times_S F \longrightarrow G \times_Q G
$$
is surjective.
\end{enumerate}
If $u$ is a monomorphism, then $q$ is faithfully flat and locally of
finite presentation, the displayed morphism is an isomorphism, and
$Q$ represents the fppf quotient sheaf. In this case formation of $Q$
commutes with arbitrary base change. If $G \to S$ is flat, formation
of the quotient in the category of locally ringed spaces commutes with
flat base change.
\end{proposition}

\begin{proof}
By Algebra, Lemma \ref{algebra-lemma-artinian-finite-length}, the ring
$A$ is a finite product of local Artinian rings. All the schemes and
assertions in the statement split over the corresponding open and closed
subschemes of $S$, and the resulting quotients recombine by disjoint union.
Thus we may assume that $A$ is local.

If $G$ is flat over $S$, the result, with left cosets in place of right
cosets, is \cite[Expos\'e VI$_A$, Th\'eor\`eme 3.2]{SGA3}; inversion on
$G$ exchanges the two conventions. The flat-base-change assertion is
\cite[Expos\'e VI$_A$, 4.7]{SGA3}. If $u$ is a monomorphism, it is a
closed immersion and the flatness hypothesis on $G$ can be omitted by
\cite[Expos\'e VI$_A$, Th\'eor\`eme 3.3.2]{SGA3}. In that case $q$ is
an fppf $F$-torsor. The fppf quotient sheaf and its formation commute
with every base change by their definitions, which gives the last
base-change assertion.
\end{proof}

\begin{remark}
\label{remark-artinian-group-action-quotient-residual}
The theorem cited from FGA does not impose either of the two additional
alternatives in Proposition
\ref{proposition-artinian-group-action-quotient}. The editorial note in
\cite[Expos\'e V, note 1]{SGA3} points to Expos\'e VI$_A$, 3.2 and 3.3
as its proof. However, 3.2 assumes that both $F$ and $G$ are flat, while
3.3 removes flatness of $G$ only when $u$ is a monomorphism. Thus those
passages do not by themselves prove the remaining case in which $G$ is
not flat and $u$ is not a monomorphism. We do not assert that remaining
case here.
\end{remark}

\begin{lemma}
\label{lemma-artinian-flat-subgroup-quotient}
\begin{reference}
\cite[Expos\'e 212, Corollaire 7.3, p. 115]{FGA}.
\end{reference}
In the situation of Proposition
\ref{proposition-artinian-group-action-quotient}, assume that $u$ is a
monomorphism. Then $G$ is flat over $S$ if and only if $Q = G/F$ is flat
over $S$. Formation of $Q$ commutes with arbitrary base change. If $F$
is normal in $G$, then $Q$ has a unique group scheme structure for which
$q : G \to Q$ is a homomorphism.
\end{lemma}

\begin{proof}
The morphism $q$ is faithfully flat and locally of finite presentation
by Proposition \ref{proposition-artinian-group-action-quotient}. If $Q$
is flat over $S$, then $G \to S$ is flat by composition. The converse is
fpqc locality of flatness on the source along $q$, see Descent, Lemma
\ref{descent-lemma-flat-fpqc-local-source}. Base change was proved in
the proposition. If $F$ is normal, the fppf quotient sheaf is a group
sheaf. Its representing scheme $Q$ therefore acquires the asserted
unique multiplication, identity, and inverse by Yoneda's lemma.
\end{proof}

\begin{lemma}
\label{lemma-homomorphism-algebraic-groups-factorization}
\begin{reference}
\cite[Expos\'e 212, Corollaire 7.4, pp. 115--116]{FGA}.
\end{reference}
Let $k$ be a field and let $u : F \to G$ be a homomorphism of group
schemes of finite type over $k$. There is a factorization
$$
F \longrightarrow F' \longrightarrow G
$$
such that the first arrow is the fppf quotient by the closed normal
subgroup scheme $\Ker(u)$ and the second arrow is a closed immersion.
The scheme quotient $G/F = G/F'$ exists.
\end{lemma}

\begin{proof}
Set $K = \Ker(u)$. Group schemes over a field are separated by Groupoids,
Lemma \ref{groupoids-lemma-group-scheme-over-field-separated}, so
$K \subset F$ is a closed normal subgroup scheme; it is flat over $k$.
Proposition
\ref{proposition-artinian-group-action-quotient} and Lemma
\ref{lemma-artinian-flat-subgroup-quotient} give the group scheme
$F' = F/K$ and an fppf quotient homomorphism $F \to F'$. The map $u$
descends uniquely to a homomorphism $i : F' \to G$.

The map $i$ is a monomorphism as a map of fppf sheaves. Indeed, fppf
locally two sections of $F'$ lift to sections of $F$, and equality of
their images in $G$ says that their quotient belongs to $K$. Since
$F'$ and $G$ are of finite type over $k$, the morphism $i$ is
quasi-compact. Hence $i$ is a closed immersion by
\cite[Expos\'e VI$_B$, Corollaire 1.4.2]{SGA3}. Applying Proposition
\ref{proposition-artinian-group-action-quotient} once more to
$F' \subset G$ constructs $G/F'$. The actions of $F$ and $F'$ have the
same quotient, and hence $G/F = G/F'$.
\end{proof}
