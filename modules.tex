...
\label{lemma-abelian}
\begin{reference}
\cite[Chapter I, \S 1, nos. 7 and 8, pp. 203--205]{FAC}
\end{reference}
...
\begin{reference}
\cite[Chapter III, \S 6, no. 81, opening paragraph, p. 276]{FAC}
\end{reference}
\begin{history}
The cited source defines the support of a coherent algebraic sheaf by its
nonzero stalks. The definition below is the same one for a module on an
arbitrary ringed space and also records the support of a section.
\end{history}
Let $(X, \mathcal{O}_X)$ be a ringed space.
Let $\mathcal{F}$ be a sheaf of $\mathcal{O}_X$-modules.
\begin{enumerate}
...
\begin{reference}
\cite[Chapter I, \S 2, no. 12, Definition 1, p. 207]{FAC}
\end{reference}
Let $(X, \mathcal{O}_X)$ be a ringed space.
Let $\mathcal{F}$ be a sheaf of $\mathcal{O}_X$-modules.
...
\begin{reference}
\cite[Chapter I, \S 2, no. 12, Proposition 1, p. 207]{FAC}
\end{reference}
Let $X$ be a ringed space.
...
\end{reference}
\begin{history}
The cited source proves closedness by observing that if a finite type sheaf
has zero stalk at a point, then the zero section generates on a neighbourhood
of that point. The proof below packages the same argument as the local
vanishing lemma for finite type modules.
\end{history}
\begin{slogan}
...
\begin{reference}
\cite[Chapter II, \S 4, no. 48, associated-sheaf construction,
p. 240]{FAC}
\end{reference}
\begin{history}
The source forms $\mathcal{O} \otimes_A M$ from the constant sheaves
attached to the coordinate ring and an $A$-module, and records its
functoriality. The constructions below make the necessary sheafification
explicit, identify the result with pullback from the one-point ringed space,
and work for an arbitrary ringed space and ring map.
\end{history}
Let $(X, \mathcal{O}_X)$ be a ringed space.
...
is given by the section $\sum_i \alpha(r_{ij})$ where the $r_{ij}$
...
\begin{reference}
\cite[Chapter I, \S 2, no. 12, Definition 2, p. 208]{FAC}
\cite[Chapter I, \S 2, no. 15, Definition 3, p. 210]{FAC}
\cite[Chapter II, \S 2, no. 38, p. 231]{FAC}
\end{reference}
Let $(X, \mathcal{O}_X)$ be a ringed space.
Let $\mathcal{F}$ be a sheaf of $\mathcal{O}_X$-modules.
...
\begin{reference}
\cite[Chapter I, \S 2, no. 12, Proposition 2, p. 208]{FAC}
\end{reference}
Let $(X, \mathcal{O}_X)$ be a ringed space.
...
\begin{reference}
\cite[Chapter I, \S 2, no. 12, Proposition 3, p. 208]{FAC}
\cite[Chapter I, \S 2, no. 13, Theorems 1 and 2 and their
Corollaries, pp. 208--209]{FAC}
\cite[Chapter I, \S 2, no. 15, Proposition 8 and Corollary,
pp. 210--211]{FAC}
\end{reference}
Let $(X, \mathcal{O}_X)$ be a ringed space.
\begin{enumerate}
...
\begin{reference}
\cite[Chapter I, \S 2, no. 15, Proposition 7, p. 210]{FAC}
\end{reference}
Let $(X, \mathcal{O}_X)$ be a ringed space.
Let $\mathcal{F}$ be an $\mathcal{O}_X$-module.
...
\begin{lemma}
\label{lemma-coherent-change-rings}
\begin{reference}
\cite[Chapter I, \S 2, no. 16, Theorem 3, p. 211]{FAC}
\end{reference}
Let $(X, \mathcal{O}_X)$ be a ringed space such that $\mathcal{O}_X$ is
coherent as an $\mathcal{O}_X$-module. Let
$\mathcal{I} \subset \mathcal{O}_X$ be a sheaf of ideals which is coherent
as an $\mathcal{O}_X$-module. Let $\mathcal{F}$ be an
$\mathcal{O}_X/\mathcal{I}$-module. Then $\mathcal{F}$ is coherent as an
$\mathcal{O}_X/\mathcal{I}$-module if and only if it is coherent as an
$\mathcal{O}_X$-module. Moreover, $\mathcal{O}_X/\mathcal{I}$ is coherent
as a module over itself.
\end{lemma}

\begin{proof}
Set $\mathcal{O}' = \mathcal{O}_X/\mathcal{I}$. An $\mathcal{O}'$-module
is of finite type over $\mathcal{O}'$ if and only if it is of finite type
over $\mathcal{O}_X$.

Suppose first that $\mathcal{F}$ is coherent over $\mathcal{O}_X$. Let
$U \subset X$ be open and let $s_1, \ldots, s_n$ be sections of
$\mathcal{F}$ over $U$. The sheaf of relations between the $s_i$ over
$\mathcal{O}'|_U$ is the image of the sheaf of relations over
$\mathcal{O}_U$ under the map
$$
\mathcal{O}_U^{\oplus n} \longrightarrow (\mathcal{O}'|_U)^{\oplus n}.
$$
Thus it is of finite type over $\mathcal{O}_X$, hence over
$\mathcal{O}'$. This proves that $\mathcal{F}$ is coherent over
$\mathcal{O}'$.

By Lemma \ref{lemma-coherent-abelian}, $\mathcal{O}'$ is coherent as an
$\mathcal{O}_X$-module. Applying the implication just proved to
$\mathcal{F} = \mathcal{O}'$ shows that $\mathcal{O}'$ is coherent as a
module over itself.

Conversely, suppose that $\mathcal{F}$ is coherent over $\mathcal{O}'$.
By Lemma \ref{lemma-coherent-finite-presentation}, locally on $X$ it is the
cokernel of a map
$$
(\mathcal{O}')^{\oplus q} \longrightarrow
(\mathcal{O}')^{\oplus p}.
$$
Finite direct sums of $\mathcal{O}'$ are coherent over $\mathcal{O}_X$.
Another application of Lemma \ref{lemma-coherent-abelian} shows that
$\mathcal{F}$ is coherent over $\mathcal{O}_X$.
\end{proof}
...
\begin{reference}
\cite[Chapter I, \S 2, no. 17, Propositions 10 and 11 and Corollary,
pp. 211--212]{FAC}
\end{reference}
...
If $\mathcal{O}_X \to i_*\mathcal{O}_Z$ is an isomorphism, then
$i_*\mathcal{F}$ is coherent if and only if $\mathcal{F}$ is coherent.
In this case $\mathcal{O}_X$ is coherent as a module over itself if and
only if $\mathcal{O}_Z$ is coherent as a module over itself.
\end{lemma}
...
\medskip\noindent
Assume now that $\mathcal{O}_X \to i_*\mathcal{O}_Z$ is an isomorphism.
Let $U \subset X$ be open and set $V = i^{-1}(U)$. A finite collection
of sections of $\mathcal{F}$ over $V$ is the same as a finite collection
of sections of $i_*\mathcal{F}$ over $U$. Moreover, exactness of
pushforward by the closed immersion $V \to U$
(Lemma \ref{lemma-i-star-exact}) identifies the pushforward
of the sheaf of relations over $\mathcal{O}_Z|_V$ with the sheaf of
relations over $\mathcal{O}_X|_U$. The finite type assertion already
proved, applied to $V \to U$, shows that one relation sheaf is of finite
type if and only if the other is. The definition of coherence now gives
the second assertion. Taking $\mathcal{F} = \mathcal{O}_Z$ gives the last
assertion.
\end{proof}
...
as a $\Gamma(U, \mathcal{O}_U)$-module we may
...
\begin{reference}
\cite[Chapter I, \S 1, no. 10, Proposition 8, pp. 205--206]{FAC}
\end{reference}
Let $(X, \mathcal{O}_X)$ be a ringed space.
...
\begin{reference}
\cite[Chapter I, \S 1, no. 10, pp. 205--206]{FAC}
\end{reference}
Let $(X, \mathcal{O}_X)$ be a ringed space.
...
\begin{reference}
\cite[Chapter I, \S 2, no. 14, Proposition 4, p. 209]{FAC}
\end{reference}
Let $(X, \mathcal{O}_X)$ be a ringed space.
Let $\mathcal{F}$, $\mathcal{G}$ be $\mathcal{O}_X$-modules.
\begin{enumerate}
...
\begin{reference}
\cite[Chapter I, \S 1, no. 11, pp. 206--207]{FAC}
\cite[Chapter I, \S 2, no. 14, Proposition 5, pp. 209--210]{FAC}
\end{reference}
Let $(X, \mathcal{O}_X)$ be a ringed space.
Let $\mathcal{F}$, $\mathcal{G}$ be $\mathcal{O}_X$-modules.
\begin{enumerate}
...
\begin{reference}
\cite[Chapter I, \S 1, no. 11, pp. 206--207]{FAC}
\cite[Chapter III, \S 5, no. 73, Proposition 1(a) and proof,
pp. 267--268]{FAC}
\end{reference}
\begin{history}
No. 73 applies the stalk comparison below to the sheaves associated to a
finite graded module $M$ and an arbitrary graded module $N$ on projective
space. Its map is induced by the fraction rule
$m/P'\mapsto\varphi(m)/(PP')$. The projective comparison retains that
formula, while the present lemma supplies its general finite-presentation
stalk criterion.
\end{history}
Let $(X, \mathcal{O}_X)$ be a ringed space.
Let $\mathcal{F}$, $\mathcal{G}$ be $\mathcal{O}_X$-modules.
...
\begin{reference}
\cite[Chapter I, \S 2, no. 14, Proposition 6, p. 210]{FAC}
\end{reference}
Let $(X, \mathcal{O}_X)$ be a ringed space.
Let $\mathcal{F}$, $\mathcal{G}$ be $\mathcal{O}_X$-modules.
...
\begin{reference}
\cite[Chapter I, \S 2, no. 15, Proposition 9, p. 211]{FAC}
\end{reference}
Let $(X,\mathcal{O}_X)$ be a ringed space.
...
$U$ of $x$, an integer $n \geq 1$ and sections $s_i \in \mathcal{L}(U)$,
...
\longrightarrow \mathcal{O}_X
$$
Looking over an open covering trivializing $\mathcal{L}$, we see
...
\begin{reference}
\cite[Chapter III, \S 2, no. 59, pp. 251--252]{FAC}
\cite[Chapter III, \S 2, no. 60, Proposition 9, p. 252]{FAC}
\end{reference}
\begin{history}
For classical projective space with
$\mathcal{L}=\mathcal{O}_{\mathbf{P}^r}(1)$, the cited source denotes the
second graded module by $\Gamma(\mathcal{F})$ and defines its $S$-action by
tensoring a section of $\mathcal{O}(p)$ with a section of
$\mathcal{F}(q)$. The definition below separates the intrinsic section
ring from the given polynomial ring and applies to every ringed space with
an invertible sheaf.

No. 60 uses this integer-graded module for finite sums of twists and notes
that $\Gamma_*(\mathcal{O}_{\mathbf{P}^r})$ agrees with the polynomial
ring when $r \geq 1$, whereas for $r = 0$ it is not finitely generated:
sections in negative twist degrees persist. This is the boundary between
the full integer-graded construction and a finiteness condition imposed
only on its high-degree tail.
\end{history}
Let $(X, \mathcal{O}_X)$ be a ringed space.
...
$\mathcal{U} : X = \bigcup_{j \in J} U_j$ where $J \to \mathcal{P}(X)$,
...
\label{lemma-s-open}
\begin{slogan}
A (local) trivialisation of a line bundle
...
is an $\mathcal{O}_1$-linear map such that for all local sections
...
a sheaf of sets on $X$ and $\alpha : \mathcal{E} \to \mathcal{B}$
...
$$
and it suffices to show these maps are quasi-isomorphisms. To see this
...
\begin{proof}
Consider the maps of sheaves of rings
$$
