...
\begin{lemma}
\label{lemma-schemes-formal-moduli-no-infinitesimal-automorphisms}
\begin{reference}
\cite[Expos\'e 195, C, Proposition 4.1, pp. 385--386]{FGA}.
\end{reference}
Let $\Lambda$ be a Noetherian local ring with residue field $k$, and let
$X$ be a scheme over $k$. For a local object $A$ of
$\mathcal{A}_\Lambda$ (Formal Deformation Theory, Definition
\ref{formal-defos-definition-artinian-algebra-functor-cech}), with residue
field $\kappa(A)$, let $F(A)$ be the set of isomorphism classes of pairs
$(X_A, \alpha_A)$, where $X_A$ is a flat $A$-scheme and
$$
\alpha_A : X_A \times_{\Spec(A)} \Spec(\kappa(A))
\longrightarrow X \times_{\Spec(k)} \Spec(\kappa(A))
$$
is an isomorphism. Isomorphisms of pairs are required to respect $\alpha_A$.
For arbitrary $A$, define $F(A)$ as the product of these sets over the local
factors of $A$. If
$$
\text{Der}_k(\mathcal{O}_X, \mathcal{O}_X) = 0,
$$
then $F : \mathcal{A}_\Lambda \to \textit{Sets}$ is strictly
prorepresentable. If $X$ is proper over $k$, then the corresponding local
prorepresenting algebra is Noetherian.

If $X$ is smooth over $k$, the tangent space at the marked point is
canonically
$$
H^1(X, \mathcal{T}_{X/k}), \qquad
\mathcal{T}_{X/k} =
\SheafHom_{\mathcal{O}_X}(\Omega_{X/k}, \mathcal{O}_X).
$$
\end{lemma}

\begin{proof}
We first eliminate automorphisms, including after extension of the residue
field. Let $l/k$ be a finite field extension. Finite base change gives
$$
\text{Der}_l(\mathcal{O}_{X_l}, \mathcal{O}_{X_l}) =
\text{Der}_k(\mathcal{O}_X, \mathcal{O}_X) \otimes_k l = 0.
$$
Indeed, after pushing forward along the finite morphism $X_l \to X$, this is
the fact that sheaf homomorphisms into a finite direct sum commute with that
direct sum. Lemma \ref{lemma-schemes-TI} identifies this group with the
infinitesimal automorphisms of $X_l$. Formal Deformation Theory, Lemma
\ref{formal-defos-lemma-infaut-trivial} then shows that every automorphism of
a marked deformation over a local Artinian ring with residue field $l$ is
trivial.

We verify the two local equalizer tests of Formal Deformation Theory, Theorem
\ref{formal-defos-theorem-grothendieck-levelt-prorepresentability}. Let
$A \to B$ be one of the maps occurring in those tests, with $A$ local. If a
class over $B$ has equal pullbacks to $B \otimes_A B$, the equality supplies
an isomorphism between the two pullbacks. It is unique by the preceding
paragraph. Consequently its three pullbacks to
$B \otimes_A B \otimes_A B$ satisfy the cocycle condition automatically.

Suppose first that $B$ is a nonzero finite free $A$-module. The morphism
$\Spec(B) \to \Spec(A)$ is an fppf covering. Fppf descent data for algebraic
spaces are effective by Bootstrap, Lemma
\ref{bootstrap-lemma-descend-algebraic-space}. Thus the marked flat
$B$-scheme descends to an algebraic space $Y$ over $A$. Flatness descends by
Descent on Spaces, Lemma
\ref{spaces-descent-lemma-descending-property-flat}, and the marking descends
to an isomorphism
$Y_{\kappa(A)} \cong X_{\kappa(A)}$. Since
$Y_{\kappa(A)} \subset Y$ is a thickening, $Y$ is a scheme by More on
Morphisms of Spaces, Lemma
\ref{spaces-more-morphisms-lemma-thickening-scheme}. Full faithfulness in
fppf descent gives an equivalence between marked flat $A$-schemes and the
descent data under consideration.

Suppose next that $A \to B$ is injective and $B/A$ has length $1$. Put
$\mathfrak m = \mathfrak m_A$. The proof of Formal Deformation Theory, Lemma
\ref{formal-defos-lemma-length-one-cech-equalizer} gives
$$
\mathfrak mB = \mathfrak m
$$
and the required ring equalizer. Hence Descent, Lemma
\ref{descent-lemma-artinian-small-effective-descent-flat-modules} applies.
Choose an affine basis of $X$ together with all of its principal affine
refinements. The inverse image of any member of this basis in a marked
deformation over $B$ is affine: it is a nilpotent thickening of the
corresponding affine open after base change to $B/\mathfrak m$. Its coordinate
algebra is flat over $B$. The cited descent lemma descends the underlying
module to a flat $A$-module. Multiplication, the unit, and all restriction
maps descend uniquely by the full faithfulness part of that lemma, and their
identities can be checked after base change to $B$.

For completeness, these descended affine pieces have the required open
overlaps. If $V = D(f) \subset U$ is one of the chosen principal refinements,
lift $f$ to the descended coordinate algebra of $U$. After base change to
$B$, localization at this lift is the coordinate algebra of the inverse image
of $V$. Full faithfulness identifies its descent with the localization of the
descended algebra of $U$. The affine pieces therefore glue to a flat
$A$-scheme marked by $X$ on its special fibre. The same argument applied to
morphisms proves full faithfulness. Thus marked flat schemes over $A$ are
again equivalent to the relevant descent data over $B$.
...
F(B \otimes_A B)
$$
is an equalizer. The functor $F$ commutes with finite products by definition,
so the Grothendieck--Levelt theorem cited above proves strict
prorepresentability. Moreover, $F(K)$ is a singleton for every field
$K \in \mathcal{A}_\Lambda$. Thus the strict representing system has a unique
component through its $k$-point, and this component may be taken to consist of
local Artinian algebras with residue field $k$.

The tangent space at the marked $k$-point is the tangent space of
$\Deformationcategory_X$, because the two functors are equivalent after the
automorphisms have been eliminated. Lemma \ref{lemma-schemes-TI} identifies
it with
$$
\text{Ext}^1_{\mathcal{O}_X}(\NL_{X/k}, \mathcal{O}_X).
$$
If $X$ is smooth, the naive cotangent complex is represented by the finite
locally free module $\Omega_{X/k}$ in degree zero, and this Ext group is
$H^1(X, \mathcal{T}_{X/k})$.

Finally assume $X$ is proper. Lemma \ref{lemma-proper-schemes-TI} says that
the tangent space is finite dimensional. In a local strict prorepresenting
presentation, this bounds the relative cotangent spaces of all the Artinian
quotients. Since $\Lambda$ is Noetherian, its cotangent space is finite
dimensional as well, and hence the absolute cotangent spaces of the quotients
are bounded. The inverse limit is therefore Noetherian by the
bounded-cotangent-space argument of Formal Deformation Theory, Lemma
\ref{formal-defos-lemma-limit-artinian}.
\end{proof}

\begin{lemma}
\label{lemma-abelian-variety-formal-moduli-smooth}
\begin{reference}
\cite[Expos\'e 195, C, remarks following Proposition 4.1,
pp. 386--387]{FGA} and
\cite[Theorem 2.2.1, pp. 273--280]{Oort-local-moduli}.
\end{reference}
Let $\Lambda$ be a complete Noetherian local ring with residue field $k$,
and let $X$ be an abelian variety of dimension $g$ over $k$. For a local
object $A$ of $\mathcal{A}_\Lambda$, let $F(A)$ be the set of isomorphism
classes of abelian schemes $X_A$ over $A$ endowed with an isomorphism
$$
X_A \times_{\Spec(A)} \Spec(\kappa(A))
\longrightarrow
X \times_{\Spec(k)} \Spec(\kappa(A))
$$
of abelian schemes. For arbitrary $A$, define $F(A)$ as the product over
the local factors. Then $F$ is strictly prorepresentable. The local
prorepresenting algebra at the marked $k$-point is isomorphic to
$$
\Lambda[[t_{ij} \mid 1 \leq i, j \leq g]].
$$
In particular, the deformation functor of $X$ as an abelian scheme is
formally smooth over $\Lambda$ and has tangent space of dimension $g^2$.
Moreover, forgetting the group law identifies its restriction to
$\mathcal{C}_\Lambda$ with the functor of isomorphism classes of marked
deformations of the underlying scheme $X$.
\end{lemma}

\begin{proof}
We first discuss prorepresentability. We use the rigidity fact that a
homomorphism between abelian schemes over a local Artinian ring which is zero
on the closed fibre is zero. Consequently an automorphism of a marked abelian
scheme which is the identity on the closed fibre is the identity. This is also
the argument which shows that forgetting the group law gives the final
bijection in the statement; see
\cite[Proposition 2.2.6]{Oort-local-moduli}. Indeed, a section lifting the
origin exists by smoothness, the rigidity lemma supplies a unique group law
with that section as origin, and changing the lift of the origin translates
the resulting group scheme.

We may now use the two equalizer tests in Formal Deformation Theory, Theorem
\ref{formal-defos-theorem-grothendieck-levelt-prorepresentability}. The proof
is the same descent argument as in Lemma
\ref{lemma-schemes-formal-moduli-no-infinitesimal-automorphisms}. For a
nonzero finite free map $A \to B$, fppf descent gives the underlying marked
scheme over $A$; multiplication, inverse, and the identity section descend by
full faithfulness. For an injective map $A \to B$ with $B/A$ of length $1$,
the affine-basis argument and effective descent for flat modules used there
descend the underlying scheme and the three structure morphisms. The group
identities may be checked after base change to $B$. Properness, finite
presentation, and smoothness follow from the corresponding properties of the
closed fibre. Rigidity makes the descent isomorphism unique, and hence makes
its cocycle condition automatic. Thus the two diagrams
$$
F(A) \longrightarrow F(B)
\mathrel{\substack{\longrightarrow \\[-0.6ex] \longrightarrow}}
F(B \otimes_A B)
$$
are equalizers. The cited theorem proves strict prorepresentability.

It remains to prove formal smoothness. Let $B \to A$ be a small extension in
$\mathcal{C}_\Lambda$ with kernel $I$, and let $X_A$ be an abelian scheme
deforming $X$. The obstruction to lifting its underlying smooth scheme lies
in
$$
\omega(X_A) \in H^2(X, \mathcal{T}_{X/k}) \otimes_k I
$$
by Cotangent Complex, Lemma
\ref{cotangent-lemma-find-obstruction-ringed-spaces}. Put
$P_A = X_A \times_A X_A$. The two projections and the Kunneth decomposition
(Varieties, Lemma \ref{varieties-lemma-kunneth}) give injections
$$
i_1, i_2 : H^2(X, \mathcal{T}_{X/k}) \otimes_k I
\longrightarrow H^2(X \times_k X, \mathcal{T}_{X \times_k X/k}) \otimes_k I.
$$
The product construction of the obstruction gives
$$
\omega(P_A) = i_1(\omega(X_A)) + i_2(\omega(X_A)).
$$
Consider the automorphism $a : P_A \to P_A$ given by
$a(x, y) = (x + y, y)$. Functoriality of the obstruction under this
automorphism gives
$$
\omega(P_A) = (a_0^*)^{-1}d a_0(\omega(P_A))
= 2i_1(\omega(X_A)) + i_2(\omega(X_A)).
$$
The two displayed equalities imply $i_1(\omega(X_A)) = 0$. Since $i_1$ is
injective, $\omega(X_A) = 0$. This argument, including the product formula,
is the characteristic-free obstruction calculation in
\cite[pp. 279--280]{Oort-local-moduli}. Thus the underlying scheme lifts to
$B$, and the forgetful bijection already proved equips the lift with an
abelian-scheme structure. Hence $F|_{\mathcal{C}_\Lambda}$ is smooth.

Finally, Groupoids, Lemma
\ref{groupoids-lemma-group-scheme-module-differentials} identifies
$\mathcal{T}_{X/k}$ with the trivial vector bundle having fibre the tangent
space of $X$ at the origin. The classical identification
$\dim_k H^1(X, \mathcal{O}_X) = g$ therefore gives
$$
\dim_k H^1(X, \mathcal{T}_{X/k}) = g^2.
$$
This is the tangent space by Lemma \ref{lemma-schemes-TI}. Formal Deformation
Theory, Lemma \ref{formal-defos-lemma-smooth} now identifies the local
prorepresenting algebra with the displayed power series ring.
\end{proof}

\begin{lemma}
\label{lemma-smooth-proper-curve-formal-moduli-dimension}
\begin{reference}
\cite[Expos\'e 195, C, remarks following Proposition 4.1, p. 386]{FGA}.
\end{reference}
Let $k$ be an algebraically closed field, let $\Lambda$ be a complete
Noetherian local ring with residue field $k$, and let $X$ be a smooth proper
connected curve of genus $g$ over $k$. The functor on
$\mathcal{C}_\Lambda$ of isomorphism classes of flat deformations of $X$,
marked on the closed fibre, is prorepresented by
$$
\begin{cases}
\Lambda & \text{if } g = 0,\\
\Lambda[[t]] & \text{if } g = 1,\\
\Lambda[[t_1, \ldots, t_{3g - 3}]] & \text{if } g \geq 2.
\end{cases}
$$
In every case the formal moduli scheme is smooth over $\Lambda$.
\end{lemma}

\begin{proof}
If $g = 0$, then $X \cong \mathbf{P}^1_k$ by Algebraic Curves, Proposition
\ref{curves-proposition-projective-line}. A deformation over an Artinian
local $\Lambda$-algebra is proper and flat, and hence is $\mathbf{P}^1$ over
that algebra by Deformation Theory, Lemma
\ref{defos-lemma-projective-space-rigid-complete-local}. An automorphism of
$\mathbf{P}^1_k$ lifts because an invertible $2$ by $2$ matrix lifts over a
surjection of local rings. Thus the marking does not introduce a second
isomorphism class, and the functor is the one-point functor represented by
$\Lambda$.

Suppose $g = 1$. Choose a point of $X(k)$ and use it as the origin of the
resulting abelian variety. The final assertion of Lemma
\ref{lemma-abelian-variety-formal-moduli-smooth} identifies deformations of
this abelian variety with deformations of its underlying curve. The same
lemma, with $g = 1$, gives the ring $\Lambda[[t]]$.

Suppose $g \geq 2$. Algebraic Curves, Lemma
\ref{curves-lemma-smooth-vector-fields} gives
$H^0(X, \mathcal{T}_{X/k}) = 0$. Lemma
\ref{lemma-schemes-formal-moduli-no-infinitesimal-automorphisms} therefore
gives a Noetherian prorepresenting algebra $R$. The deformation category is
unobstructed by Lemma \ref{lemma-curve-isolated-lci}; a smooth curve is a
local complete intersection and has empty singular locus. Formal Deformation
Theory, Lemma \ref{formal-defos-lemma-smooth-power-series-classical} shows
that $R$ is a power series ring over $\Lambda$ with one variable for each
dimension of $H^1(X, \mathcal{T}_{X/k})$.

For a smooth curve, $\Omega_{X/k} \cong \omega_X$, and hence
$\mathcal{T}_{X/k} \cong \omega_X^{-1}$. Riemann--Roch (Algebraic Curves,
Lemma \ref{curves-lemma-rr}) gives
$$
\chi(X, \mathcal{T}_{X/k})
= (2 - 2g) - \frac{1}{2}(2g - 2) = 3 - 3g.
$$
The zeroth cohomology vanishes as above, so
$\dim_k H^1(X, \mathcal{T}_{X/k}) = 3g - 3$, as desired.
\end{proof}

\begin{lemma}
\label{lemma-proper-smooth-formal-moduli-unobstructed}
\begin{reference}
\cite[Expos\'e 182, Th\'eor\`eme 10, p. 209]{FGA}
\end{reference}
Let $X$ be a smooth proper scheme over a field $k$. Set
$$
\mathcal{T}_{X/k} =
\SheafHom_{\mathcal{O}_X}(\Omega_{X/k}, \mathcal{O}_X).
$$
If
$$
H^0(X, \mathcal{T}_{X/k}) = H^2(X, \mathcal{T}_{X/k}) = 0,
$$
then the functor of isomorphism classes of flat deformations of $X$ over
local Artinian $k$-algebras is prorepresented by
$$
k[[x_1, \ldots, x_r]],
\qquad r = \dim_k H^1(X, \mathcal{T}_{X/k}).
$$
In particular, $X$ has a universal unobstructed formal deformation over a
regular complete local $k$-algebra.
\end{lemma}

\begin{proof}
Let $F$ be the functor of isomorphism classes in
$\Deformationcategory_X$. Since $X$ is smooth, Lemma \ref{lemma-schemes-TI}
identifies its infinitesimal automorphisms with
$H^0(X, \mathcal{T}_{X/k})$ and its tangent space with
$H^1(X, \mathcal{T}_{X/k})$. Thus Lemma \ref{lemma-schemes-hull} gives a
prorepresenting object $R$ for $F$.

\medskip\noindent
We claim that $F$ is smooth. By Formal Deformation Theory, Lemma
\ref{formal-defos-lemma-smoothness-small-extensions}, it suffices to lift a
deformation $Y \to \Spec(A)$ across a small extension $B \to A$ with kernel
$I$. The morphism $Y \to \Spec(A)$ is smooth: it is flat, locally of finite
presentation, and its only fibre is $X$. Cotangent Complex, Lemma
\ref{cotangent-lemma-find-obstruction-ringed-spaces} places the obstruction in
$$
\Ext^2_{\mathcal{O}_Y}
(L_{Y/A}, \mathcal{O}_X \otimes_k I)
= \Ext^2_{\mathcal{O}_X}
(\Omega_{X/k}, \mathcal{O}_X \otimes_k I)
= H^2(X, \mathcal{T}_{X/k}) \otimes_k I = 0.
$$
Here the coefficient module is supported on the special fibre, and we used
smooth base change for differentials. The resulting first order thickening is
a scheme by More on Morphisms, Lemma
\ref{more-morphisms-lemma-first-order-thickening}, and is flat over $B$ by
More on Morphisms, Lemma \ref{more-morphisms-lemma-deform}. Hence it is the
required deformation and the claim follows.

\medskip\noindent
The morphism from the prorepresentable functor $\underline{R}$ to the final
functor is therefore smooth. Formal Deformation Theory, Lemma
\ref{formal-defos-lemma-smooth-morphism-power-series} shows that
$R \cong k[[x_1, \ldots, x_r]]$. The number of variables is the dimension of
the tangent space computed above.
\end{proof}
...
\section{Finite locally free coverings}
\label{section-finite-locally-free-coverings}

\noindent
We formulate the extension problem for coverings in terms of an adic system.
The flatness over the coefficient ring in the following situation is
essential; see Remark \ref{remark-formal-coverings-missing-flatness}. The
use of the reduced coefficient algebra in the marking is essential as well;
see Remark \ref{remark-formal-coverings-reduced-marking}.

\begin{situation}
\label{situation-formal-coverings}
Let $(\Lambda, \mathfrak m, k)$ be a complete Noetherian local ring and put
$\Lambda_n = \Lambda/\mathfrak m^{n + 1}$. Suppose given
\begin{enumerate}
\item Noetherian schemes $X_n$ flat over $\Lambda_n$ and cartesian
identifications
$$
X_{n + 1} \times_{\Spec(\Lambda_{n + 1})} \Spec(\Lambda_n) = X_n,
$$
\item compatible retrocompact opens $U_n \subset X_n$ such that, after every
base change by a finite $\Lambda_n$-algebra, the resulting open is scheme
theoretically dense, and
\item a finite locally free morphism $p_0 : Z_0 \to X_0$ whose restriction
over $U_0$ is finite \'etale.
\end{enumerate}
For $A \in \mathcal{A}_\Lambda$, choose $n$ such that
$\mathfrak m^{n + 1}A = 0$ and set
$$
X_A = X_n \times_{\Spec(\Lambda_n)} \Spec(A),
\qquad
U_A = U_n \times_{X_n} X_A.
$$
These schemes are independent of $n$ up to canonical isomorphism. Write
$A_{\mathrm{red}} = A/\sqrt{(0)}$. The map $\Lambda \to A_{\mathrm{red}}$
factors through $k$. Let $F(A)$ be the set of isomorphism classes of finite
locally free morphisms
$p_A : Z_A \to X_A$ endowed with an isomorphism
$$
Z_A \times_{\Spec(A)} \Spec(A_{\mathrm{red}})
\longrightarrow
Z_0 \times_{\Spec(k)} \Spec(A_{\mathrm{red}})
$$
over $X_0 \times_{\Spec(k)} \Spec(A_{\mathrm{red}})$.
The isomorphisms used in defining $F(A)$ are required to respect this marking.
\end{situation}

\begin{lemma}
\label{lemma-formal-coverings-rigidity}
\begin{reference}
\cite[Expos\'e 195, C, Section 5, p. 387]{FGA}.
\end{reference}
In Situation \ref{situation-formal-coverings}, let $A$ be an object of
$\mathcal{A}_\Lambda$ and let $Z_A$ and $Z'_A$ be two marked coverings. The
restriction map
$$
\Mor_{X_A}(Z_A, Z'_A)
\longrightarrow
\Mor_{X_{A_{\mathrm{red}}}}
(Z_A|_{X_{A_{\mathrm{red}}}}, Z'_A|_{X_{A_{\mathrm{red}}}})
$$
is injective. In particular, a marked covering has no nontrivial
automorphism inducing the identity on its reduction.
\end{lemma}

\begin{proof}
The nilradical of $A$ is nilpotent. Over $U_A$, the morphism
$Z'_A \to X_A$ is finite locally free and its reduction is finite \'etale.
It is unramified by More on Morphisms, Lemma
\ref{more-morphisms-lemma-properties-that-extend-over-thickenings}; since it
is already flat and of finite presentation, it is \'etale. In particular, it
is formally unramified. Thus two morphisms as in the statement which agree on
the reduction agree after restriction to $U_A$.

We finish on an affine open $V \subset X_A$. Write the restrictions of the
two coverings as $\Spec(C)$ and $\Spec(D)$. The two morphisms correspond to
two homomorphisms $D \to C$. Their difference vanishes after restriction to
$U_A \cap V$. The module $C$ is finite locally free over
$\mathcal{O}_{X_A}(V)$. Since $U_A \cap V$ is scheme theoretically dense,
Morphisms, Lemma
\ref{morphisms-lemma-characterize-scheme-theoretically-dense} shows that a
section of a finite locally free module which vanishes there is zero. Hence
the two algebra homomorphisms, and therefore the two morphisms, are equal.
\end{proof}

\begin{lemma}
\label{lemma-formal-coverings-local-completion}
\begin{reference}
\cite[Expos\'e 195, C, Section 5, p. 388, and the 1962 erratum,
p. 301]{FGA}.
\end{reference}
Fix $A$ in Situation \ref{situation-formal-coverings}. On open subsets
$V \subset X_A$, isomorphism classes of marked finite locally free coverings
form a Zariski sheaf $G_A$. For $x \in X_A$, finite presentation identifies
its stalk with the corresponding class set over $\Spec(\mathcal{O}_{X_A,x})$,
and completion induces an injection
$$
(G_A)_x
= G_A(\Spec(\mathcal{O}_{X_A,x}))
\longrightarrow
G_A(\Spec(\widehat{\mathcal{O}}_{X_A,x})).
$$
\end{lemma}

\begin{proof}
Finite locally free algebras, their markings, and their isomorphisms glue in
the Zariski topology. Lemma \ref{lemma-formal-coverings-rigidity} makes every
isomorphism compatible with a marking unique. Hence local isomorphism classes
glue, not merely the underlying groupoid, and $G_A$ is a sheaf. The usual
finite-presentation limit argument spreads a covering and its marking from a
local ring to a neighbourhood, giving the assertion about the stalk.

Put $R = \mathcal{O}_{X_A,x}$. The map $R \to \widehat R$ is faithfully flat
by Algebra, Lemma \ref{algebra-lemma-completion-faithfully-flat}. The pullback
of $U_A$ remains scheme theoretically dense by Morphisms, Lemma
\ref{morphisms-lemma-flat-morphism-scheme-theoretically-dense-open}. Suppose
two classes over $R$ become isomorphic over $\widehat R$. By Lemma
\ref{lemma-formal-coverings-rigidity}, this isomorphism is unique. Its two
pullbacks to $\widehat R \otimes_R \widehat R$ are consequently equal, so it
is an fpqc descent datum. Descent of morphisms gives a unique isomorphism over
$R$, proving injectivity. This is the inclusion in the corrected formula; it
is not an equality in general.
\end{proof}
...
\begin{lemma}
\label{lemma-formal-coverings-limit-algebraize}
\begin{reference}
\cite[Expos\'e 195, C, Section 5, p. 388]{FGA}.
\end{reference}
Let $(Z_n \to X_n)$ be a compatible system of marked finite locally free
coverings in Situation \ref{situation-formal-coverings}.
\begin{enumerate}
\item The isomorphism class of the system is the same datum as a compatible
element of $\lim_n G_{\Lambda_n}(X_n)$.
\item Suppose there is a proper scheme $X$ over $\Lambda$ with
$X_n = X \times_\Lambda \Lambda_n$. Then the system algebraizes uniquely to
a finite locally free covering $Z \to X$ with the prescribed reductions.
\end{enumerate}
\end{lemma}

\begin{proof}
For (1), an equality between the restriction of the class at level $n + 1$
and the class at level $n$ supplies an isomorphism. It is unique by Lemma
\ref{lemma-formal-coverings-rigidity}; hence these isomorphisms automatically
satisfy every compatibility condition and define a unique adic system.

For (2), Cohomology of Schemes, Lemma
\ref{coherent-lemma-algebraize-formal-scheme-finite-over-proper} algebraizes
the compatible finite morphisms and their algebra structures. Let
$\mathcal{B}$ be the resulting coherent $\mathcal{O}_X$-algebra. At a point
$x \in X$, every quotient of $\mathcal{B}_x$ by a power of
$\mathfrak m\mathcal{O}_{X,x}$ is finite locally free over the corresponding
quotient of $\mathcal{O}_{X,x}$. Algebra, Lemma
\ref{algebra-lemma-flat-module-powers} therefore shows that
$\mathcal{B}_x$ is flat over $\mathcal{O}_{X,x}$. It is finite, and hence
finite free over the local ring $\mathcal{O}_{X,x}$. Thus $Z \to X$ is finite
locally free. Full faithfulness in Grothendieck existence, or Cohomology of
Schemes, Lemma \ref{coherent-lemma-algebraize-morphism}, gives uniqueness and
algebraizes the marking.
\end{proof}

\begin{proposition}
\label{proposition-formal-coverings-prorepresentable}
\begin{reference}
\cite[Expos\'e 195, C, Proposition 5.1, pp. 388--389]{FGA}.
\end{reference}
In Situation \ref{situation-formal-coverings}, the functor
$$
F : \mathcal{A}_\Lambda \longrightarrow \textit{Sets}
$$
is strictly prorepresentable.
\end{proposition}

\begin{proof}
The functor commutes with finite products. We verify the two local Cech tests
of Formal Deformation Theory, Theorem
\ref{formal-defos-theorem-grothendieck-levelt-prorepresentability}.

Let $A \to B$ be a map in $\mathcal{A}_\Lambda$ with $A$ local. If $B$ is a
nonzero finite free $A$-module, then $A \to B$ is faithfully flat. Finite
locally free algebras satisfy fpqc descent. If a class over $B$ has equal
pullbacks to $B \otimes_A B$, the required isomorphism between those pullbacks
is unique by Lemma \ref{lemma-formal-coverings-rigidity}; hence it
automatically satisfies the cocycle condition and the covering descends. Put
$K=A_{\mathrm{red}}$. Faithful flatness shows that
$K \to B_{\mathrm{red}}$ is injective. It is a finite map from a field to a
nonzero reduced Artinian ring, and hence is faithfully flat. The marking and
its compatibility descend along this map. This proves the first equalizer
test.

Suppose instead that $A \to B$ is injective and $B/A$ has length $1$.
Formal Deformation Theory, Lemma
\ref{formal-defos-lemma-length-one-cech-equalizer} gives the ring equalizer
and its proof gives $\mathfrak nB = \mathfrak n$, where $\mathfrak n$ is the
maximal ideal of $A$. Consider a class over $B$ whose two pullbacks are equal.
As above, rigidity turns the unique isomorphism into a descent datum.

We descend it on an affine $V = \Spec(R) \subset X_A$. The ring $R$ is flat
over $A$ by the flatness assumption in Situation
\ref{situation-formal-coverings}. Let $C$ be the finite locally free algebra
of the covering on $V_B$. It is flat as a $B$-module. Descent, Lemma
\ref{descent-lemma-artinian-small-effective-descent-flat-modules} produces an
$A$-flat module $M$ with $M \otimes_A B \cong C$. Full faithfulness for flat
modules first descends the $R$-action. Put $K=A/\mathfrak n$ and
$\overline B=B_{\mathrm{red}}$. The injection $A \to B$ induces an
injection $K \to \overline B$: an element of $A$ whose image in $B$ is
nilpotent was already nilpotent. Thus $K \to \overline B$ is faithfully
flat. We have
$$
(M/\mathfrak nM) \otimes_K \overline B
= C \otimes_B \overline B.
$$
The module on the right is finite locally free over
$(R/\mathfrak nR) \otimes_K \overline B$. Hence
$M/\mathfrak nM$ is finite locally free over $R/\mathfrak nR$ by fpqc
descent. Since $\mathfrak n$ is nilpotent, Algebra, Lemma
\ref{algebra-lemma-criterion-flatness-fibre-nilpotent} shows that $M$ is flat
over $R$. Lifting finitely many generators modulo $\mathfrak n$ shows that
$M$ is finite over $R$, and hence it is finite locally free.

Now $R$, $M$, and $M \otimes_R M$ are flat over $A$. Full faithfulness in the
same descent lemma descends the unit and multiplication of $C$. The algebra
identities hold because they hold after base change to $B$. The descended
algebras agree on affine overlaps by uniqueness and therefore glue. The
given marking over $\overline B$ is compatible with this descent datum. It
descends along the faithfully flat map $K \to \overline B$ to the required
marking over $K$. We have proved the second equalizer test, and the
Grothendieck--Levelt criterion proves the proposition.
\end{proof}

\begin{remark}
\label{remark-formal-coverings-trivial-etale}
If $U_0 = X_0$, then every covering in the situation is finite \'etale.
Invariance of finite \'etale coverings under nilpotent thickenings (Fundamental
Groups, Lemma \ref{pione-lemma-thickening}) makes $F$ the one-point functor,
represented by $\Lambda$. In the ramified case the strict prorepresenting
topological algebra need not be Noetherian.
\end{remark}

\begin{remark}
\label{remark-formal-coverings-reduced-marking}
The reduced-fibre marking in Situation
\ref{situation-formal-coverings} is necessary when
$\mathcal{A}_\Lambda$ contains all finite-length $\Lambda$-algebras. A
marking only over $A/\mathfrak mA$, as in the literal formulation in
Expos\'e 195, is not stable enough for the length-one equalizer test.

For an example, let $k$ have characteristic different from $2$ and set
$$
\Lambda=k[\epsilon]/(\epsilon^2),
\qquad
A=k[\epsilon,a]/(\epsilon^2,a^2,\epsilon a),
\qquad
B=k[\epsilon,b]/(\epsilon^2,b^2).
$$
Make $A$ a subalgebra of $B$ by $a=\epsilon b$. Then $B/A$ has length $1$
over the local ring $A$. Moreover,
$$
\mathfrak mA=(\epsilon)
\quad\text{whereas}\quad
\mathfrak mB=(\epsilon,\epsilon b)=(\epsilon,a).
$$
Take $X=\Spec(\Lambda[z])$, $U=D(z)$, and the fixed covering
$Z_0=\Spec(k[z,s]/(s^2-z))$. The finite locally free covering over $X_A$
defined by
$$
s^2=z+a
$$
is finite \'etale over $U_A$. After base change to $B$ it has the required
$B/\mathfrak mB$-marking and its two pullbacks to $B\otimes_A B$ agree. It
cannot descend as an $A/\mathfrak mA$-marked covering: its reduction there
has discriminant ideal $(4(z+a))$, whereas the fixed covering has
discriminant ideal $(4z)$. These ideals are distinct in
$k[a,z]/(a^2)$. Flat-module descent and rigidity show that no different
covering over $A$ can provide the missing preimage. Thus the literal functor
fails the Cech equalizer property even though $X$ is flat over $\Lambda$.

Marking over $A_{\mathrm{red}}$ repairs this defect. For a local $A$ this is
its residue field, and for every injective test map $A \to B$ the induced map
$A_{\mathrm{red}} \to B_{\mathrm{red}}$ is faithfully flat. Notice that for
$A=\Lambda_n$ the reduced fibre is still $X_0$, so the adic system and its
algebraization retain the prescribed covering $Z_0$.
\end{remark}

\begin{remark}
\label{remark-formal-coverings-missing-flatness}
The statement of Expos\'e 195, Proposition 5.1 does not explicitly assume
flatness of the formal scheme over $\Lambda$. Its 1962 erratum strengthens the
nonzerodivisor condition defining $U$ and replaces an incorrect equality with
the completion inclusion of Lemma
\ref{lemma-formal-coverings-local-completion}, but it does not add flatness.
Without flatness the prorepresentability assertion is false.

Indeed, let $k$ have characteristic different from $2$, put
$$
B = k[t]/(t^4),
\qquad
\Lambda = k \oplus kt^2 \oplus kt^3 \subset B,
$$
and write $x=t^2$, $y=t^3$. Thus
$\Lambda \cong k[x,y]/(x,y)^2$, and $B/\Lambda$ has length $1$ as a
$\Lambda$-module. Set
$$
R = (\Lambda/(x))[z] = k[y,z]/(y^2),
\qquad X = \Spec(R),
\qquad U = D(z).
$$
The element $z$ is a nonzerodivisor both on $X$ and on its closed fibre. The
two rank-two coverings
$$
\Spec(R[s]/(s^2-z)) \longrightarrow X,
\qquad
\Spec(R[s]/(s^2-z-y)) \longrightarrow X
$$
have the same marked closed fibre and are finite \'etale over $U$. They are not
isomorphic: their discriminant ideals are $(4z)$ and $(4(z+y))$, which are
distinct because $y$ is nonzero modulo $(z)$. After base change along the
injective length-one map $\Lambda \to B$, they become identical, since
$(x)B=(t^2,t^3)$ kills $y$. Thus the map on covering classes is not injective.
Every strictly prorepresentable functor sends an injective map of coefficient
algebras to an injective map of sets, which gives the contradiction. The
flatness hypothesis in Situation \ref{situation-formal-coverings} prevents
exactly this failure of base change.
\end{remark}


\begin{lemma}
\label{lemma-formal-coverings-curve-local-product}
\begin{reference}
\cite[Expos\'e 195, C, Section 5, p. 389, together with the 1962 erratum,
p. 301]{FGA}.
\end{reference}
Fix $A$ in Situation \ref{situation-formal-coverings}. Suppose that
$$
X_A \setminus U_A = \{x_1, \ldots, x_r\}
$$
is a finite set of closed points. Then restriction induces a bijection
$$
G_A(X_A) \longrightarrow \prod_{i = 1}^r (G_A)_{x_i}.
$$
Suppose in addition that, for
$R_i = \mathcal{O}_{X_A, x_i}$, there is an element $f_i \in R_i$ such that
$$
U_A \times_{X_A} \Spec(R_i) = D(f_i)
$$
and such that the completion $\widehat R_i$ is the $f_i$-adic completion.
Then completion induces a bijection
$$
G_A(X_A) \longrightarrow
\prod_{i = 1}^r G_A(\Spec(\widehat R_i)).
$$
In particular, the additional hypotheses hold in the one dimensional setup
where the complement is locally cut out by the nonzerodivisor used to define
$U_A$.
\end{lemma}

\begin{proof}
The restriction of $G_A$ to $U_A$ is the one-point sheaf: finite \'etale
coverings and their morphisms lift uniquely across nilpotent thickenings by
Fundamental Groups, Lemma \ref{pione-lemma-thickening}. Given germs
$s_i \in (G_A)_{x_i}$, represent them on neighbourhoods $V_i$ of $x_i$.
Shrinking $V_i$, we may assume that $V_i$ contains none of the other $x_j$.
Every intersection $V_i \cap V_j$ is contained in $U_A$, so the sections
agree there. They also agree with the unique section over $U_A$. The sheaf
property in Lemma \ref{lemma-formal-coverings-local-completion} glues them to
a section over $X_A$. The same argument proves uniqueness. This gives the
first bijection.

It remains to prove that the completion map at $x_i$ is surjective under the
additional hypotheses. Set $R = R_i$, $f = f_i$, and
$S = \widehat R_i$. A class over $S$ restricts over $S_f$ to a finite \'etale
covering. Its marking and invariance under nilpotent thickenings give a unique
isomorphism with the pullback of the fixed covering over $R_f$. More on
Algebra, Theorem \ref{more-algebra-theorem-formal-glueing} and Remark
\ref{more-algebra-remark-formal-glueing-algebras} glue the two covering
algebras and this isomorphism to a finite locally free $R$-algebra. The same
tensor-compatible equivalence glues the marking. Thus every completed-local
class comes from a class over $R$. Injectivity is Lemma
\ref{lemma-formal-coverings-local-completion}, and the second bijection
follows.

For the final assertion, the punctured spectrum of each one dimensional
local ring is $D(f_i)$. Since $V(f_i)$ is the closed point, the $f_i$-adic
and maximal-adic topologies are equivalent.
\end{proof}


\begin{remark}
\label{remark-formal-coverings-normal-not-smooth}
\begin{reference}
\cite[Expos\'e 195, C, Section 5, pp. 389--390]{FGA}.
\end{reference}
Normality of the closed fibre does not by itself make the formal moduli space
in Proposition \ref{proposition-formal-coverings-prorepresentable} formally
smooth. Here is an explicit obstruction.

Let $k$ be a field, let $\Lambda = k[[\epsilon]]$, and put
$\Lambda_n = \Lambda/(\epsilon^{n + 1})$. Take
$$
R_n = (\Lambda_n[t])_{(\epsilon, t)},
\qquad
X_n = \Spec(R_n),
\qquad
U_n = D(t).
$$
This is a flat system in Situation \ref{situation-formal-coverings}:
localizations of polynomial algebras are flat, and $t$ remains a
nonzerodivisor after every coefficient base change. Thus
$X_0 = \Spec(k[t]_{(t)})$ is normal. Over $R_0 = k[t]_{(t)}$, let $C_0$
be free with basis $1, x_1, x_2, x_3$ and multiplication
$$
x_i^2 = t x_i,
\qquad
x_i x_j = 0 \quad (i \ne j).
$$
It is a finite locally free covering algebra. Indeed, it is the subalgebra
of $R_0^4$ generated by $1$ and $x_i = t e_i$ for three of the standard
idempotents, and $C_0[1/t] = R_0[1/t]^4$ is finite \'etale.

Set $A = \Lambda/(\epsilon^2)$ and
$R=(A[t])_{(\epsilon,t)}$. On the free $R$-module with the same basis,
define a commutative algebra $C$ by
\begin{align*}
x_1^2 &= t x_1 + \epsilon x_2, &
x_2^2 &= t x_2 + \epsilon x_3, &
x_3^2 &= t x_3, \\
x_1x_2 &= -\epsilon x_2, &
x_1x_3 &= 0, &
x_2x_3 &= -\epsilon x_3.
\end{align*}
A direct check on the unordered triples in $x_1,x_2,x_3$ shows that this
multiplication is associative modulo $\epsilon^2$. Its reduction is $C_0$,
and $C[1/t]$ is finite \'etale by invariance under nilpotent thickenings.
Consequently $C$ defines an element of $F(A)$.

This element does not lift to $A' = \Lambda/(\epsilon^3)$. Indeed, if a lift
existed, lift the displayed basis to it. Its multiplication would have the
form
$$
\widetilde\mu = \mu_0 + \epsilon\nu + \epsilon^2q,
$$
where $\mu_0$ is the multiplication on $C_0$, $\nu$ is the first-order term
displayed above, and $q$ is an $R_0$-bilinear correction. Write
$q_{ij}=q(x_i,x_j)$. The coefficient of $\epsilon^2$ in associativity for
$(x_1x_2)x_2=x_1(x_2^2)$ gives
$$
-x_3 + \mu_0(q_{12},x_2) - tq_{12} - \mu_0(x_1,q_{22}) = 0.
$$
Modulo $t$, if $a_0$ and $b_0$ are the scalar coefficients of $q_{12}$ and
$q_{22}$, respectively, this becomes
$$
-x_3 + a_0x_2 - b_0x_1 = 0.
$$
This is impossible because $1,x_1,x_2,x_3$ remain a basis modulo $t$.
Hence $F(A') \to F(A)$ is not surjective, even though $X_0$ is normal, and
the asserted formal smoothness fails.
\end{remark}
