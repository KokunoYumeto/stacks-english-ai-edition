...
\begin{lemma}
\label{lemma-quotient-lifts-torsor}
\begin{reference}
\cite[Expos\'e 221, Proposition 5.1 and Corollary 5.2]{FGA}.
\end{reference}
In Situation \ref{situation-q}, let $T \subset T'$ be a first order
thickening of schemes over $B$ with ideal sheaf $\mathcal{J}$. Let
\begin{equation}
\label{equation-quotient-to-lift}
0 \longrightarrow \mathcal{K} \longrightarrow \mathcal{F}_T
\longrightarrow \mathcal{Q} \longrightarrow 0
\end{equation}
define an element of $Q_{\mathcal{F}/X/B}(T)$. Set
$$
\mathcal{A} =
\SheafHom_{\mathcal{O}_{X_T}}
(\mathcal{K},
\mathcal{Q} \otimes_{\mathcal{O}_T} \mathcal{J}).
$$
On the common underlying topological space of $X_T$ and $X_{T'}$, let
$\mathcal{L}$ be the sheaf whose sections over an open $U$ are the lifts of
the restriction of (\ref{equation-quotient-to-lift}) to a quotient of
$\mathcal{F}_{T'}|_U$ flat over $T'$. Then $\mathcal{A}$ acts on
$\mathcal{L}$ and this action is simply transitive wherever
$\mathcal{L}$ is nonempty.

If lifts exist locally, then $\mathcal{L}$ is an $\mathcal{A}$-torsor. Its
class
$$
c(\mathcal{Q}) \in H^1(X_T, \mathcal{A})
$$
vanishes if and only if a global lift exists. If it vanishes, the set of
global lifts is a principal homogeneous space under
$H^0(X_T, \mathcal{A})$. In particular, a global lift exists if local lifts
exist and $H^1(X_T, \mathcal{A}) = 0$.
\end{lemma}

\begin{proof}
We identify sheaves on $X_T$ and $X_{T'}$ using the equivalence associated
to a thickening. Write $\mathcal{I}$ for the ideal of
$X_T \subset X_{T'}$. Suppose over an open $U$ we have a lift
$$
0 \to \mathcal{K}' \to \mathcal{F}_{T'}|_U
\to \mathcal{Q}' \to 0.
$$
Flatness of $\mathcal{Q}'$ gives canonical identifications
$$
\mathcal{I}\mathcal{Q}' =
\mathcal{Q} \otimes_{\mathcal{O}_T} \mathcal{J}
\quad\text{and}\quad
\mathcal{K}'/\mathcal{I}\mathcal{K}' = \mathcal{K}|_U;
$$
see Deformation Theory, Lemma
\ref{defos-lemma-deform-module-ringed-topoi}.

Given a second lift with quotient $\mathcal{Q}''$, the composite
$\mathcal{K}' \to \mathcal{F}_{T'}|_U \to \mathcal{Q}''$ has image in
$\mathcal{I}\mathcal{Q}''$ and annihilates
$\mathcal{I}\mathcal{K}'$. It therefore determines a unique map
$$
\mathcal{K}|_U \longrightarrow
(\mathcal{Q} \otimes_{\mathcal{O}_T}\mathcal{J})|_U.
$$
Conversely, for such a map $\varphi$, define
$\mathcal{K}'_\varphi \subset \mathcal{F}_{T'}|_U$ to consist of the
local sections whose reduction belongs to $\mathcal{K}$ and whose image
in $\mathcal{Q}'$ is $\varphi$ of that reduction. Then
$\mathcal{F}_{T'}|_U/\mathcal{K}'_\varphi$ reduces to $\mathcal{Q}$ and
has the same $\mathcal{I}$-multiple as $\mathcal{Q}'$.
The flatness criterion in the lemma cited above shows that it is flat over
$T'$. These two constructions are inverse to one another and are compatible
with restriction. This proves the assertion about the action.

Quotients and their kernels glue over open coverings, so $\mathcal{L}$ is a
sheaf. If it is locally nonempty, the first part makes it an
$\mathcal{A}$-torsor. The remaining assertions follow from Cohomology,
Lemmas \ref{cohomology-lemma-torsors-h1} and
\ref{cohomology-lemma-trivial-torsor}.
\end{proof}
...
\begin{lemma}
\label{lemma-cotangent-space-quot}
\begin{reference}
\cite[Expos\'e 221, Corollary 5.3]{FGA}.
\end{reference}
In Situation \ref{situation-quot}, put
$Q = \Quotfunctor_{\mathcal{F}/X/B}$ and assume that $Q$ is an algebraic
space (for example, this holds if $X \to B$ is separated by Proposition
\ref{proposition-quot}). Let $g : T \to Q$ correspond to
an exact sequence
$$
0 \longrightarrow \mathcal{K} \longrightarrow \mathcal{F}_T
\longrightarrow \mathcal{G} \longrightarrow 0.
$$
For every quasi-coherent $\mathcal{O}_T$-module $\mathcal{M}$ there is a
canonical isomorphism, functorial in $\mathcal{M}$,
\begin{equation}
\label{equation-cotangent-space-quot}
\Hom_{\mathcal{O}_T}(g^*\Omega_{Q/B}, \mathcal{M})
=
\Hom_{\mathcal{O}_{X_T}}
(\mathcal{K},
\mathcal{G} \otimes_{\mathcal{O}_T} \mathcal{M}).
\end{equation}
In particular, if $x : \Spec(k) \to Q_b$ is a point, then
$$
T_{Q_b, x} =
\Hom_{\mathcal{O}_{X_k}}(\mathcal{K}, \mathcal{G}).
$$
\end{lemma}

\begin{proof}
Let $T \subset T'$ be the split first order thickening with underlying
space $T$, structure sheaf
$\mathcal{O}_{T'} = \mathcal{O}_T \oplus \mathcal{M}$, and square zero
ideal $\mathcal{M}$. Give $T'$ its $B$-structure through the retraction
$T' \to T$. The set of lifts of $g$ to a $B$-morphism $T' \to Q$ is a
principal homogeneous space under the left hand side of
(\ref{equation-cotangent-space-quot}); this follows from More on Morphisms
of Spaces, Lemma \ref{spaces-more-morphisms-lemma-action-sheaf}. It has a
distinguished element obtained by composing $T' \to T$ with $g$.

By representability of $Q$ the same set is the set of lifts of the quotient
$\mathcal{F}_T \to \mathcal{G}$. Finite presentation and properness of the
support are unchanged by this nilpotent thickening. Lemma
\ref{lemma-quotient-lifts-torsor} identifies this set, with the same
distinguished element, as a principal homogeneous space under the right
hand side of (\ref{equation-cotangent-space-quot}). The difference
constructions in the two torsor actions give the displayed canonical
isomorphism. Taking $T = \Spec(k)$ and $\mathcal{M} = k$ gives the final
statement.
\end{proof}
...
\begin{reference}
\cite[Expos\'e 232, Proposition 2.1 and Corollary 2.4]{FGA}.
\end{reference}
In Situation \ref{situation-pic}.
...
$$
f_{ij} \in
\Gamma(X_{B_i \times_B B_j}, \mathcal{O}_{X_{B_i \times_B B_j}}^*) =
\Gamma(B_i \times_B B_j, \mathcal{O}_{B_i \times_B B_j}^*)
$$
...
\begin{proposition}
\label{proposition-formal-picard-prorepresentable-tangent}
\begin{reference}
\cite[Expos\'e 195, C, Proposition 3.1, p. 384, and the correction on
p. 301]{FGA}.
\end{reference}
Let $\Lambda$ be a Noetherian ring, let $S = \Spec(\Lambda)$, and let
$f : X \to S$ be a flat morphism of schemes. Assume that
$$
\mathcal{O}_T \longrightarrow f_{T, *}\mathcal{O}_{X_T}
$$
is an isomorphism for every scheme $T$ of finite type over $S$. Denote by
$\mathcal{A}_\Lambda$ the category of $\Lambda$-algebras which have finite
length as $\Lambda$-modules. Then the functor
$$
F : \mathcal{A}_\Lambda \longrightarrow \textit{Sets}, \qquad
A \longmapsto \Picardfunctor_{X/S}(\Spec(A))
$$
is strictly prorepresentable.

For $A \in \mathcal{A}_\Lambda$, put
$A_\epsilon = A[\epsilon]/(\epsilon^2)$. If $\xi \in F(A)$, then the fibre
$$
F(A_\epsilon, \xi) =
\{\xi' \in F(A_\epsilon) \mid \xi'|_A = \xi\}
$$
is canonically isomorphic to
$$
H^1(X_A, \mathcal{O}_{X_A}).
$$
\end{proposition}

\begin{proof}
The functor $F$ commutes with finite products. We verify the two local
equalizer tests of Formal Deformation Theory, Theorem
...
nonzero finite free $A$-module, then $\Spec(B) \to \Spec(A)$ is an fppf
cover. The required equalizer is therefore the sheaf condition in the
definition of $\Picardfunctor_{X/S}$.

Suppose that $A \to B$ is injective and $B/A$ has length $1$. If $\mathfrak m$
is the maximal ideal of $A$, then
$\mathfrak mB = \mathfrak m$ by Formal Deformation Theory, Lemma
\ref{formal-defos-lemma-length-one-cech-equalizer}. Put
$$
k = A/\mathfrak m, \qquad C = B/\mathfrak m.
$$
Thus $A = B \times_C k$ and $k \to C$ is faithfully flat. We recall why
invertible modules have effective descent in this situation. If
$U = \Spec(R)$ is an affine open of $X_A$, then $R$ is flat over $A$ and
tensoring the cartesian square of rings with $R$ gives
$$
R = (R \otimes_A B) \mathbin{\times}_{R \otimes_A C} (R \otimes_A k).
$$
Reduce the descent datum on an invertible module over $U_B$ modulo
$\mathfrak m$. Faithfully flat descent along $k \to C$ gives an invertible
module over $U_k$. More on Algebra, Lemmas
\ref{more-algebra-lemma-finite-module-over-fibre-product} and
\ref{more-algebra-lemma-flat-module-over-fibre-product} patch the two
modules to a finite flat module over $U$; it has rank $1$ after pullback to
$U_B$ and $U_k$, whose images cover $U$, and hence is invertible. Full
faithfulness in Descent, Lemma
\ref{descent-lemma-equalizer-flat-module-descent} makes these local descents
unique and hence glues them on overlaps. This is the scheme version of the
argument in Descent, Lemma
\ref{descent-lemma-artinian-small-effective-descent-flat-modules}.

It remains to pass from the Picard groupoid to the fppf sheaf of its
isomorphism classes. For every finite type $S$-scheme $D$ and every invertible
module $\mathcal{L}$ on $X_D$, the hypothesis gives
$$
\operatorname{Aut}(\mathcal{L}) =
\Gamma(X_D, \mathcal{O}_{X_D}^*) = \Gamma(D, \mathcal{O}_D^*).
$$
Hence the automorphisms in the preceding descent argument are exactly the
automorphisms pulled back from the base. Applying the same effective descent
argument to invertible modules on the bases shows that quotienting by these
automorphisms commutes with the equalizer. Consequently
$$
F(A) \longrightarrow F(B)
\mathrel{\substack{\longrightarrow \\[-0.6ex] \longrightarrow}}
F(B \otimes_A B)
$$
is an equalizer. The cited prorepresentability criterion now proves the first
assertion.

For the tangent assertion, $X_A \subset X_{A_\epsilon}$ is a first order
thickening with ideal $\mathcal{O}_{X_A}\epsilon$. More on Morphisms, Lemma
\ref{more-morphisms-lemma-picard-group-first-order-thickening} gives the
corresponding exact sequence of units, first cohomology, and Picard groups.
Our hypothesis identifies the map on global units with the surjection
$$
A_\epsilon^* \longrightarrow A^*.
$$
It follows that the kernel of restriction on Picard classes is canonically
$$
H^1(X_A, \mathcal{O}_{X_A}).
$$
Translation by the distinguished lift of $\xi$ obtained by base change
identifies the fibre in the statement with this kernel. The construction is
fppf local on $\Spec(A)$ and therefore applies to the sheafified Picard
functor.
\end{proof}

\begin{remark}
\label{remark-formal-picard-separable-correction}
The parenthetical reduction of the global-functions hypothesis in the cited
source has a correction: the phrase ``if $f$ is proper'' must read ``if $f$
is proper and separable''. The proposition above uses the base-change
hypothesis itself, so this correction does not enter its proof.
\end{remark}

\begin{lemma}
\label{lemma-flat-geometrically-connected-fibres-with-section}
\begin{reference}
\cite[Expos\'e 232, Proposition 2.1, Corollary 2.4, and Remark 2.5]{FGA}.
\end{reference}
...
\begin{lemma}
\label{lemma-picard-cech-quotient-with-local-section}
\begin{reference}
\cite[Expos\'e 232, Remark 2.6, p. 148]{FGA}.
\end{reference}
In Situation \ref{situation-pic}, assume that
$\mathcal{O}_U \to f_{U, *}\mathcal{O}_{X_U}$ is an isomorphism for every
scheme $U$ over $B$. Let $T \to B$ be a scheme and let $T' \to T$ be a
surjective fppf morphism such that $X_{T'} \to T'$ has a section. Set
$T'' = T' \times_T T'$. Then the diagram
$$
\Picardfunctor_{X/B}(T) \longrightarrow
\Pic(X_{T'})/\Pic(T')
\mathrel{\substack{\longrightarrow \\[-0.6ex] \longrightarrow}}
\Pic(X_{T''})/\Pic(T'')
$$
is an equalizer. In particular, every element of
$\Picardfunctor_{X/B}(T)$ is represented after pullback to $T'$ by an
invertible module on $X_{T'}$. If $X_T$ is a scheme and $X_T \to T$ is a
surjective fppf morphism, then one may take $T' = X_T$; the required section
after base change is the diagonal.
\end{lemma}

\begin{proof}
Since $\Picardfunctor_{X/B}$ is an fppf sheaf, the diagram
$$
\Picardfunctor_{X/B}(T) \longrightarrow
\Picardfunctor_{X/B}(T')
\mathrel{\substack{\longrightarrow \\[-0.6ex] \longrightarrow}}
\Picardfunctor_{X/B}(T'')
$$
is an equalizer. The section over $T'$ and either of its pullbacks to
$T''$ allow us to apply
Lemma \ref{lemma-flat-geometrically-connected-fibres-with-section} over
$T'$ and $T''$. This identifies the last two terms with the two quotients
in the statement. These identifications are compatible with pullback
because they are induced by the canonical maps from Picard groups to the
Picard functor.
\end{proof}
...
Here $g' : X_T \to X_{T'}$ is the base change of $g$.
\end{enumerate}
...
\begin{reference}
\cite[Expos\'e 232, Remark 2.5, p. 147]{FGA}; in the sentence describing
rigidified invertible modules, ``on $S$'' must read ``on $X$''.
\end{reference}
In Situation \ref{situation-pic} let $\sigma : B \to X$ be a section.
