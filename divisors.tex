...
\begin{reference}
\cite[Chapter III, \S 5, no. 75, proof of Theorem 3 and Remark,
pp. 269--270]{FAC}
\end{reference}
\begin{history}
The cited proof treats a nonsingular subvariety of projective space. At
each point it obtains a regular sequence of length equal to the codimension
which generates the ideal, and the subsequent remark isolates the locally
complete intersection hypothesis actually used. The lemma below is the
scheme-theoretic regular-immersion form of the regular ambient and regular
subscheme case; its proof reduces to the same regular-local quotient
statement.
\end{history}
...
\begin{lemma}
\label{lemma-coherent-subsheaf-meromorphic-functions}
\begin{reference}
\cite[Chapter II, \S 2, no. 40, pp. 232--233]{FAC}
\end{reference}
\begin{history}
In the classical terminology of the cited source, an
$\mathcal{O}_X$-submodule of the constant sheaf of rational functions is
called a sheaf of fractional ideals. No invertibility or local-freeness is
part of this terminology.
\end{history}
Let $X$ be a locally Noetherian integral scheme. Let $\mathcal{K}_X$ be
the sheaf of meromorphic functions.
\begin{enumerate}
\item An $\mathcal{O}_X$-submodule $\mathcal{F} \subset \mathcal{K}_X$
is coherent if and only if it is of finite type.
\item The $\mathcal{O}_X$-module $\mathcal{K}_X$ is coherent if and only if
$\dim(X) = 0$.
\end{enumerate}
\end{lemma}

\begin{proof}
A coherent module is of finite type by definition. Conversely, suppose that
$\mathcal{F}$ is of finite type. Fix $x \in X$. After replacing $X$ by an
affine open neighbourhood $U = \Spec(A)$ of $x$, we may choose
$f_1, \ldots, f_r \in \mathcal{K}_X(U)$ which generate
$\mathcal{F}|_U$. By Lemma
\ref{lemma-meromorphic-functions-integral-scheme}, we may view the $f_i$
as elements of the fraction field of $A$. Choose a nonzero $b \in A$ such
that $bf_i \in A$ for all $i$. Multiplication by $b$ identifies
$\mathcal{F}|_U$ with the finite type submodule
$b\mathcal{F}|_U \subset \mathcal{O}_U$. The module $\mathcal{O}_U$ is
coherent by Cohomology of Schemes, Lemma
\ref{coherent-lemma-coherent-Noetherian}; hence $b\mathcal{F}|_U$ is
coherent by Modules, Lemma \ref{modules-lemma-coherent-abelian}.
This proves (1).

If $\dim(X) = 0$, every local ring of the integral scheme $X$ is a field,
and hence $\mathcal{K}_X = \mathcal{O}_X$ is coherent. Conversely, suppose
that $\mathcal{K}_X$ is coherent. For every $x \in X$, the fraction field
of the domain $\mathcal{O}_{X, x}$ is then finite as an
$\mathcal{O}_{X, x}$-module, and hence integral over
$\mathcal{O}_{X, x}$. If $a \in \mathcal{O}_{X, x}$ is nonzero, a monic
equation for $a^{-1}$ shows that $a^{-1} \in \mathcal{O}_{X, x}$.
Thus every local ring of $X$ is a field and $\dim(X) = 0$.
\end{proof}
...
\begin{theorem}[Grothendieck's parafactoriality theorem]
\label{theorem-ci-parafactorial}
\begin{reference}
\cite[Expos\'e XI, Th\'eor\`eme 3.13(ii)]{SGA2}.
\end{reference}
Let $(A, \mathfrak m)$ be a Noetherian local ring which is a complete
intersection in the sense of Divided Power Algebra, Definition
\ref{dpa-definition-lci}. If $\dim(A)\geq 4$, then
$$
\Pic(\Spec(A)\setminus\{\mathfrak m\})=0.
$$
\end{theorem}

\begin{proof}
This is the local complete intersection case of Grothendieck's
parafactoriality theorem. Since every invertible module over a local ring is
free, the displayed vanishing is the Picard-group form of the cited result.
\end{proof}

\begin{theorem}[Samuel--Grothendieck]
\label{theorem-ci-Cartier-codimension-three}
\begin{reference}
\cite[Expos\'e XI, Corollaire 3.14]{SGA2}.
\end{reference}
Let $(A, \mathfrak m)$ be a Noetherian local ring which is a complete
intersection. Set $X=\Spec(A)$.
\begin{enumerate}
\item If $A$ is normal and a Weil divisor $D$ on $X$ is Cartier at every
prime of height at most $3$, then $D$ is Cartier.
\item If $A_\mathfrak p$ is a UFD for every prime $\mathfrak p$ of height at
most $3$, then $A$ is a UFD.
\item In particular, a complete intersection which is regular in codimension
$\leq 3$ is a UFD.
\end{enumerate}
\end{theorem}

\begin{proof}
We first prove (1) by induction on $\dim(A)$. There is nothing to prove when
$\dim(A)\leq 3$, because the hypothesis then applies at $\mathfrak m$.
Suppose $\dim(A)\geq 4$. For every nonmaximal prime $\mathfrak p$, the ring
$A_\mathfrak p$ is again a complete intersection by Divided Power Algebra,
Lemma \ref{dpa-lemma-ci-good}, and the hypothesis on $D$ localizes. Induction
therefore shows that $D$ is Cartier on
$$
U=X\setminus\{\mathfrak m\}.
$$
Theorem \ref{theorem-ci-parafactorial} says that $\Pic(U)=0$. Consequently
$D|_U$ is the divisor of a rational function $f$. The Weil divisor
$D-\text{div}(f)$ is supported at the closed point. Since this point has
codimension at least $4$, it cannot occur in a Weil divisor. Thus
$D=\text{div}(f)$, proving (1).

A complete intersection is Cohen--Macaulay and hence has property $(S_2)$.
Under the hypothesis of (2), its localizations at primes of height at most
$1$ are regular: they are zero-dimensional fields or one-dimensional local
normal domains. Serre's criterion for normality, Algebra, Lemma
\ref{algebra-lemma-criterion-normal}, shows that $A$ is normal. Every Weil
divisor is Cartier at the primes of height at most $3$ by hypothesis and
hence Cartier by (1). Thus every height one prime of $A$ is principal, so
$A$ is a UFD by Algebra, Lemma
\ref{algebra-lemma-characterize-UFD-height-1}. This proves (2). Part (3)
follows from (2) and More on Algebra, Lemma
