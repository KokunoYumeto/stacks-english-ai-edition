...
\section{The Grothendieck--Levelt criterion}
\label{section-grothendieck-levelt}

\noindent
The prorepresentability criterion in this section concerns all Artinian
algebras over a Noetherian ring, with varying residue fields. This is a
different category from the fixed-residue-field category
$\mathcal{C}_\Lambda$ used in the rest of this chapter.

\begin{definition}
\label{definition-artinian-algebra-functor-cech}
Let $\Lambda$ be a Noetherian ring. Denote $\mathcal{A}_\Lambda$ the category
of $\Lambda$-algebras which have finite length as $\Lambda$-modules. Given a
functor $F : \mathcal{A}_\Lambda \to \textit{Sets}$ and a ring map
$A \to B$ in $\mathcal{A}_\Lambda$, we say that $F$ has the
{\it Cech equalizer property for $A \to B$} if
$$
F(A) \longrightarrow F(B)
\mathrel{\substack{\longrightarrow \\[-0.6ex] \longrightarrow}}
F(B \otimes_A B)
$$
is an equalizer diagram. The two arrows are induced by
$b \mapsto b \otimes 1$ and $b \mapsto 1 \otimes b$.
\end{definition}

\begin{lemma}
\label{lemma-length-one-cech-equalizer}
Let $(A, \mathfrak m)$ be an Artinian local ring and let $A \to B$ be an
injective ring map. If $B/A$ has length $1$ as an $A$-module, then
$$
A \longrightarrow B
\mathrel{\substack{\longrightarrow \\[-0.6ex] \longrightarrow}}
B \otimes_A B
...
\begin{proof}
Put $Q = B/A$. We have $\mathfrak mQ = 0$, and hence
$\mathfrak mB \subset A$. The ideal $\mathfrak mB$ is nilpotent, so its
inclusion in $A$ implies $\mathfrak mB \subset \mathfrak m$. The reverse
inclusion is automatic, and consequently $\mathfrak mB = \mathfrak m$.
Tensoring
$$
A \longrightarrow B \longrightarrow Q \longrightarrow 0
$$
with $B$ over $A$ gives an exact sequence
$$
B \longrightarrow B \otimes_A B \longrightarrow Q \otimes_A B
\longrightarrow 0,
$$
where the first arrow is $b \mapsto 1 \otimes b$.
Suppose $b \otimes 1 = 1 \otimes b$. The image of $b \otimes 1$ in
$Q \otimes_A B$ is therefore zero. Since $Q$ is a one dimensional
$A/\mathfrak m$-vector space, the map
$$
Q \longrightarrow Q \otimes_A B, \qquad q \longmapsto q \otimes 1
$$
is injective: after choosing a basis of $Q$ it becomes the injective map
$A/\mathfrak m \to B/\mathfrak mB$. Thus the image of $b$ in $Q$ is zero,
so $b \in A$. The reverse inclusion is immediate.
\end{proof}

\begin{theorem}
\label{theorem-grothendieck-levelt-prorepresentability}
\begin{reference}
\cite[Expos\'e 195, B, Th\'eor\`eme 1, pp. 377--378]{FGA},
\cite[Section 4, Proposition 2, pp. 135--140]{Levelt-left-exact}, and
\cite[pp. 1--3]{Levelt-left-exact-correction}.
\end{reference}
Let $\Lambda$ be a Noetherian ring and let
$F : \mathcal{A}_\Lambda \to \textit{Sets}$ be a functor. The following are
equivalent.
\begin{enumerate}
\item The functor $F$ is strictly prorepresentable in the sense of Categories,
Definition
\ref{categories-definition-strict-prorepresentable-minimal-pair}.
\item The functor $F$ commutes with finite products and has the Cech equalizer
property for every $A \to B$ for which
$$
A \longrightarrow B
\mathrel{\substack{\longrightarrow \\[-0.6ex] \longrightarrow}}
B \otimes_A B
$$
is an equalizer diagram of rings.
\item The functor $F$ commutes with finite products and has the Cech equalizer
property whenever $A$ is local and either
\begin{enumerate}
\item $B$ is a nonzero finite free $A$-module, or
\item $A \to B$ is injective and $B/A$ has length $1$ as an $A$-module.
\end{enumerate}
\end{enumerate}
\end{theorem}

\begin{proof}
The category $\mathcal{A}_\Lambda$ is essentially small and has finite
limits. Moreover, every nonempty collection of subobjects of an object has a
minimal member: subobjects are in particular $\Lambda$-submodules of a finite
length $\Lambda$-module. Categories, Proposition
\ref{categories-corollary-artinian-left-exact-strict-prorepresentable}
therefore says that (1) is equivalent to $F$ being left exact.

If $F$ is left exact, then it preserves the equalizer diagrams in (2), so
(1) implies (2). Suppose (2) holds. A nonzero finite free algebra over a local
ring is faithfully flat. The ring equalizer in (2) is exact in this case by
Descent, Proposition \ref{descent-proposition-descent-module}, applied to the
base ring as a module. The length one case is
Lemma \ref{lemma-length-one-cech-equalizer}. Thus (2) implies (3).

Assume (3). These are the conditions traditionally denoted $(P)$, $(L)$, and
$(M)$: preservation of finite products, faithfully flat Cech equalizers out
of local rings, and Cech equalizers for minimal local extensions with trivial
residue extensions. For finite Artinian algebras, the maps in $(L)$ are finite
free; the length one formulation in (3) is the corresponding local form of
$(M)$. The corrected Grothendieck--Levelt devissage proves that these tests
imply that $F$ is left exact; see the last two references in the statement.

For clarity, the residue-field step in that devissage is as follows. After
reducing an equalizer problem to a local base $A$, one chooses a finite normal
extension containing the finitely many residue fields which occur and a
finite free local extension $A \to A'$ inducing it. The functor is first
shown to be left exact on the subcategory with this fixed residue field. The
Cech equalizers for $A \to A'$ and for its base changes then descend the
required equalizer from $A'$ to $A$. The 1970 correction supplies the
fixed-residue-field argument without the false minimal-extension assertion
used in the original proof. Hence $F$ is left exact, and (1) follows from the
Categories proposition cited above.
\end{proof}

\begin{remark}
\label{remark-levelt-correction}
The correction in the reference is essential. The last assertion of
Section 3, Lemma 5 of the 1969 paper is false: a simple monomorphism of
Artinian algebras which is not faithfully flat need not induce only trivial
residue field extensions. The criterion itself remains valid; the 1970 paper
replaces precisely the part of the proof which used that assertion.
\end{remark}

\begin{proposition}
\label{proposition-formal-hom-prorepresentable-tangent}
\begin{reference}
\cite[Expos\'e 195, C, Proposition 2.1, p. 382]{FGA}.
\end{reference}
Let $\Lambda$ be a Noetherian ring and let $X$ and $Y$ be schemes over
$\Lambda$. Assume that $X$ is flat over $\Lambda$. The functor
$$
F : \mathcal{A}_\Lambda \longrightarrow \textit{Sets}, \qquad
A \longmapsto \Mor_A(X_A, Y_A)
$$
...
$$
\Gamma\left(X_A,
\SheafHom_{\mathcal{O}_{X_A}}
(\xi^*\Omega_{Y_A/A}, \mathcal{O}_{X_A})\right).
$$
\end{proposition}

\begin{proof}
The functor $F$ commutes with finite products because the spectrum of a
product of rings is a disjoint union. We verify the two local equalizer tests
of Theorem \ref{theorem-grothendieck-levelt-prorepresentability}.

Let $A \to B$ be a map in $\mathcal{A}_\Lambda$ with $A$ local. If $B$ is a
nonzero finite free $A$-module, then $A \to B$ is faithfully flat. Morphisms
of schemes satisfy fpqc descent by Descent, Remark
\ref{descent-remark-morphisms-of-schemes-satisfy-fpqc-descent}; hence the
required diagram for $F$ is an equalizer.

Suppose instead that $A \to B$ is injective and $B/A$ has length $1$.
Lemma \ref{lemma-length-one-cech-equalizer} gives the ring equalizer, so
$\Spec(B) \to \Spec(A)$ is a finite effective epimorphism by Descent, Lemma
\ref{descent-lemma-finite-effective-epimorphism-functions}. Since $X_A$ is
flat over $A$, Descent, Lemma
\ref{descent-lemma-finite-effective-epimorphism-flat-schemes} gives precisely
the required equalizer of sets of morphisms. The theorem cited above now
proves that $F$ is strictly prorepresentable.

For the tangent statement, regard
$\Spec(A) \subset \Spec(A_\epsilon)$ as a first order thickening and base
change it to $X$ and $Y$. The base change of $\xi$ is a distinguished element
of $F(A_\epsilon, \xi)$. More on Morphisms, Lemma
\ref{more-morphisms-lemma-action-sheaf} and Remark
\ref{more-morphisms-remark-another-special-case} show that the sheaf of such
lifts is acted on simply transitively by
$$
\SheafHom_{\mathcal{O}_{X_A}}
(\xi^*\Omega_{Y_A/A},
\mathcal{C}_{X_A/X_{A_\epsilon}}).
$$
The canonical identification
$\mathcal{C}_{X_A/X_{A_\epsilon}} = \mathcal{O}_{X_A}\epsilon$ and the
distinguished lift identify its global sections with the displayed group.
\end{proof}

\begin{remark}
\label{remark-formal-hom-tangent-degree}
The tangent group in Proposition
\ref{proposition-formal-hom-prorepresentable-tangent} is in degree zero.
The degree-one formula printed in the reference cannot be correct: for
$X_A = \Spec(A)$ and $Y_A = \mathbf{A}^1_A$, the lift set is naturally $A$,
whereas the corresponding first cohomology group is zero. In a general
lifting problem without the distinguished base-change lift, degree one may
instead contain the obstruction to the existence of a global lift.
\end{remark}

\begin{lemma}
\label{lemma-formal-hom-noetherian-components}
\begin{reference}
\cite[Expos\'e 195, A, Proposition 5.1, pp. 375--376 and
C, Corollaire, p. 382]{FGA}.
\end{reference}
Let $\Lambda$ be a Noetherian ring. Let $X$ and $Y$ be schemes over
$\Lambda$. If $X$ is flat and proper over $\Lambda$ and $Y$ is locally of
finite type over $\Lambda$, then the local components of a topological
$\Lambda$-algebra prorepresenting
$$
A \longmapsto \Mor_A(X_A, Y_A), \qquad A \in \mathcal{A}_\Lambda,
$$
are Noetherian.
\end{lemma}

\begin{proof}
Proposition \ref{proposition-formal-hom-prorepresentable-tangent} gives the
prorepresentability. Fix a field $K \in \mathcal{A}_\Lambda$ and a morphism
$\xi : X_K \to Y_K$ corresponding to a point of a local component. The
scheme $X_K$ is Noetherian and proper over $K$. Since $Y_K$ is locally of
finite type over $K$, the module $\Omega_{Y_K/K}$ is coherent by Morphisms,
Lemma \ref{morphisms-lemma-finite-type-differentials}. Hence
$$
\SheafHom_{\mathcal{O}_{X_K}}
(\xi^*\Omega_{Y_K/K}, \mathcal{O}_{X_K})
$$
is coherent by Cohomology of Schemes, Lemma
\ref{coherent-lemma-tensor-hom-coherent}. Its global sections form a finite
dimensional $K$-vector space by Cohomology of Schemes, Lemma
\ref{coherent-lemma-proper-over-affine-cohomology-finite}.
By the tangent formula in the proposition, the tangent space of the local
component is therefore finite dimensional. The finite-tangent criterion for
a local component of a strict Artinian proobject now gives Noetherianity. In
the language used above, this is the bounded-cotangent-space argument of
Lemma \ref{lemma-limit-artinian}.
\end{proof}
