...
\noindent
The results in this section are the supplement to Expos\'e 236. Unless a
statement says otherwise, $S$ is Noetherian, the schemes under consideration
are proper over $S$, and the relevant fppf Picard functors are assumed to be
represented by schemes. The source observes that stack formulations remove
...
\end{reference}
Let $g:X\to Y$ be a surjective morphism of proper schemes over $S$. Pullback
of invertible modules defines a morphism
$$
g^*:\Picardfunctor_{Y/S}\longrightarrow\Picardfunctor_{X/S}
$$
...
fppf base change on $\Picardfunctor_{X/S}$, a Picard class is represented by
an invertible module on $X$. The nonflat-descent construction in the cited
source parametrizes its possible antecedents on $Y$ by the finite-presentation
space of descent data for the two projections from $X\times_YX$ to $X$.
...
In the situation of Theorem
\ref{theorem-picard-pullback-proper-surjective-finite-type}, let $M$ be a
set of points of $\Picardfunctor_{Y/S}$. Then $M$ is contained in a
quasi-compact open subscheme if and only if $g^*(M)$ is. Moreover,
$$
\Picardfunctor_{Y/S}^{\tau}
=(g^*)^{-1}(\Picardfunctor_{X/S}^{\tau}).
$$
\end{lemma}
...
\end{reference}
Let $X\to S$ be projective and flat with geometrically integral fibres
satisfying Serre's condition $S_2$, and let $\mathcal H$ be relatively ample.
A collection of invertible modules on geometric fibres of $X\to S$ is
bounded if its Hilbert polynomials with respect to $\mathcal H$ range over a
finite set.
\end{lemma}
...
\end{reference}
Let $Y\to S$ be projective, let $\mathcal H$ be relatively ample, and let
$X\subset Y$ be the zero scheme of a section of $\mathcal H$. If every
irreducible component of every fibre of $Y\to S$ has dimension at least $3$,
then restriction
$$
...
\end{reference}
Let $X\to S$ be projective with integral geometric fibres of dimension $n$,
and let $\mathcal H$ be relatively ample. For geometric points
$\overline{s}_i\to S$ and invertible modules $\mathcal M_i$ on
$X_{\overline{s}_i}$, write
$$
\chi(X_{\overline{s}_i},
\mathcal M_i\otimes\mathcal H_{\overline{s}_i}^{\otimes t})
=a_{0,i}t^n+a_{1,i}t^{n-1}+a_{2,i}t^{n-2}+\cdots.
$$
The collection of the $\mathcal M_i$ is bounded if and only if the numbers
...
\end{reference}
Let $X\to S$ be proper. For every nonzero integer $r$, multiplication by $r$
...
\end{reference}
Let $X\to S$ be a proper morphism, where $S$ is Noetherian. For a geometric
point $\overline{s}\to S$, let $P_{\overline{s}}$ be the Picard scheme of
...
\end{reference}
Let $X$ be a proper scheme over an algebraically closed field $k$, and let
...
\end{reference}
Let $X$ be a proper scheme over a field $k$, and let $\mathcal L$ be an
...
$\mathcal F=\mathcal O_Y$, for every closed integral subscheme
...
$$
\end{enumerate}
If $X$ is projective and $\mathcal H=\mathcal O_X(1)$ is ample, these
...
$$
\chi(Y,\mathcal L|_Y)-\chi(Y,\mathcal O_Y)
...
\end{reference}
Let $X\to S$ be projective and flat with integral geometric fibres, where
$S$ is Noetherian, and suppose the fppf Picard functor is represented by a
scheme
$$
P=\Picardfunctor_{X/S}.
$$
...
$\mathcal L$ on $X\times_SP$. For integers $m,n$, the function on geometric
points of this cover given by
$$
\chi(X_s,\mathcal L_s^{\otimes m}\otimes\mathcal H_s^{\otimes n})
-\chi(X_s,\mathcal H_s^{\otimes n})
$$
