...
\end{reference}
Let $k$ be a field. Let $X$ and $Y$ be geometrically connected schemes of
finite type over $k$. Assume that $X$ or $Y$ is proper over $k$. Choose an
...
$$
is an isomorphism. In particular, if $k$ is algebraically closed, then
$$
\pi_1(X \times_k Y, \overline{z}) \longrightarrow
...
\medskip\noindent
For a general field, apply Lemma \ref{lemma-ses-field} to $X$, $Y$, and
$X \times_k Y$. The algebraically closed case and the natural projections
give a commutative diagram with exact rows
$$
\xymatrix{
1 \ar[r] & \pi_1((X \times_k Y)_{\overline{k}}) \ar[r] \ar[d]^{\cong} &
\pi_1(X \times_k Y) \ar[r] \ar[d] &
G_k \ar[r] \ar@{=}[d] & 1 \\
1 \ar[r] & \pi_1(X_{\overline{k}}) \times
\pi_1(Y_{\overline{k}}) \ar[r] &
\pi_1(X) \times_{G_k} \pi_1(Y) \ar[r] & G_k \ar[r] & 1.
}
$$
The middle vertical arrow is an isomorphism by a diagram chase.
\end{proof}
