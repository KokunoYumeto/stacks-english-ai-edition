...
\section{Comparison over the complex numbers}
\label{section-comparison-complex}

\noindent
Let $X$ be a scheme of finite type over $\mathbf{C}$. We write $X^{an}$ for
the associated complex analytic space and $X(\mathbf{C})$ for its underlying
set of complex points with the classical topology. Analytification sends a
finite \'etale morphism of schemes to a finite covering in the analytic
topology. The converse is the content of the following theorem.

\begin{theorem}[Riemann existence]
\label{theorem-riemann-existence}
\begin{reference}
\cite[Expos\'e 190, n\textsuperscript{o} 3, pp. 308--310]{FGA} and
\cite[Expos\'e XII, Th\'eor\`eme 5.1]{SGA1}.
\end{reference}
Let $X$ be a scheme of finite type over $\mathbf{C}$. The analytification
functor is an equivalence between
\begin{enumerate}
\item schemes finite \'etale over $X$, and
\item finite \'etale analytic spaces over $X^{an}$.
\end{enumerate}
Equivalently, finite \'etale covers of $X$ are the same as finite topological
covering spaces of $X(\mathbf{C})$.
\end{theorem}

\begin{proof}
This is the Riemann existence theorem; see
\cite[Expos\'e XII, Th\'eor\`eme 5.1]{SGA1}. In particular, no normality
hypothesis on $X$ is needed. The passage from a reduced scheme to an arbitrary
scheme is also visible algebraically: $X_{red} \to X$ is a universal
homeomorphism, so Proposition \ref{proposition-universal-homeomorphism}
identifies their categories of finite \'etale covers, while
$X_{red}(\mathbf{C}) = X(\mathbf{C})$.
\end{proof}

\begin{lemma}
\label{lemma-fundamental-group-complex}
Let $X$ be a connected scheme of finite type over $\mathbf{C}$, let
$x \in X(\mathbf{C})$, and let $\overline{x}$ be the corresponding geometric
point. There is a canonical isomorphism
$$
\widehat{\pi_1^{top}(X(\mathbf{C}), x)}
\longrightarrow
\pi_1(X, \overline{x}),
$$
where the left hand side is the profinite completion of the topological
fundamental group.
\end{lemma}

\begin{proof}
Apply Theorem \ref{theorem-riemann-existence} to the fibre functors at $x$ and
use the classification of finite covering spaces; equivalently, this is
...
\begin{lemma}
\label{lemma-ses-field-component}
Let $k$ be a field with algebraic closure $\overline{k}$.
Let $X$ be a connected scheme of finite type over $k$ and let
$\overline{x} \in X(\overline{k})$.
Set $G_k = \pi_1(\Spec(k), \Spec(\overline{k}))$.
Let $C$ be the connected component of $X_{\overline{k}}$ containing
$\overline{x}$ and let $H \subset G_k$ be the stabilizer of $C$. Then the
image of
$$
\pi_1(X, \overline{x}) \longrightarrow G_k
$$
is $H$, and there is a short exact sequence
$$
1 \longrightarrow \pi_1(C, \overline{x}) \longrightarrow
\pi_1(X, \overline{x}) \longrightarrow H \longrightarrow 1.
$$
\end{lemma}

\begin{proof}
The connected components of $X_{\overline{k}}$ are open and closed and
there are only finitely many of them. The group $G_k$ acts transitively on
this finite set: a union of orbits would be a $G_k$-stable open and closed
subset and hence would descend to an open and closed subset of $X$ by
Varieties, Lemma \ref{varieties-lemma-closed-fixed-by-Galois}, applied also
to its complement. This would contradict the connectedness of $X$.
In particular, $H$ is open in $G_k$.

\medskip\noindent
Let $k' \subset \overline{k}$ be the finite separable extension with
$H = \text{Gal}(\overline{k}/k')$. Applying the same descent lemma over
$k'$ to $C$ and its complement gives an open and closed subscheme
$X' \subset X_{k'}$ whose base change to $\overline{k}$ is $C$.
View $X' \to X$ as a morphism of $k$-schemes. After base change to
$\overline{k}$ its source is the disjoint union of the conjugates of $C$,
one for every coset of $H$ in $G_k$, and the morphism identifies these
with all the connected components of $X_{\overline{k}}$. It is therefore
an isomorphism after this faithfully flat base change, hence an isomorphism
by Descent, Lemma \ref{descent-lemma-descending-property-isomorphism}.

\medskip\noindent
Thus $X$, as a scheme, has the $k'$-structure $X'$, and its base change
to $\overline{k}$ over $k'$ is the connected scheme $C$. Applying
Lemma \ref{lemma-ses-field} to $X'/k'$ gives the displayed exact sequence
and identifies the image in $G_k$ with
...
\begin{lemma}
\label{lemma-pi1-surjective-pushforward-structure-sheaf}
\begin{reference}
\cite[Expos\'e 182, Lemme 4, p. 212]{FGA}
\end{reference}
Let $f : X \to S$ be a quasi-compact and quasi-separated morphism of
connected schemes. If
$$
\mathcal{O}_S \longrightarrow f_*\mathcal{O}_X
$$
is an isomorphism, then for every geometric point $\overline{x}$ of $X$,
with image $\overline{s}$ in $S$, the map
$$
\pi_1(X, \overline{x}) \longrightarrow \pi_1(S, \overline{s})
$$
is surjective. In particular, this applies when $f$ is proper and the
displayed map is an isomorphism.
\end{lemma}

\begin{proof}
Let $S' \to S$ be a connected finite \'etale morphism and set
$X' = X \times_S S'$. Since $S' \to S$ is flat, flat base change gives
$$
\mathcal{O}_{S'} \longrightarrow f'_*\mathcal{O}_{X'}
$$
as an isomorphism, where $f' : X' \to S'$ is the projection; see
Cohomology of Schemes, Lemma
\ref{coherent-lemma-flat-base-change-cohomology}. Hence
$$
\Gamma(S', \mathcal{O}_{S'}) = \Gamma(X', \mathcal{O}_{X'}).
$$
If $X'$ were disconnected, the right hand side would contain a nontrivial
idempotent, contrary to the connectedness of $S'$. Thus pullback by $f$
sends every connected finite \'etale cover of $S$ to a connected finite
\'etale cover of $X$. The result follows from
Lemma \ref{lemma-functoriality-galois-surjective}.
\end{proof}

\begin{lemma}
\label{lemma-finite-unramified-etale-after-pullback}
Let $f : X \to S$ be a proper morphism such that
$\mathcal{O}_S \to f_*\mathcal{O}_X$ is an isomorphism. Let $p : T \to S$ be
a finite unramified morphism, let $t \in T$, and let $s = p(t)$. If the base
change
$$
X \times_S T \longrightarrow X
$$
is \'etale at every point over $t$, then $p$ is \'etale at $t$.
Consequently, if the base change is \'etale, then $p$ is \'etale.
\end{lemma}

\begin{proof}
First, $f$ is surjective. Indeed, its image is closed, and on the complement
the equality $\mathcal{O}_S = f_*\mathcal{O}_X$ would identify a nonzero
structure sheaf with zero.

\medskip\noindent
The assertion is \'etale local around $s$ and $t$. After an \'etale base change
the equality of structure sheaves is preserved by Cohomology of Schemes,
Lemma \ref{coherent-lemma-flat-base-change-cohomology}. Thus, by \'Etale
Morphisms, Lemma \ref{etale-lemma-finite-unramified-etale-local}, we may
assume that $S$ is affine and
$$
T = \coprod T_i
$$
with every $T_i \to S$ a closed immersion. Let $T_1$ be the factor containing
$t$. The non-\'etale locus of $X_{T_1} \to X$ has closed image in $T_1$ by
properness, and this image does not contain $t$. After shrinking $S$ around
$s$, we may therefore assume that $X_{T_1} \to X$ is \'etale.

\medskip\noindent
The morphism $X_{T_1} \to X$ is both a closed immersion and \'etale, and hence
is an open immersion by \'Etale Morphisms, Theorem
\ref{etale-theorem-etale-radicial-open}. Thus $X_{T_1}$ is open and closed in
$X$. It is defined by an idempotent in
$$
\Gamma(X, \mathcal{O}_X) = \Gamma(S, \mathcal{O}_S).
$$
Let $S_1 \subset S$ be the corresponding open and closed subscheme. Then
$X_{T_1} = X_{S_1}$ as subschemes of $X$.

\medskip\noindent
Let $\mathcal{I}_1$ be the ideal defining $T_1$. Over $S_1$ its pullback to
$X$ is zero; the equality $\mathcal{O}_S = f_*\mathcal{O}_X$ therefore gives
$\mathcal{I}_1|_{S_1} = 0$. Over $S \setminus S_1$ the scheme $X_{T_1}$ is
empty. Surjectivity of $f$ then implies that $T_1$ is empty there. Hence
$T_1 = S_1$. Thus $T_1 \to S$ is an open immersion as well as a closed
immersion, proving that $p$ is \'etale at $t$.
\end{proof}

\begin{lemma}
\label{lemma-local-Stein-trivial-component}
\begin{reference}
\cite[Expos\'e 182, Th\'eor\`eme 11, pp. 213--214]{FGA}
\end{reference}
Let $S$ be a locally Noetherian scheme. Let $f : X \to S$ be a proper
morphism such that $\mathcal{O}_S \to f_*\mathcal{O}_X$ is an isomorphism.
Let $h : X' \to X$ be a finite morphism and let
$$
X' \longrightarrow T \longrightarrow S
$$
be the Stein factorization of $f \circ h$. Let $s \in S$, let $t \in T$ lie
over $s$, and let $C \subset X'_s$ be the open and closed connected component
corresponding to $t$. Assume
\begin{enumerate}
\item $h$ is \'etale at every point of $C$, and
\item the canonical morphism
$$
C \longrightarrow
X_s \times_{\Spec(\kappa(s))} \Spec(\kappa(t))
$$
is an isomorphism.
\end{enumerate}
Then there is an open neighbourhood $V \subset T$ of $t$ such that
$V \to S$ is \'etale and the canonical morphism
$$
X' \longrightarrow X \times_S T
$$
is an isomorphism over $V$.
\end{lemma}

\begin{proof}
Write $g : X' \to T$ for the first morphism in the Stein factorization and
consider
$$
\varphi = (h, g) : X' \longrightarrow X \times_S T.
$$
By More on Morphisms, Theorem
\ref{more-morphisms-theorem-stein-factorization-general}, the scheme $T$ is
the relative spectrum of $(f \circ h)_*\mathcal{O}_{X'}$. This algebra is
coherent by Cohomology of Schemes, Proposition
\ref{coherent-proposition-proper-pushforward-coherent}; hence $T$ is finite
over $S$. It follows that $\varphi$ is finite. Its fibre over $t$ is the
canonical morphism in assumption (2), and therefore is an isomorphism. The
isomorphism locus of $\varphi$ is open in $X \times_S T$. Its complement has
closed image in $T$, because
$X \times_S T \to T$ is proper, and this image does not contain $t$.
Let $W \subset T$ be the complement of this image. Thus $W$ is an open
neighbourhood of $t$ and $\varphi$ is an isomorphism over $W$.

\medskip\noindent
Choose $c \in C$. Via the isomorphism over $W$, the morphism $h$ is the base
change of $T \to S$ by $X \to S$ in a neighbourhood of $c$. Consequently
$$
\Omega_{T/S, t} \otimes_{\mathcal{O}_{T, t}} \kappa(c)
= \Omega_{X'/X, c} \otimes_{\mathcal{O}_{X', c}} \kappa(c) = 0
$$
by assumption (1). Since $\kappa(c)$ is a field extension of $\kappa(t)$,
faithful flatness and Nakayama's lemma give $\Omega_{T/S, t} = 0$.
The morphism $T \to S$ is finite and hence locally of finite type, so
Morphisms, Lemma \ref{morphisms-lemma-unramified-omega-zero} shows that it is
unramified at $t$. The isomorphism over $W$ and assumption (1) also show that
$X \times_S T \to X$ is \'etale at every point over $t$. Lemma
\ref{lemma-finite-unramified-etale-after-pullback} now shows that $T \to S$ is
\'etale at $t$. Intersecting $W$ with the \'etale locus gives the required
open neighbourhood $V$.
\end{proof}

\begin{lemma}
\label{lemma-finite-etale-descends-trivial-all-fibres}
\begin{reference}
\cite[Expos\'e 182, Corollaire 1, p. 214]{FGA}
\end{reference}
Let $S$ be a locally Noetherian scheme. Let $f : X \to S$ be a proper
morphism such that $\mathcal{O}_S \to f_*\mathcal{O}_X$ is an isomorphism.
Let $h : X' \to X$ be a finite \'etale morphism, let
$$
X' \longrightarrow T \longrightarrow S
$$
be the Stein factorization of $f \circ h$, and let
$\varphi : X' \to X \times_S T$ be the canonical morphism. The following are
equivalent:
\begin{enumerate}
\item $T \to S$ is finite \'etale and $\varphi$ is an isomorphism;
\item for every $s \in S$, every $t \in T$ over $s$, and the connected
component $C \subset X'_s$ corresponding to $t$, the canonical morphism
$$
C \longrightarrow
X_s \times_{\Spec(\kappa(s))} \Spec(\kappa(t))
$$
is an isomorphism.
\end{enumerate}
In particular, a finite \'etale cover of $X$ is pulled back from a finite
\'etale cover of $S$ if and only if it is geometrically trivial on every
fibre of $f$; the cover of $S$ is then unique up to unique isomorphism.
\end{lemma}

\begin{proof}
The fibres of $f$ are geometrically connected by the structure-sheaf identity
in the statement and
More on Morphisms, Theorem
\ref{more-morphisms-theorem-stein-factorization-general}. Thus if (1) holds,
then (2) follows by taking fibres. Conversely, assume (2). The
morphism $h$ is \'etale along every $C$, so
Lemma \ref{lemma-local-Stein-trivial-component}, applied at every point of
$T$, shows that $T \to S$ is \'etale and that $\varphi$ is an isomorphism.
The morphism $T \to S$ is finite by the proof of
Lemma \ref{lemma-local-Stein-trivial-component}.

\medskip\noindent
Finally, if $X'$ is pulled back from a finite \'etale cover $T' \to S$, then
flat base change and $f_*\mathcal{O}_X = \mathcal{O}_S$ give
$$
(f \circ h)_*\mathcal{O}_{X'} = (T' \to S)_*\mathcal{O}_{T'}.
$$
Hence $T'$ is the middle term of the Stein factorization, proving uniqueness.
\end{proof}

\begin{lemma}
\label{lemma-function-fields-local-Stein-trivial-component}
\begin{reference}
\cite[Expos\'e 182, Corollaire 2, p. 214]{FGA}
\end{reference}
In the situation of Lemma \ref{lemma-local-Stein-trivial-component}, assume in
addition that $S$, $X$, and $X'$ are integral. Let $K$, $L$, and $L'$ be their
function fields. Then there is a finite separable extension $K'/K$ contained
in $L'$ such that the multiplication map
$$
L \otimes_K K' \longrightarrow L'
$$
is an isomorphism. In particular, $L$ and $K'$ are linearly disjoint over $K$
and $L'$ is their compositum.
\end{lemma}

\begin{proof}
Let $X' \to T \to S$ be the Stein factorization. The scheme $T$ is integral:
it is the relative spectrum of $(f \circ h)_*\mathcal{O}_{X'}$, and the ring
of sections over every nonempty affine open of $T$ is the ring of sections of
a nonempty open of the integral scheme $X'$, hence is a domain. Let $K'$ be
the function field of $T$. The morphism $T \to S$ is finite by the proof of
Lemma \ref{lemma-local-Stein-trivial-component}.

\medskip\noindent
The same lemma gives a nonempty open $V \subset T$ which is \'etale over $S$
and over which
$$
X' \longrightarrow X \times_S T
$$
is an isomorphism. Since $T$ and $S$ are integral, their generic points belong
to $V$ and its image, respectively. It follows that $K'/K$ is finite separable
and that the generic fibre product is integral. Thus $L \otimes_K K'$ is a domain. It is a
finite-dimensional $L$-algebra, so it is a field, and the generic form of the
displayed isomorphism identifies it with $L'$. This proves all the assertions.
\end{proof}
...
\begin{lemma}
\label{lemma-finite-etale-descends-if-trivial-on-fibre}
\begin{reference}
\cite[Expos\'e 182, Corollaire 1, p. 214]{FGA}
\end{reference}
Let $f : X \to S$ be a flat proper morphism of finite presentation whose
geometric fibres are connected and reduced. Assume $S$ is connected and let
$\overline{s}$ be a geometric point of $S$. For a finite \'etale morphism
$Y \to X$, the following are equivalent:
\begin{enumerate}
\item there is a finite \'etale morphism $T \to S$ and an isomorphism
$Y \cong T \times_S X$ over $X$, and
\item the finite \'etale cover
$Y \times_X X_{\overline{s}} \to X_{\overline{s}}$ is isomorphic to a
finite disjoint union of copies of $X_{\overline{s}}$.
\end{enumerate}
When these conditions hold, $T$ and the displayed isomorphism are unique up
to unique isomorphism.
\end{lemma}

\begin{proof}
The finite \'etale cover $Y \to X$ corresponds to a finite set with a
continuous action of $\pi_1(X)$. It is pulled back from $S$ if and only if
this action factors through $\pi_1(X) \to \pi_1(S)$. By
Proposition \ref{proposition-first-homotopy-sequence}, this map is surjective
and its kernel is the image of $\pi_1(X_{\overline{s}})$. Thus the action
factors through $\pi_1(S)$ if and only if its restriction to
$\pi_1(X_{\overline{s}})$ is trivial. By the Galois category description,
the latter condition is exactly (2). Uniqueness follows from the full
faithfulness of $\textit{F\'Et}_S \to \textit{F\'Et}_X$, proved in
Proposition \ref{proposition-first-homotopy-sequence}.
\end{proof}

\begin{lemma}
\label{lemma-fundamental-group-product-one-proper}
\begin{reference}
\cite[Expos\'e 182, Corollaire 5, p. 215, with the 1962 correction]{FGA}
\end{reference}
Let $k$ be a field. Let $X$ and $Y$ be connected schemes of finite type
over $k$. Assume that $X$ or $Y$ is proper over $k$. Choose an
algebraic closure $\overline{k}$ and points
$$
\overline{x} \in X(\overline{k}), \qquad
\overline{y} \in Y(\overline{k}), \qquad
\overline{z} = (\overline{x}, \overline{y}).
$$
Set $G_k = \pi_1(\Spec(k), \Spec(\overline{k}))$. Then the natural map
$$
\pi_1(X \times_k Y, \overline{z}) \longrightarrow
\pi_1(X, \overline{x}) \times_{G_k} \pi_1(Y, \overline{y})
$$
is an isomorphism. Consequently, the natural map
$$
\pi_1(X \times_k Y, \overline{z}) \longrightarrow
\pi_1(X, \overline{x}) \times \pi_1(Y, \overline{y})
$$
is injective. If $k$ is algebraically closed, then it is an isomorphism.
\end{lemma}

\begin{proof}
We first prove the result when $k$ is algebraically closed. By
Lemma \ref{lemma-thickening} we may replace $X$ and $Y$ by their reductions.
The induced closed immersion between the products is a thickening as all
schemes in question are Noetherian, so this replacement does not change the
fundamental group of the product either. The resulting schemes are
geometrically reduced by Varieties, Lemma
\ref{varieties-lemma-perfect-reduced}. After interchanging
$X$ and $Y$ if necessary, we may assume that $X$ is proper. The projection
$$
p : X \times_k Y \longrightarrow Y
$$
is flat, proper, and of finite presentation, and its geometric fibres are
connected and reduced. The assertion is unaffected by changing base points,
so we may choose them to be $k$-rational. Proposition
\ref{proposition-first-homotopy-sequence} therefore gives an exact sequence
$$
\pi_1(X) \longrightarrow \pi_1(X \times_k Y)
\longrightarrow \pi_1(Y) \longrightarrow 1.
$$
The inclusions of the two factors through these points and the two
projections show that the natural
map from the middle group to $\pi_1(X) \times \pi_1(Y)$ is surjective. If an
element maps to $(1, 1)$, exactness shows that it comes from $\pi_1(X)$, and
the first projection then shows that this element of $\pi_1(X)$ is trivial.
Thus the natural map is also injective, proving the algebraically closed
case.

\medskip\noindent
For a general field, let $C_X$ and $C_Y$ be the connected components of
$X_{\overline{k}}$ and $Y_{\overline{k}}$ containing $\overline{x}$ and
$\overline{y}$. Let $H_X, H_Y \subset G_k$ be their stabilizers. The product
$C_X \times_{\overline{k}} C_Y$ is connected: its projection to $C_Y$ is
open and surjective with connected fibres. Thus it is the connected component
of $(X \times_k Y)_{\overline{k}}$ containing $\overline{z}$. Its stabilizer is
$$
H = H_X \cap H_Y.
$$
Let $W$ be the connected component of $X \times_k Y$ containing
$\overline{z}$; its fundamental group is the fundamental group of
$X \times_k Y$ at $\overline{z}$. Set
$$
P = \pi_1(X, \overline{x}) \times_{G_k}
\pi_1(Y, \overline{y}).
$$
Lemma \ref{lemma-ses-field-component}, applied to $X$, $Y$, and $W$, gives
a commutative diagram with exact rows
$$
\xymatrix{
1 \ar[r] & \pi_1(C_X \times_{\overline{k}} C_Y) \ar[r]
\ar[d]^{\cong} &
\pi_1(W, \overline{z}) \ar[r] \ar[d] &
H \ar[r] \ar@{=}[d] & 1 \\
1 \ar[r] & \pi_1(C_X, \overline{x}) \times
\pi_1(C_Y, \overline{y}) \ar[r] &
P \ar[r] & H \ar[r] & 1.
}
$$
The bottom row is exact because
$\pi_1(X, \overline{x}) \to H_X$ and
$\pi_1(Y, \overline{y}) \to H_Y$ are surjective. The left vertical arrow
is an isomorphism by the algebraically closed case; if one of $X$ and $Y$
is proper, then the corresponding geometric component is proper. The middle
vertical arrow is therefore an isomorphism by a diagram chase.
\end{proof}
...
\begin{lemma}
\label{lemma-fundamental-group-smooth-proper-curve-generators}
\begin{reference}
\cite[Expos\'e 182, Corollaire 3, p. 217]{FGA}
\end{reference}
Let $k$ be an algebraically closed field. Let $X$ be a smooth proper
connected curve over $k$ of genus $g$. There exist elements
$$
a_1, b_1, \ldots, a_g, b_g \in \pi_1(X)
$$
which topologically generate $\pi_1(X)$ and satisfy
$$
...
\begin{proof}
Assume first that $k$ has characteristic zero. We can find a subfield
$k_0 \subset k$ finitely generated over $\mathbf{Q}$ and a smooth proper
geometrically connected curve $X_0$ over $k_0$ whose base change to $k$
is $X$. Let $\overline{k}_0 \subset k$ be the algebraic closure of $k_0$
in $k$, and choose an embedding $\overline{k}_0 \to \mathbf{C}$.
Base change gives isomorphisms
$$
\pi_1(X) \cong \pi_1(X_{0, \overline{k}_0})
\cong \pi_1(X_{0, \mathbf{C}})
$$
by Lemma \ref{lemma-finite-etale-invariant-over-proper}. The genus is
unchanged by Algebraic Curves, Lemma
\ref{curves-lemma-genus-base-change}. By the Riemann existence theorem
\cite[Expos\'e XII, Corollaire 5.2]{SGA1}, the last group is the profinite
completion of the usual fundamental group of a compact Riemann surface of
genus $g$. The latter has the presentation
$$
\langle a_1, b_1, \ldots, a_g, b_g \mid
[a_1, b_1] \cdots [a_g, b_g] = 1 \rangle.
$$
This proves the result in characteristic zero.

\medskip\noindent
Assume that $k$ has characteristic $p > 0$. Choose a Cohen ring $A$ with
residue field $k$ by Algebra, Lemma
\ref{algebra-lemma-cohen-rings-exist}. By Deformation Theory, Lemma
\ref{defos-lemma-smooth-proper-curve-lift-complete-local}, there is a
smooth projective morphism $\mathcal{X} \to \Spec(A)$ whose special fibre
is $X$. Its geometric fibres are connected by More on Morphisms, Lemma
\ref{more-morphisms-lemma-proper-flat-geom-red}; indeed, the number of
geometric connected components is locally constant and is one on the
special fibre. If $\overline{K}$ is an algebraic closure of the fraction
field of $A$, then $\mathcal{X}_{\overline{K}}$ has genus $g$ by
Algebraic Curves, Lemma
\ref{curves-lemma-genus-in-nodal-family-of-curves}. The characteristic-zero
case supplies $2g$ generators with the stated relation for
$\pi_1(\mathcal{X}_{\overline{K}})$. Their images have the same relation
and topologically generate $\pi_1(X)$ because the specialization map is
surjective by Lemma \ref{lemma-specialization-map-surjective}.
\end{proof}

\begin{proposition}
\label{proposition-fundamental-group-smooth-projective-finitely-generated}
\begin{reference}
\cite[Expos\'e 182, Corollaire 4, pp. 217--218]{FGA}
\end{reference}
Let $k$ be an algebraically closed field. Let $X$ be a smooth projective
connected scheme over $k$. Then $\pi_1(X)$ is topologically finitely
generated.
\end{proposition}

\begin{proof}
We argue by induction on $d = \dim(X)$. If $d = 0$, then $X = \Spec(k)$,
and if $d = 1$, the result follows from
Lemma \ref{lemma-fundamental-group-smooth-proper-curve-generators}.
Assume $d \geq 2$. Since $X$ is smooth and connected, it is irreducible.
Choose a projective embedding of $X$. There is a hyperplane section
$Y \subset X$ which is smooth and irreducible by More on Morphisms,
Remark \ref{more-morphisms-remark-geometric-proof-bertini-irreducible}.

For $x \in X \setminus Y$, regularity of $X$ and the dimension formula give
$$
\text{depth}(\mathcal{O}_{X, x}) + \dim(\overline{\{x\}}) = d > 1.
$$
Hence the restriction functor
$\textit{F\'Et}_X \to \textit{F\'Et}_Y$ is fully faithful by
Proposition \ref{proposition-lefschetz-fully-faithful}. Consequently the map
$\pi_1(Y) \to \pi_1(X)$ is surjective by
Lemma \ref{lemma-functoriality-galois-surjective}. The induction hypothesis
shows that $\pi_1(Y)$ is topologically finitely generated, and the same is
therefore true of its quotient $\pi_1(X)$.
\end{proof}
...
\begin{definition}
\label{definition-tame-fundamental-group}
Let $X$ be a locally Noetherian scheme and let $U \subset X$ be a connected
dense open subscheme. Assume that the local ring of $X$ at every prime
divisor contained in $X \setminus U$ is a discrete valuation ring. Let
$\overline{u}$ be a geometric point of $U$. Denote by
$$
\textit{F\'Et}^t_{U/X} \subset \textit{F\'Et}_U
$$
the full subcategory of finite \'etale covers which are tamely ramified over
$X$ in codimension $1$. The {\it tame fundamental group of $U$ with respect
to $X$} is
$$
\pi_1^t(U/X, \overline{u}) =
\text{Aut}(Y \mapsto Y_{\overline{u}}),
\qquad Y \in \textit{F\'Et}^t_{U/X}.
$$
\end{definition}

\noindent
The category in the definition is a Galois category. Namely, tame extensions
are preserved on passing to subextensions and admit common finite tame
overextensions by More on Algebra, Lemmas
\ref{more-algebra-lemma-subextension-tame} and
\ref{more-algebra-lemma-permanence-tame}. Thus the subcategory is stable
under finite limits, finite colimits, subobjects, and quotients in
$\textit{F\'Et}_U$. Restriction of the fibre functor gives a canonical
continuous surjection
$$
\pi_1(U, \overline{u}) \longrightarrow
\pi_1^t(U/X, \overline{u}).
$$
...
purity for $\mathcal{O}_{X, x}$ by Lemma \ref{lemma-local-purity}.
\end{proof}
...
\begin{proof}
This is a special case of Lemma \ref{lemma-purity-one-divisor-general}.
...
By Lemma \ref{lemma-pullback-tame-codim1}
...
\begin{theorem}[Tame specialization]
\label{theorem-tame-specialization}
\begin{reference}
\cite[Expos\'e 182, Th\'eor\`eme 14, p. 219]{FGA} and
\cite[Expos\'e XIII, Corollaire 2.9 and Section 2.10]{SGA1}.
\end{reference}
Let $S$ be a locally Noetherian scheme. Let $f : X \to S$ be a smooth
proper morphism with geometrically connected fibres. Let $D \subset X$ be
an effective Cartier divisor which is smooth over $S$, and set
$U = X \setminus D$. Given a specialization
$\overline{s}_1 \leadsto \overline{s}_0$ of geometric points of $S$, there
is a specialization homomorphism, well defined up to inner automorphism,
$$
sp^t :
\pi_1^t(U_{\overline{s}_1}/X_{\overline{s}_1})
\longrightarrow
\pi_1^t(U_{\overline{s}_0}/X_{\overline{s}_0}).
$$
It is surjective. If the characteristic of $\kappa(\overline{s}_0)$ is zero,
then it is an isomorphism. If that characteristic is $p > 0$, then it induces
an isomorphism on maximal prime-to-$p$ quotients.
\end{theorem}

\begin{proof}
The smooth divisor $D$ is a relative normal crossings divisor with one
branch. The cited base change theorem for tame covers says that finite tame
covers on the two geometric fibres are related by a specialization functor.
Its degree-zero assertion makes this functor fully faithful. Hence the
associated homomorphism of Galois groups is surjective by
Lemma \ref{lemma-functoriality-galois-surjective}.

If a finite group $G$ has order invertible at $\overline{s}_0$, the
degree-one assertion of the same base change theorem identifies the
$G$-torsors on the two fibres. Every connected Galois cover with group of
order prime to the positive residue characteristic is such a torsor. Thus
specialization gives an equivalence on the finite Galois objects of
prime-to-$p$ order when the residue characteristic is $p > 0$, which is
exactly the asserted isomorphism on maximal prime-to-$p$ quotients. In residue
characteristic zero this applies to every finite group and proves that
$sp^t$ itself is an isomorphism.
\end{proof}

\begin{proposition}
\label{proposition-tame-fundamental-group-punctured-curve}
\begin{reference}
\cite[Expos\'e 182, Corollaire sans num\'ero, pp. 219--220]{FGA} and
\cite[Expos\'e XIII, Corollaire 2.12]{SGA1}.
\end{reference}
Let $k$ be an algebraically closed field. Let $X$ be a smooth proper
connected curve over $k$ of genus $g$, let $s_1, \ldots, s_n$ be distinct
closed points, and set $U = X \setminus \{s_1, \ldots, s_n\}$. There are
topological generators
$$
x_1, y_1, \ldots, x_g, y_g, \sigma_1, \ldots, \sigma_n
\quad\text{of}\quad \pi_1^t(U/X)
$$
such that $\sigma_j$ generates a tame inertia subgroup at $s_j$ and
$$
\left(\prod_{i = 1}^g [x_i, y_i]\right)
\sigma_1 \cdots \sigma_n = 1.
$$
If $k$ has characteristic $p > 0$, the maximal prime-to-$p$ quotient of
$\pi_1^t(U/X)$ is the pro-prime-to-$p$ completion of the group with these
generators and this single relation. In characteristic zero,
$\pi_1^t(U/X)$ is the profinite completion of that group.

Equivalently, let $G$ be any finite group if $k$ has characteristic zero,
and let $G$ have order prime to $p$ if $k$ has characteristic $p > 0$.
Given generators
$x_i, y_i, \sigma_j$ of $G$ satisfying the displayed relation, there exists
a connected finite \'etale Galois cover of $U$ with group $G$ for which the
inertia group above $s_j$ is generated by $\sigma_j$.
\end{proposition}

\begin{proof}
In characteristic zero every finite cover is tame along the boundary.
After spreading out the pointed curve and using
Theorem \ref{theorem-tame-specialization} in characteristic zero, we may
embed the field of definition into $\mathbf{C}$. The Riemann existence
theorem \cite[Expos\'e XII, Corollaire 5.2]{SGA1} identifies the fundamental
group with the profinite completion of the topological fundamental group of
a genus $g$ surface punctured at $n$ points. This gives the generators, the
relation, and the inertia description.

Suppose $k$ has characteristic $p > 0$. Choose a Cohen ring $A$ with residue
field $k$. By Deformation Theory, Lemma
\ref{defos-lemma-smooth-proper-curve-lift-complete-local}, the curve $X$
lifts to a smooth projective curve $\mathcal{X}$ over $A$. Each $s_j$ lifts
to a section of $\mathcal{X} \to \Spec(A)$ by More on Algebra, Lemma
\ref{more-algebra-lemma-lift-smooth-henselian}; the sections are disjoint
because their special-fibre points are distinct. Their union is a smooth
relative effective Cartier divisor $\mathcal{D}$. The geometric generic
fibre has genus $g$ as in the proof of
Lemma \ref{lemma-fundamental-group-smooth-proper-curve-generators}.

Apply Theorem \ref{theorem-tame-specialization} to
$(\mathcal{X}, \mathcal{D})$. The images of the characteristic-zero
generators topologically generate the tame fundamental group of the special
fibre and satisfy the displayed relation. The local standard tame coverings
of Example \ref{example-tamely-ramified} show that the image of each
$\sigma_j$ generates the corresponding tame inertia group. The same theorem
identifies the maximal prime-to-$p$ quotients, proving that there are no
further relations there. The final formulation follows from the Galois
correspondence.
\end{proof}
