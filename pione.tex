...
\begin{enumerate}
\item $h$ is unramified at every point of $C$, and
...
$V \to S$ is unramified and the canonical morphism
$$
...
After replacing $T$ by the complement of this image, we may assume that
$\varphi$ is an isomorphism.

\medskip\noindent
Choose $c \in C$. Via the isomorphism $\varphi$, the morphism $h$ is the base
change of $T \to S$ by $X \to S$. Consequently
$$
...
unramified at $t$. Shrinking to its unramified locus gives the required open
neighbourhood $V$.
\end{proof}
...
a finite unramified morphism. If the base change
$$
X \times_S T \longrightarrow X
$$
is \'etale, then $p$ is \'etale.
\end{lemma}
...
\medskip\noindent
The assertion is \'etale local on $S$. After an \'etale base change the equality
of structure sheaves is preserved by Cohomology of Schemes, Lemma
\ref{coherent-lemma-flat-base-change-cohomology}. Thus, by \'Etale Morphisms,
Lemma \ref{etale-lemma-finite-unramified-etale-local}, we may assume that $S$
is affine and
$$
T = \coprod T_i
$$
with every $T_i \to S$ a closed immersion. The base change
$X_{T_i} \to X$ is both a closed immersion and \'etale, and hence is an open
immersion by \'Etale Morphisms, Theorem
\ref{etale-theorem-etale-radicial-open}. Thus $X_{T_i}$ is open and closed in
$X$. It is defined by an idempotent in
$$
\Gamma(X, \mathcal{O}_X) = \Gamma(S, \mathcal{O}_S).
$$
Let $S_i \subset S$ be the corresponding open and closed subscheme. Then
$X_{T_i} = X_{S_i}$ as subschemes of $X$.

\medskip\noindent
Let $\mathcal{I}_i$ be the ideal defining $T_i$. Over $S_i$ its pullback to
$X$ is zero; the equality $\mathcal{O}_S = f_*\mathcal{O}_X$ therefore gives
$\mathcal{I}_i|_{S_i} = 0$. Over $S \setminus S_i$ the scheme $X_{T_i}$ is
empty. Surjectivity of $f$ then implies that $T_i$ is empty there. Hence
$T_i = S_i$. Every $T_i \to S$ is consequently an open immersion as well as
a closed immersion, and $p$ is \'etale.
\end{proof}
...
morphism $h$ is unramified along every $C$, so
Lemma \ref{lemma-local-Stein-trivial-component}, applied at every point of
$T$, shows that $T \to S$ is unramified and that $\varphi$ is an isomorphism.
Thus the base change of $T \to S$ by $X \to S$ is the \'etale morphism $h$.
Lemma \ref{lemma-finite-unramified-etale-after-pullback} shows that
$T \to S$ is \'etale. It is finite by the proof of
