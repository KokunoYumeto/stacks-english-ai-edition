...
\begin{reference}
\cite[Chapter III, \S 4, no. 69, Corollary 2 and transition map,
p. 263]{FAC}
\end{reference}
\begin{history}
For $S=K[t_0,\ldots,t_r]$, the cited source uses the cofinal ideals
$J_k=(t_0^k,\ldots,t_r^k)$ rather than the powers of
$I=S_+$. Indeed,
$I^{(r+1)(k-1)+1}\subset J_k\subset I^k$ for $k\geq1$. Its exact sequence
$0\to J_k\to S\to S/J_k\to0$ shifts
$\operatorname{Ext}^q_S(J_k,M)$ to
$\operatorname{Ext}^{q+1}_S(S/J_k,M)$ for $q\geq1$, with the corresponding
five-term exact sequence in degrees zero and one. Thus the source's Ext
colimit is the positive-complement form of the local-cohomology system below,
with the shift kept explicit rather than absorbed into notation.
\end{history}
Let $A$ be a ring and let $I \subset A$ be a finitely generated ideal.
