...
Let $\mathcal{C}$ be a site.
Let $\mathcal{G}$, $\mathcal{F}$ be sheaves of sets.
...
It maps a section $s \in \mathcal{G}(u(U))$ in the summand corresponding to
...
\item Given $r \geq 0$ we say $\mathcal{F}$ is {\it locally generated
...
Let $\mathcal{G}$ be an abelian sheaf on $\mathcal{C}$.
...
to get coverings $\{U_{ij} \to U_i\}$ and factorizations
$$
...
$C_{ij} \circ B_i|_{U_{ij}} \circ A|_{U_{ij}} = 0$.
Then $\{U_{ij} \to U\}$ is a covering and we get the
desired factorizations
using $B_{ij} = C_{ij} \circ B_i|_{U_{ij}}$ and
...
$V \in \Ob(\mathcal{D})$. Of course, then we may replace $U$ by $u(V)$.
...
Given any section $s : \mathcal{O}_2 \to \mathcal{A}$
of $\pi$ and any $\varphi$-derivation $D : \mathcal{O}_2 \to \mathcal{F}$
the map
$$
...
a sheaf of sets on $\mathcal{C}$ and $\alpha : \mathcal{E} \to \mathcal{B}$
...
and $\phi$ is a morphism $\phi : U \to u(V)$ of $\mathcal{C}$
...
Sites, Lemma \ref{sites-lemma-when-shriek}.
Of course a similar formula holds for a direct sum of modules
...
Sites, Lemma \ref{sites-lemma-cocontinuous-morphism-topoi}.
Then $g^{-1}$ on sheaves of pointed sets has a left adjoint $g_!$
