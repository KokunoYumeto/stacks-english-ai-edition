...
\begin{reference}
\cite[Chapter III, \S 6, no. 80, Remarks 2 and 3, p. 276]{FAC}
\end{reference}
\begin{history}
The cited source calls $\chi(X, \mathcal{O}_X)$ the arithmetic genus and,
for a projective classical variety whose irreducible components all have
dimension $d$, records the comparison
$\chi(X, \mathcal{O}_X) = 1 + (-1)^d p_a(X)$ with the classical convention.
It does not assume smoothness or an algebraically closed ground field for
that comparison, whereas the discussion below does. The preceding source
remark also forecasts a relation between the virtual arithmetic genus and an
Euler characteristic in Riemann--Roch theory; no such theorem is asserted
here.
\end{history}
