...
REFINEMENTS = {
    ...
    "SMOOTHING-022": [("$g_j \\in \\mathfrak p^2$", "$g_j \\in \\mathfrak p_B^2$")],
    "SMOOTHING-023": [("over $k$", "over $R/\\pi R$")],
}
...
def verify_payload(config: dict, source_map: list[dict]) -> dict:
    authority = (ROOT / "authority/source/smoothing.tex").read_bytes()
    payload = (ROOT / "payload/smoothing.tex").read_bytes()
    operations: list[tuple[int, int, bytes, bytes, str]] = []
    operation_ids: list[str] = []
    for unit in source_map:
        if (
            unit["source"] != "smoothing.tex"
            or unit["authority"] != "authority/source/smoothing.tex"
            or unit["payload"] != "payload/smoothing.tex"
            or unit["authority_sha256"] != sha_bytes(authority)
        ):
            raise AssertionError("R26 source map contains an unexpected source binding")
    ...
    if replay != payload:
        raise AssertionError("payload contains changes outside the 39 mapped operations")

    stem = config["stems"]["smoothing"]
...
def main() -> int:
    ...
    try:
        ...
        spec_input = ROOT / "R26_SMOOTHING_ADJUDICATION_SPEC.input.json"
        spec_generated = ROOT / "R26_SMOOTHING_ADJUDICATION_SPEC.json"
        ...
        if (
            ...
            or spec["semantic_unit_count"] != 25
            or spec["operation_count"] != 39
            or spec["producer_row_count"] != 35
            or spec["accepted_producer_row_count"] != 31
            or spec["rejected_producer_row_count"] != 1
            or spec["prior_alias_producer_row_count"] != 3
            or (ROOT / "SMOOTHING_SOURCE_DEFECT_LEDGER.input.csv").read_bytes()
            != (ROOT / "authority/canon/SMOOTHING_SOURCE_DEFECT_LEDGER.csv").read_bytes()
            or (ROOT / "SMOOTHING_SOURCE_EMENDATIONS.input.json").read_bytes()
            != (ROOT / "authority/canon/SMOOTHING_SOURCE_EMENDATIONS.json").read_bytes()
        ):
        ...
        if expected_ids != EXPECTED_IDS:
            raise AssertionError("configured stable IDs are not contiguous 1177..1201")
        stable = json.loads((ROOT / "stable-units.json").read_text(encoding="utf-8"))
        source_map = load_jsonl(ROOT / "source-map.jsonl")
        producer_rows = list(csv.DictReader(
            (ROOT / "authority/canon/SMOOTHING_SOURCE_DEFECT_LEDGER.csv")
        ...
        expected_accepted = set(EXPECTED_PRODUCER_IDS) - set(REJECTED_IDS) - set(DUPLICATE_ALIASES)
        if (
            stable["unit_count"] != 25
            ...
            or len(all_producers) != 31
            or len(set(all_producers)) != 31
            or set(all_producers) != expected_accepted
            or [row["id"] for row in producer_rows] != EXPECTED_PRODUCER_IDS
            or any(row.get("source_file") != "smoothing.tex" for row in producer_rows)
            or spec["prior_aliases"] != [
                {"producer_id": producer_id, "result": "exact_duplicate_of_admitted_r1_unit", "stable_id": stable_id}
        ...
        if (
            ...
            or operation_spec["operation_count"] != 39
            or operation_spec["authority_sha256"] != config["stems"]["smoothing"]["authority_sha256"]
        ...
        if payload["operation_count"] != 39:
            raise AssertionError("exact payload operation-count mismatch")
        report["checks"]["exact_payload"] = {"passed": True, "smoothing": payload}
