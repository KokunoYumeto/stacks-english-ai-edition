...
def replay_payload() -> dict:
    ...
    if sha_bytes(authority) != CONFIG["stems"]["algebra"]["authority_sha256"]:
...
def main() -> int:
    ...
    if unit_ids != CONFIG["expected_unit_ids"] or producer_ids != CONFIG["expected_producer_ids"]:
        raise AssertionError("stable-unit mapping mismatch")
    candidate_pdf, authority_pdf = ROOT / "builds" / "algebra.pdf", ROOT / "builds" / "algebra.authority.pdf"
