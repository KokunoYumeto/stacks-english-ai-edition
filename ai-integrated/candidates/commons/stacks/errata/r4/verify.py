...
def main() -> int:
    ...
    try:
        ...
        expected_ids = [f"MC-STK-ERR-{number:04d}" for number in range(63, 217)]
        ...
        if stable["unit_count"] != 154 or ids != expected_ids or map_ids != expected_ids:
            raise AssertionError("stable unit or source-map closure mismatch")
        if len({row["producer_id"] for row in source_map}) != 154:
            raise AssertionError("producer correction identities are not unique")
        report["checks"]["unit_closure"] = {"passed": True, "expected": 154, "manifested": 154}
        ...
        if inventory["unit_count"] != 154 or sorted(classified) != expected_ids or len(classified) != len(set(classified)) or inventory["unmapped_formula_or_diagram_changes"] != 0:
        ...
        if len(rejected) != 16 or len(duplicates) != 2 or not decisions:
            raise AssertionError("decision/rejection closure mismatch")
        report["checks"]["lease_ledgers_inventory"] = {"passed": True, "decisions": len(decisions), "rejections": 16, "duplicate_aliases": 2}
        ...
        report["checks"]["visual_pdf_qa"] = {"passed": True, "pdfs": len(CONFIG["stems"]), "direct_units": 154, "receipt_sha256": sha256(visual_path)}
