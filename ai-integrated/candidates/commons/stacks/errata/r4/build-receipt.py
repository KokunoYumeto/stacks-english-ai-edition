...
def parse_log(path: Path) -> dict:
    ...
    refs = collections.Counter(re.findall(
        r"LaTeX Warning: (?:Hyper reference|Reference) `([^']+)' .*? undefined on input line \d+\.", flat
    ))
    cites = collections.Counter(re.findall(
        r"LaTeX Warning: Citation `([^']+)' .*? undefined on input line \d+\.", flat
    ))
...
def main() -> int:
    ...
    for stem, expected in CONFIG["stems"].items():
        ...
        warnings_match = (
            candidate_summary["undefined_reference_targets"] == authority_summary["undefined_reference_targets"]
        ...
        passed = (
            ...
            and warnings_match and binding
