...
if tuple(CONFIG["stems"]) != (STEM,):
    raise AssertionError("validation requires the single configured stem")
STRUCTURE_PATTERNS = {
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate the complete sealed R31 candidate evidence chain.")
    parser.parse_args()
    ...
    try:
        ...
        for chapter in receipt["chapters"]:
            if (
                ...
                # This reopen changes prose/math notation only; it introduces no
                # reference target delta.  Require the candidate and untouched
                # authority undefined-reference multisets to remain identical.
                or chapter["undefined_reference_target_multisets_identical_to_authority"] is not True
