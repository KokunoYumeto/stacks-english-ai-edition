...
def main() -> int:
    parser = argparse.ArgumentParser(description="Create the sealed R31 build receipt from replay outputs.")
    parser.parse_args()
    ...
    if execution.get("passed") is not True or any(execution.get(key) != value for key, value in expected_execution.items()):
        raise AssertionError("build execution identity or pass state mismatch")
    receipt = {
    ...
    for stem, expected in CONFIG["stems"].items():
        ...
        literal_reference_multisets_identical = (
            collections.Counter(candidate_summary["undefined_reference_targets"])
            == collections.Counter(authority_summary["undefined_reference_targets"])
        )
        references_match_authority_plus_configured_exceptions = (
            collections.Counter(candidate_summary["undefined_reference_targets"])
            == expected_candidate_refs
        )
        citation_multisets_match_authority = (
            candidate_summary["undefined_citation_targets"]
            == authority_summary["undefined_citation_targets"]
        )
        warnings_match = (
            references_match_authority_plus_configured_exceptions
            and citation_multisets_match_authority
        )
        page_delta_matches = (
        ...
        receipt["chapters"].append({
            ...
            "undefined_reference_target_multisets_identical_to_authority": literal_reference_multisets_identical,
            "undefined_reference_target_multiset_matches_authority_plus_configured_exceptions": references_match_authority_plus_configured_exceptions,
            "undefined_citation_target_multisets_match_authority": citation_multisets_match_authority,
