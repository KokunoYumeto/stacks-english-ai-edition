...
def main() -> int:
    ...
    if len(units) != config["accepted"] or len(source_map) != config["accepted"] or len(operations) != config["operation_count"] or spec["operation_count"] != config["operation_count"]:
        raise AssertionError("candidate unit/operation closure mismatch")
    if sha(authority) != config["stems"][stem]["authority_sha256"]:
    ...
    if (ROOT / "rejections.jsonl").read_bytes() != b"":
        raise AssertionError("R31 final adjudication has no rejected units")
    print(json.dumps({"passed": True, "units": len(units), "operations": len(operations), "payload_bytes": len(payload), "payload_sha256": sha(payload), "stable_id_range": [expected_ids[0], expected_ids[-1]], "authority_long_lines_gt_80": authority_long_lines, "payload_long_lines_gt_80": payload_long_lines}, sort_keys=True))
    return 0
