...
def replay_payload() -> dict:
    authority_path = ROOT / "authority" / "source" / "spaces-duality.tex"
    payload_path = ROOT / "payload" / "spaces-duality.tex"
    operation_path = ROOT / "operation-spec.json"
    authority = authority_path.read_bytes()
    if sha_bytes(authority) != CONFIG["stems"]["spaces-duality"]["authority_sha256"]:
    ...
    if (
        ...
        or spec["authority_sha256"] != CONFIG["stems"]["spaces-duality"]["authority_sha256"]
    ):
...
def main() -> int:
    ...
    if (
        ...
        or CONFIG["proof_closure"]["producer_rows"] != len(accepted_producer_ids) + len(rejected_producer_ids)
    ...
    candidate_pdf, authority_pdf = ROOT / "builds" / "spaces-duality.pdf", ROOT / "builds" / "spaces-duality.authority.pdf"
    ...
    render_pdf = render_manifest.get("pdfs", {}).get("spaces-duality", {})
