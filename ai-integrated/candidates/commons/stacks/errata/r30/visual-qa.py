...
def mapped_pages(stem: str, page_count: int) -> list[int]:
    """Return the hash-bound correction-sensitive page inventory derived by SyncTeX."""
    visual = CONFIG.get("visual_qa", {})
    page_map_path = ROOT / visual.get("source_page_map_path", "")
    if not page_map_path.is_file():
        raise AssertionError("sealed source-page map is missing")
    ...
    if (
        ...
        or page_map.get("unique_page_count") != len(pages or [])
        ...
        raise AssertionError("invalid SyncTeX-derived correction-sensitive page inventory")
    return pages
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Record R30 structural and manual visual QA after the render inspection is complete."
    )
    parser.add_argument("--private-render-root", type=Path, required=True)
    parser.add_argument(
        "--manual-review-complete",
        action="store_true",
        help="Attest that every contact sheet and configured correction-sensitive page was inspected.",
    )
    args = parser.parse_args()
    if not args.manual_review_complete:
        parser.error("--manual-review-complete is required after, and only after, the main-agent render inspection")
    render_root = args.private_render_root.resolve()
    render_manifest_path = render_root / "render-manifest.json"
    render_manifest = json.loads(render_manifest_path.read_text(encoding="utf-8"))
    if render_manifest.get("schema") != "mathematics-commons-stacks-private-render-manifest/v1":
    ...
    report = {
        ...
        "render_protocol": {
            "full_page_render": f"Poppler pdftoppm PNG at {CONFIG['visual_qa']['full_page_dpi']} dpi for every candidate page",
            "full_page_contact_sheet_review": "All rendered pages manually inspected in ordered 4-by-4 contact sheets",
            "high_resolution_review": f"The SyncTeX-derived correction-sensitive page inventory was rendered at {CONFIG['visual_qa']['high_resolution_dpi']} dpi and manually inspected",
            "manual_review_attested": True,
        ...
        "adverse_evidence": [
            ...
            "The standalone candidate retains the frozen authority's cross-chapter unresolved targets and adds exactly one configured candidate-only target, sites-modules-section-internal-hom, introduced by accepted unit INJECTIVES-CH19-D005; the build receipt proves there is no other reference or citation delta.",
        ],
    }
    ...
    for stem in CONFIG["stems"]:
        ...
        if render_pdf.get("pdf_sha256") != sha256(pdf) or render_pdf.get("dpi") != CONFIG["visual_qa"]["full_page_dpi"]:
            raise AssertionError(f"render manifest PDF binding mismatch: {stem}")
        render_rows = render_pdf.get("renders", [])
        ...
        if len(contact_sheets) != math.ceil(state["pages"] / CONFIG["visual_qa"]["contact_sheet_page_capacity"]):
        ...
        if [row["file"] for row in render_manifest.get("contact_sheets", [])] != [row["file"] for row in contact_evidence]:
            raise AssertionError(f"contact-sheet manifest mismatch: {stem}")
        for expected, observed in zip(render_manifest["contact_sheets"], contact_evidence):
            if expected["bytes"] != observed["bytes"] or expected["sha256"] != observed["sha256"]:
                raise AssertionError(f"stale contact sheet: {observed['file']}")

        high_res_pages = mapped_pages(stem, state["pages"])
        review_pages = high_res_pages
        if not set(review_pages).issubset(high_res_pages):
            raise AssertionError(f"correction-sensitive pages lack high-resolution renders: {stem}")
        high_res_manifest = render_manifest.get("high_resolution", {})
        high_res_rows = high_res_manifest.get("renders", [])
        if high_res_manifest.get("dpi") != CONFIG["visual_qa"]["high_resolution_dpi"] or [row.get("page") for row in high_res_rows] != high_res_pages:
            raise AssertionError(f"high-resolution render manifest mismatch: {stem}")
        high_res_evidence = []
        for row in high_res_rows:
            page_number = row["page"]
            path = render_root / "highres" / f"{stem}_p{page_number}.png"
            observed = {"page": page_number, "file": path.name, "bytes": path.stat().st_size, "sha256": sha256(path)}
            if row["file"] != observed["file"] or row["bytes"] != observed["bytes"] or row["sha256"] != observed["sha256"]:
                raise AssertionError(f"stale high-resolution page: {stem} {page_number}")
            high_res_evidence.append(observed)
        ...
        report["pdfs"].append({
            ...
            **state,
            ...
            "manual_findings": {
                ...
                "high_resolution_pages_inspected": review_pages,
                "clipping": 0,
                "overlap": 0,
            ...
            "reflow_note": (
                f"Candidate and authority page counts are recorded above; every candidate page was inspected, including all pages directly affected by the {CONFIG['operation_count']} mapped {stem}.tex operations."
            ),
        })
