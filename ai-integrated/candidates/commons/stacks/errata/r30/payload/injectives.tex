...
If, in (\ref{equation-compare}), $\mathcal{C}$ is the category
...
Let $\kappa$ be the cardinality of the set of submodules of $M$.
...
\begin{proof}
For an abelian group $A$ we denote by $j : A \to J(A)$ the functorial
...
\begin{proof}
For any ring $R$ and any $R$-module $M$ we denote by
...
Suppose that the $\mathcal{G}_i$, $i\in I$, form a set of abelian sheaves
...
\noindent
Recall that for an object $X$ of $\mathcal{C}$ we denote by $\mathbf{Z}_X$
...
See Modules on Sites, Section \ref{sites-modules-section-internal-hom}.
We will also need
...
\begin{proof}
This is because $\mathcal{J}$ is an injective abelian sheaf.
\end{proof}
...
an injective abelian sheaf, the underlying map extends to a map
...
The composition of $ev$ with the displayed
map gives
...
any morphism $A \to A'$ exists and the induced morphism $i' : A' \to B'$
is an admissible monomorphism, and
\item the base change of an admissible epimorphism $p : B \to C$ via
any morphism $C' \to C$ exists and the induced morphism $p' : B' \to C'$
...
we see that together they define a morphism $A' + \Im(\psi) \to I$
...
\begin{proof}
Choose a generator $U$ of $\mathcal{A}$. Denote by $c$ the function of
...
\end{enumerate}
Then $I^\bullet$ is a $K$-injective complex.
\end{lemma}
...
Theorem \ref{theorem-injective-embedding-grothendieck}.
For every complex $M^\bullet$ denote by $J^\bullet(M^\bullet)$ the complex
...
Denote by $\mathbf{M}^\bullet(-)$ the functor constructed in
Lemma \ref{lemma-functorial-homotopies}.
Denote by $\mathbf{N}^\bullet(-)$ the functor constructed in
...
$\mathbf{T}_{\alpha'}(M^\bullet)$ for some $\alpha' < \alpha$.
In particular, by the construction of the functor
...
$F : \mathcal{A}^{opp} \to \textit{Ab}$ which is additive and right exact, i.e.,
...
use that $\alpha^{pp'} \circ \alpha^{p + 1p} = \alpha^{p + 1p'}$
...
Let $K^\bullet$ be a filtered complex of $\mathcal{A}$, see
Homology, Definition \ref{homology-definition-filtered-complex}.
For ease of notation denote by $K$, $F^pK$, $\text{gr}^pK$ the
...
For ease of notation denote by $M$, $M/F^pM$, $\text{gr}^pM$ the
...
and we set $\varphi^i = \varphi^{i - 1} \circ \delta$.
...
$\text{Mod}_R$. Choose a suitably large cardinal $\kappa$ and denote by $E$
...
is isomorphic to an element $(A_i, f_i)$ of $I$ and we conclude that $f$
