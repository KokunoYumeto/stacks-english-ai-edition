...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the pre-review or final R30 candidate manifest.")
    parser.parse_args()
    replay_path = ROOT / "replay" / "independent-review.json"
    replay = None
    if replay_path.exists():
        candidate = json.loads(replay_path.read_text(encoding="utf-8"))
        if candidate.get("schema") == "mathematics-commons-stacks-errata-independent-review/v1":
            replay = candidate
    ...
    if validation.get("passed") is not True or validation.get("candidate_id") != CONFIG["candidate_id"]:
        raise AssertionError("current R30 validation receipt is absent, stale, or failed")
    if (
        ...
        raise AssertionError("current R30 visual inspection receipt is absent, stale, or failed")
    ...
    if replay:
        ...
        if (
            replay.get("candidate_id") != CONFIG["candidate_id"]
        ...
        replay_note = (
            ...
            f"{replay['pre_review_manifest_sha256']} and validation {replay['validation_sha256']}. "
            "This final registrar-only rebind adds the independent receipt; no authority source, "
            "payload, operation specification, PDF, or rendered-page byte changed."
        )
    ...
    page_rows = [
        (row["candidate_log_summary"]["pages"], row["authority_log_summary"]["pages"])
    ...
    manifest = {
        ...
        **{key: evidence(path) for key, path in singled.items()},
        ...
        "unresolved_defects": [
            replay_note,
            "The isolated injectives.tex candidate retains the frozen authority's cross-chapter undefined-reference targets and adds exactly one configured candidate-only target, sites-modules-section-internal-hom, introduced by accepted unit INJECTIVES-CH19-D005; the build gate proves there is no other reference or citation delta.",
            ...
            "The bounded intake assigns 40 collision-free aliases to 40 accepted semantic units and 40 exact operations. All 28 producer defect rows were independently admitted, and 12 additional source defects were established directly from the same frozen authority; no unit remains rejected or unresolved.",
