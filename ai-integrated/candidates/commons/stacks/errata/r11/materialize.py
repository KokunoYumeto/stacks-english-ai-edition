...
PRODUCER_SHA256 = "EB78964BE8D50BD25BBBEF72A6F7B0960D925A039CFF04A255334D59F6952A02"
PRODUCER_BYTES = 364_895
INTAKE_SHA256 = "C7F758F25DDA65107D7DCDAB2FE7BEE0320D132FFBDC4B83CBE6D064F1F40ECB"
INTAKE_BYTES = 3_960
...
SPEC_SHA256 = "92EF3C26C055F21D7858B27866AF64CFE3E777E671A8DC543EC6660D599CB20A"
GENERATED_AT = "2026-08-24T20:26:50Z"
...
def main() -> int:
    ...
    spec_path = ROOT / "authority" / "canon" / "R11_ALG_382_385_ADJUDICATION_SPEC.json"
    ...
    expected_producers = [f"ALGEBRA-{number}" for number in range(382, 386)]
    ...
    if sorted(accepted_producers + rejected_producers) != sorted(expected_producers):
        raise AssertionError("adjudication partition is not ALGEBRA-382..385")
    expected_ids = [f"MC-STK-ERR-{number:04d}" for number in range(725, 729)]
    if [row["stable_id"] for row in accepted] != expected_ids:
        raise AssertionError("stable-ID assignment is not contiguous 0725..0728")
    ...
    generated = [
        ...
        ROOT / "authority" / "canon" / "R11_ALG_382_385_INTAKE.json",
        ROOT / "authority" / "canon" / "R11_ALG_382_385_REVIEW.md",
    ...
    write_json(ROOT / "authority" / "canon" / "R11_ALG_382_385_INTAKE.json", intake)
    ...
    decisions = [
        ("ERR-R11-D0001", "Bind R11 exclusively to the active errata/r11 lease and frozen upstream commit/tree.", "Preserves the single-writer and immutable-authority boundary."),
        ("ERR-R11-D0002", "Assign MC-STK-ERR-0725..0728 in producer order after admitted R10.", "Creates a deterministic contiguous stable-ID sequence for all four accepted rows."),
        ("ERR-R11-D0003", "Apply only exact nonoverlapping half-open UTF-8 spans in descending byte order.", "Prevents contextual or global replacement from changing unrelated source text."),
        ("ERR-R11-D0004", "Accept all four producer rows after independent source replay and deduplication against R1-R10.", "Every accepted row is a distinct bounded defect supported by the frozen source and queue checkpoint."),
        ("ERR-R11-D0005", "Keep canonical locale bytes and the literal upstream mirror outside the corrected English payload.", "Corrections belong only to Stacks — English AI Edition."),
        ("ERR-R11-D0006", "Do not contact or submit this AI-derived correction set to upstream Stacks maintainers.", "The maintained AI-open English edition is the authorized correction surface."),
    ]
    ...
    (ROOT / "rejections.jsonl").write_text(
        "".join(json.dumps({"schema": "mathematics-commons-stacks-candidate-rejection/v1",
            "id": f"ERR-R11-R{index:04d}", "timestamp_utc": GENERATED_AT,
    ...
    review_lines = [
        "# Independent review: ALGEBRA-382–385", "",
        f"Frozen authority: `{AUTHORITY_SHA256}` at `{spec['authority_commit']}`.", "",
        "The four producer rows were replayed against the frozen source and are accepted",
        "as `MC-STK-ERR-0725` through `MC-STK-ERR-0728`. No unit is rejected, aliased, or",
        ...
        "all five operations, exact source-line bounds, deduplication against R1–R10, and the",
    ...
    (ROOT / "authority" / "canon" / "R11_ALG_382_385_REVIEW.md").write_text(
        "\n".join(review_lines) + "\n", encoding="utf-8", newline="")
    ...
    config = {
        ...
        "candidate_id": spec["candidate_id"], "lease_id": "stacks-lease-000014-errata-r11",
        ...
        "expected_producer_ids": expected_producers, "namespace": "commons/stacks/errata/r11",
        "private_render_logical_path": "canon/private_evidence/errata-r11-20260824T2026Z/render-final/render-manifest.json",
