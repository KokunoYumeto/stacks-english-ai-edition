...
def main() -> int:
    ...
    review = {
        ...
        "stable_id_recomputation": {"passed": True, "derivation_rule": "Accepted producer rows map in packet order to consecutive stable IDs; rejected rows consume no stable ID.", "explicit_mapping": [{"producer_id": producer_id, "stable_id": unit_id} for producer_id, unit_id in zip(producer_ids, unit_ids)], "excluded_producer_ids": sorted(excluded_producer_ids), "unit_count": len(unit_ids), "operation_count": CONFIG["operation_count"], "first_id": unit_ids[0], "last_id": unit_ids[-1]},
