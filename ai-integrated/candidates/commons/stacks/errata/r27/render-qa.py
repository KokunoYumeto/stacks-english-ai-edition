...
if len(CONFIG["stems"]) != 1:
    raise AssertionError("R27 rendering requires exactly one configured stem")
...
def configured_high_res_pages() -> list[int]:
    ...
    pages = visual.get("high_resolution_pages", {}).get(STEM) if isinstance(visual, dict) else None
    if (
        ...
        raise AssertionError(f"candidate.config.json lacks a valid visual_qa.high_resolution_pages.{STEM}")
    return pages


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Render the sealed R27 candidate PDF and its configured high-resolution pages."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        nargs="*",
        help="Must exactly match candidate.config.json; omit to use the sealed configuration.",
    )
