...
PRODUCER_SHA256 = "B826AE5D4EA998A2B720ECA74E5478D0180BBB5A3D9F412D9391ED8012A70049"
PRODUCER_BYTES = 357_209
INTAKE_SHA256 = "AB58D695AD80A70C415C452FFD0A84026D39C49DAB863A8CB8C9DF61395EC6B0"
INTAKE_BYTES = 8_768
...
SPEC_SHA256 = "5518088C7AAA35623B27FC9F6A3D78087E36C0E43CA5A0C99212C8CE1A7C06D2"
GENERATED_AT = "2026-08-24T13:48:03Z"
...
def main() -> int:
    ...
    spec_path = ROOT / "authority" / "canon" / "R9_ALG_342_371_ADJUDICATION_SPEC.json"
    ...
    expected_producers = [f"ALGEBRA-{number}" for number in range(342, 372)]
    ...
    if sorted(accepted_producers + rejected_producers) != sorted(expected_producers):
        raise AssertionError("adjudication partition is not ALGEBRA-342..371")
    expected_ids = [f"MC-STK-ERR-{number:04d}" for number in range(686, 715)]
    if [row["stable_id"] for row in accepted] != expected_ids:
        raise AssertionError("stable-ID assignment is not contiguous 0686..0714")
    ...
    generated = [
        ...
        ROOT / "authority" / "canon" / "R9_ALG_342_371_INTAKE.json",
        ROOT / "authority" / "canon" / "R9_ALG_342_371_REVIEW.md",
    ...
    write_json(ROOT / "authority" / "canon" / "R9_ALG_342_371_INTAKE.json", intake)
    ...
    decisions = [
        ("ERR-R9-D0001", "Bind R9 exclusively to the active errata/r9 lease and frozen upstream commit/tree.", "Preserves the single-writer and immutable-authority boundary."),
        ("ERR-R9-D0002", "Assign MC-STK-ERR-0686..0714 in accepted producer order after admitted R8.", "Creates a deterministic contiguous stable-ID sequence without assigning an ID to the rejected row."),
        ("ERR-R9-D0003", "Apply only exact nonoverlapping half-open UTF-8 spans in descending byte order.", "Prevents contextual or global replacement from changing unrelated source text."),
        ("ERR-R9-D0004", "Reject ALGEBRA-344 as optional counterfactual-register polishing rather than a source defect.", "Contemporary informal English permits the printed wording; changing it is not needed for correctness or readability."),
        ("ERR-R9-D0005", "Keep canonical locale bytes and the literal upstream mirror outside the corrected English payload.", "Corrections belong only to Stacks — English AI Edition."),
        ("ERR-R9-D0006", "Do not contact or submit this AI-derived correction set to upstream Stacks maintainers.", "The maintained AI-open English edition is the authorized correction surface."),
    ]
    ...
    (ROOT / "rejections.jsonl").write_text(
        "".join(json.dumps({"schema": "mathematics-commons-stacks-candidate-rejection/v1",
            "id": f"ERR-R9-R{index:04d}", "timestamp_utc": GENERATED_AT,
    ...
    review_lines = [
        "# Independent review: ALGEBRA-342–371", "",
        f"Frozen authority: `{AUTHORITY_SHA256}` at `{spec['authority_commit']}`.", "",
        "The thirty producer rows were replayed against the frozen source. Twenty-nine are accepted",
        "as `MC-STK-ERR-0686` through `MC-STK-ERR-0714`; `ALGEBRA-344` is rejected",
        "as optional register polishing. No unit remains unresolved. Exact operations and the",
        "adjudication rationale for every accepted unit are bound by the public specification and",
        "source map. Three independent read-only reviews separately checked the dependency clusters",
        "ALGEBRA-342–354, ALGEBRA-355–362, and ALGEBRA-363–371 before materialization.", "",
    ...
    (ROOT / "authority" / "canon" / "R9_ALG_342_371_REVIEW.md").write_text(
        "\n".join(review_lines) + "\n", encoding="utf-8", newline="")
    ...
    config = {
        ...
        "candidate_id": spec["candidate_id"], "lease_id": "stacks-lease-000012-errata-r9",
