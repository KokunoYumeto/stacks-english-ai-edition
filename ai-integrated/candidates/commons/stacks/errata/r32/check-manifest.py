...
def validate_manifest_shape(manifest: dict[str, object]) -> None:
    ...
    if manifest["source_closure"] != expected_closure:
        raise AssertionError("source closure differs from sealed R32 configuration")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check the R32 manifest shape, identity, hashes, and file closure.")
    parser.parse_args()
