...
\medskip\noindent
Some of the notation, conventions, and terminology in this chapter are awkward
...
This approximation uses that the local rings of $S$ are G-rings and
...
algebraic space $X'$ separated and of finite type over $S$,
...
a $\Lambda$-algebra homomorphism. Thus we obtain a functor
\begin{equation}
...
$A \to A' \to k$ is equal to the given map $A \to k$ we see (by uniqueness
...
fibred in groupoids over $(\Sch/S)_{fppf}$. Let $k$ be a field
...
\begin{proof}
Let $y'_1, y'_2$ be objects of the left hand side. The sheaf
$\mathit{Isom}(y'_1, y'_2)$ on the category of schemes over $Y'$
...
$$
\mathcal{X}_{Y'} \to \mathcal{X}_Y \times_{\mathcal{X}_X} \mathcal{X}_{X'},
\quad
\mathcal{Y}_{Y'} \to \mathcal{Y}_Y \times_{\mathcal{Y}_X} \mathcal{Y}_{X'},
\quad
\mathcal{Z}_{Y'} \to
\mathcal{Z}_Y \times_{\mathcal{Z}_X} \mathcal{Z}_{X'}
$$
...
$$
Then $(x_Y, x_{X'}, \alpha)$ is isomorphic to the image of an object
$x_{Y'}$ in $\mathcal{X}_{Y'}$ and $(z_Y, z_{X'}, \beta)$ is isomorphic
...
Let $x_0$ be an object of $\mathcal{X}$ lying over $\Spec(k)$.
Denote by $x_{l, 0}$ the restriction of $x_0$ to $\Spec(l)$.
...
$\Spec(k) \to \Spec(B \times_l k)$. Choosing a pullback functor for $\mathcal{X}$
...
$U' \to R$. Then $U'$ is smooth over $U$, hence locally of
...
Denote by $x_0, y_0, z_0$ the objects of $\mathcal{X}, \mathcal{Y}, \mathcal{Z}$
...
$\Spec(R/\mathfrak m_R^n) \to \Spec(R/\mathfrak m_R^{n + 1})$
such that $R/\mathfrak m_R$ is a field of finite type over $S$.
...
$\xi_n|_{\Spec(R/\mathfrak m^{n - 1})} \cong \xi_{n - 1}$
and $F(\xi_n) \cong \eta_n$.
\end{proof}
...
\medskip\noindent
We first prove full faithfulness. Let $R$ be a Noetherian complete
...
morphism $u' : \Spec(R') \to U$. Denote by $x'$ the object of $\mathcal{X}$
...
product of Noetherian complete local rings to which our current
discussion applies. Denote by $p_0, p_1 : \Spec(R' \otimes_R R') \to \Spec(R')$
the two projections. By the full faithfulness shown above there exists
...
We omit the proof that $\xi_n$ is the restriction of $x$ to
$\Spec(R/\mathfrak m^n)$.
\end{proof}
...
and denote by $\mathfrak m'$ the kernel.
...
\end{enumerate}
Set $A = B/(b'_1, \ldots, b'_r)$ and denote by $\mathfrak m_A$ the
...
$\Lambda[x_1, \ldots, x_s]$ algebra $A_0$. Then
$A = \colim (A_0)_f$ where
$f \in \Lambda[x_1, \ldots, x_s] \setminus \mathfrak m'$, see
...
$\colim (\mathcal{X} \times_\mathcal{Y} \mathcal{Z})_{T_i} \to
...
$x_i|_T \cong x$ and $z_i|_T \cong z$. Then $\alpha$ corresponds to
...
\medskip\noindent
We omit the arguments showing that $\colim (\mathcal{X} \times_\mathcal{Y} \mathcal{Z})_{T_i} \to
...
Let $V$ be an affine scheme locally of finite presentation over $S$
...
Let $x$ be an object of $\mathcal{X}$ which lies over $U$. Denote by $x_0$
...
With notation as in Definition \ref{definition-versal}, let $R = \mathcal{O}_{U, u_0}^\wedge$.
...
$u_{l, 0} : \Spec(l) \to U$ be a morphism with image $u_0$, where
$k = \kappa(u_0)$ and $l/k$ is finite. Set $x_{l, 0} = x_0|_{\Spec(l)}$.
...
Let $l = \kappa(z_0)$. Denote by $z_{l, 0}$, $v_{l, 0}$,
...
\right.
$$
Let $\mathcal{X} = \mathcal{S}_F$ be the category fibred in sets associated
...
is a G-ring (see also discussion in
...
\begin{proof}
This is a reformulation of Lemma \ref{lemma-formally-smooth-on-deformation-categories}.
\end{proof}
...
$\mathcal{Z} = \mathcal{X} \times_{f, \mathcal{Y}, y} (\Sch/V)_{fppf}$.
...
type fields over $S$ is $\leq$ the size of some
...
functor. Denote by $\mathcal{X} = \mathcal{S}_F$ the category fibred in sets
...
and functors $\mathcal{C}_\Lambda \to \textit{Sets}$,
...
a finite type field over $S$. Denote by $x_0$ the composition
...
Morphisms, Lemma \ref{morphisms-lemma-enough-finite-type-points}.
Let $v_0 \in V$ be a finite type point. Then $k = \kappa(v_0)$ is
a finite type field over $S$. Set $x_0 = y|_{\Spec(k)}$,
...
$$
\item there does {\bf not} exist a pair $(W, \beta)$
...
The compatibility condition on the morphism $\beta$
...
$$
The morphism $i : U \to Z$ comes from the isomorphism $x \to x'|_U$.
...
$$
The morphism $i : U \to Z$ comes from the isomorphism $x \to \xi'|_U$.
Let $z_0 = i(u_0) \in Z$. By Lemma \ref{lemma-base-change-versal}
we see that $Z \to U'$ is smooth at $z_0$. After replacing $U$ by an
affine open neighbourhood of $u_0$, replacing $U'$ by the corresponding
open, and replacing $Z$ by the intersection of the inverse images
...
of a category fibred in groupoids from a strong formal effectiveness
axiom.
Let $S$ be a locally Noetherian scheme. Let $\mathcal{X}$ be a category fibred
in groupoids over $(\Sch/S)_{fppf}$. Assume
\begin{enumerate}
...
\medskip\noindent
If $M$ is an $A$-module we denote by $A[M]$ the $A$-algebra whose underlying
...
and let $M \to N$ be an $A$-linear map. Then there are canonical maps
...
$$
of deformation situations: we ask that the corresponding maps between
...
Denote by $x, y, z$ the objects of $\mathcal{X}, \mathcal{Y}, \mathcal{Z}$
obtained from $w$. For any $A$-module $M$ there is a $6$-term exact sequence
$$
\xymatrix{
0 \ar[r] &
\text{Inf}_w(M) \ar[r] &
...
$B[\Omega_{B/A}]$. On the other hand, we can pull back $\alpha$
...
map $M \to N$ there is an induced $A$-linear map $\mathcal{O}_x(M) \to \mathcal{O}_y(N)$
...
\noindent
This last condition explains the terminology. The module $\mathcal{O}_x(I)$
...
We will develop this theory later.
...
restriction of $\tilde \xi$ to $\Spec(R_n)$. There exist
...
$o_x(A') \in \mathcal{O}_x(I)$ be the image of $x$ under the boundary map
$$
...
Consider a deformation situation $(x, A' \to A)$. Denote by $I$ the kernel of
...
$\xi : E \to \NL_{A/\Lambda}$ be a morphism of $D^{-}(A)$ such that
...
\medskip\noindent
Assume that $x$ is versal at $u_0$ and that $u_0$ is a closed point of $U$.
...
an object $x'$ over $A'$.
Let $\mathfrak p' \subset A'$ be the kernel of the surjective map to $k$.
...
\medskip\noindent
Assume that $x$ is versal at $u_0$. By Lemma \ref{lemma-compute-ext-into-field}
the map $H^0(\xi \otimes_A^{\mathbf{L}} k)$ is dual to the map
$\Ext^0_A(\NL_{A/\Lambda}, k) \to \Ext^0_A(E, k)$. Note that
$$
...
$\Ext^0_A(\NL_{A/\Lambda}, k) \to \Ext^0_A(E, k)$
...
$H^0(E \otimes_A^\mathbf{L} k) \to
H^0(\NL_{A/\Lambda} \otimes_A^\mathbf{L} k)$
is injective. Set $R = A_\mathfrak p^\wedge$ and let $\eta$ be the
...
and a map $\xi_x : E_x \to \NL_{A/\Lambda}$,
\item
\label{item-inf}
...
$$
\Hom_A(E_x, \NL_{A/A'}) = \Ext^0_A(E_x, \NL_{A/A'}) =
\Ext^1_A(E_x, I)
$$
...
\begin{proof}
Choose an $\alpha : \Lambda[x_1, \ldots, x_n] \to A$ with kernel $J$.
...
of $(f, \text{d}f)$ in the pushout. For later use, denote by $x_C$ the
...
and denote by $\xi : E = E^\bullet[-1] \to \NL_{A/\Lambda}$ the corresponding
...
category $\Sch_\alpha/S$. Denote by
$$
...
Denote by $a : F \to G$ the restriction of $a' : F' \to G'$ to
...
Take $\xi : V \to G$ as in (2). Set $V' = V$, viewed as an
...
$X_W \to F'$ provides us with a map $X \to F'$.
...
Algebra, Lemma \ref{algebra-lemma-ring-colimit-fp}.
Since $V'_i$ is of finite type over a Noetherian scheme,
we see that $V'_i$ is a Noetherian scheme.
Set $V_i = V'_i$, viewed as an object of
$(\textit{Noetherian}/S)_\tau$. As $G'$
is limit preserving, we can choose an $i$ and a map
...
Set $X'_i = X_i$, viewed as an object of
$(\Sch/S)_\tau$. Then we see that $F' \times_{G'} V'_i$
and the functor of points $h_{X'_i}$ are both extensions
...
on $(\textit{Noetherian}/S)_\etale$, i.e., satisfies
...
\medskip\noindent
Axiom [4]. This is the axiom that every formal object is effective.
...
$(\textit{Noetherian}/S)_{fppf}$ have the same underlying category.
...
condition (4). It will not be mathematically precise as we are going to use
...
of a closed. Given $Y$ the morphism $Y_{red} \to Y$ should induce a
...
morphism $\varphi : V_2 \to V_1$ of locally Noetherian schemes over $S$
to the map
$$
...
can take $V' = V \times_{x, X} X'$, we can take $\hat x'$ to be
...
which can serve as the last ingredient namely $a$.

\medskip\noindent
Denote by $h_X(-) = \Mor_S(-, X)$ the functor of
...
\item $\hat x : V_{/Z} \to W$ is the composition of
$\tilde x_{/Z} : V_{/Z} \to X'_{/T'}$ with $g : X'_{/T'} \to W$.
\end{enumerate}
...
In Situation \ref{situation-contractions}, let $V$ be a locally Noetherian scheme over $S$.
...
form a triple as in Situation \ref{situation-contractions}
...
\ref{spaces-cohomology-lemma-flat-base-change-cohomology}). Set
...
\medskip\noindent
Let $V = \lim_{\lambda \in \Lambda} V_\lambda$ be a directed limit of schemes
...
subset of $V$, we may after increasing $\lambda$ assume
...
denote $Z$ the inverse image in $V$. We will use below that
...
$\lambda$, an \'etale morphism $\tilde V_\lambda \to V_\lambda$,
and some element $\tilde \xi_\lambda \in F(\tilde V_\lambda)$ such that
$\tilde V = \tilde V_\lambda \times_{V_\lambda} V$
and $\xi|_{\tilde V} = \tilde \xi_\lambda|_{\tilde V}$. We omit a detailed proof of
...
consistently works with finite affine \'etale coverings of all spaces
...
the morphism $\tilde X' \to X'$ is \'etale. Let
$\tilde T' \subset |\tilde X'|$ denote the inverse image of $T'$.
Let $\tilde U' \subset \tilde X'$ denote the complementary open subspace.
Let $\tilde g : \tilde X'_{/\tilde T'} \to \tilde W$
denote the formal modification which is the base change of $g$ by
...
Let $\tilde F$ denote the functor constructed from this triple.
...
Let $\tilde Z \subset \tilde V$ denote the inverse image of $Z$.
...
sets $\tilde V' = V' \times_V \tilde V$ which comes with morphisms
$\tilde{\hat x}'$ and $\tilde x'$ and shows this works.
...
Let $I_\mu \subset A_\mu$ and $I \subset A$
denote the ideals cutting out $Z_\mu$ and $Z$.
...
Hence the kernel and cokernel of $A \to C$ are supported
...
Let $Z_n \subset V$ and $Z_{\mu, n} \subset V_\mu$ denote
...
$$
where $C^\wedge$ and $C_\mu^\wedge$ are the completions with
...
$$
The fact that $V' \to V$, $\hat x'$, $x'$ witness the compatibility
...
$$
The image of these elements in $\Coker(A \to C)$ are zero
...
$(g \circ x'_{\mu, /Z_\mu})^\sharp(b_j)$ in $(C_\mu)^\wedge$.
...
Thus let $V/S$ be the spectrum of an Artinian local ring.
...
x' : V' \to X'
$$
over $S$ recovering the morphisms displayed above. Finally,
...
\ref{formal-spaces-lemma-structure-locally-noetherian}.
Set $V_n = V_{/Z} \times_{g, W} W_n$.
Then $V_n$ is a closed subscheme with underlying set $Z$.
Smoothness of $g : V_{/Z} \to W$ is equivalent to the smoothness
...
Let $\mathfrak m_1 \subset A_1$ denote the maximal ideal corresponding
...
$$
which is a finite type graded algebra over the Noetherian ring $A_1$.
...
Algebra, Lemma \ref{algebra-lemma-what-does-it-mean-again}.
Hence $B_n \to A_n$ is smooth (as a flat ring map with smooth
...
Since the set of points where a morphism is smooth is open,
