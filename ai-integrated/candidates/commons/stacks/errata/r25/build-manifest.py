...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the pre-review or final R25 candidate manifest.")
    parser.parse_args()
    ...
    if validation.get("passed") is not True or validation.get("candidate_id") != CONFIG["candidate_id"]:
        raise AssertionError("current R25 validation receipt is absent, stale, or failed")
    if (
        ...
        raise AssertionError("current R25 visual inspection receipt is absent, stale, or failed")
    ...
    manifest = {
        ...
        **{key: evidence(path) for key, path in singled.items()},
        ...
        "unresolved_defects": [
            replay_note,
            "The isolated artin.tex candidate retains cross-chapter undefined-reference targets; their multiset matches the frozen authority build exactly.",
            ...
            f"The bounded intake from {first_producer} through {last_producer} assigns {closure['accepted']} semantic units to {len(CONFIG['expected_all_producer_ids'])} accepted producer identities; {closure['rejected']} producer identities are rejected, {closure['intentionally_absent']} are intentionally absent, {closure['prior_aliases']} prior producer allegation aliases are retained, and none is unresolved.",
