...
def main() -> int:
    ...
    manifest = {
        ...
        **{key: evidence(path) for key, path in singled.items()},
        ...
        "unresolved_defects": [
            replay_note,
            "The isolated simplicial.tex candidate retains cross-chapter undefined-reference targets; their multiset matches the frozen authority build exactly.",
            ...
            f"The bounded intake closes {first_producer} through {last_producer}: {closure['accepted']} rows are accepted, {closure['rejected']} is rejected as not a defect, {closure['prior_overlay_aliases']} are aliases of prior admitted overlays, and none is unresolved.",
