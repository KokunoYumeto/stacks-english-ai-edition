...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the pre-review or final R29 candidate manifest.")
    parser.parse_args()
    ...
    if validation.get("passed") is not True or validation.get("candidate_id") != CONFIG["candidate_id"]:
        raise AssertionError("current R29 validation receipt is absent, stale, or failed")
    if (
        ...
        raise AssertionError("current R29 visual inspection receipt is absent, stale, or failed")
    ...
    manifest = {
        ...
        **{key: evidence(path) for key, path in singled.items()},
        ...
        "unresolved_defects": [
            replay_note,
            "The isolated sites-modules.tex candidate retains cross-chapter undefined-reference targets; their multiset matches the frozen authority build exactly.",
            ...
            "The bounded intake assigns 30 collision-free producer aliases to 30 semantic units and 31 exact operations. Raw producer IDs are preserved with lease-qualified aliases; three producer units are explicitly rejected and eleven target-layout-only operations remain excluded.",
