...
\begin{lemma}
\label{lemma-projective-space-rigid-complete-local}
\begin{reference}
\cite[Expos\'e 182, Corollaire 2, p. 206]{FGA}
\end{reference}
Let $(A, \mathfrak m, k)$ be a complete Noetherian local ring.
Let $X \to \Spec(A)$ be a proper flat morphism. If for some $n \geq 0$
$$
X \times_{\Spec(A)} \Spec(k) \cong \mathbf{P}^n_k,
$$
then $X \cong \mathbf{P}^n_A$ as schemes over $A$.
\end{lemma}

\begin{proof}
If $n = 0$, then $X \to \Spec(A)$ is finite by More on Morphisms,
Lemma
\ref{more-morphisms-lemma-proper-finite-fibre-finite-in-neighbourhood}.
Indeed, the open neighbourhood in that lemma contains the closed point of
the spectrum of a local ring and hence is the whole spectrum. Write
$X = \Spec(B)$. The $A$-module $B$ is finite flat and
$B/\mathfrak mB \cong k$. Thus $B$ is free of rank $1$ over $A$ by
Algebra, Lemma \ref{algebra-lemma-finite-flat-local}, and the unit map
$A \to B$, which is an isomorphism modulo $\mathfrak m$, is an
isomorphism. This proves the result in this case.

\medskip\noindent
Assume $n \geq 1$. For $q \geq 1$, set
$$
A_q = A/\mathfrak m^q, \qquad
S_q = \Spec(A_q), \qquad
X_q = X \times_{\Spec(A)} S_q, \qquad
P_q = \mathbf{P}^n_{A_q}.
$$
Choose an isomorphism $a_1 : X_1 \to P_1$. We claim that it can be
extended inductively to a compatible system of isomorphisms
$a_q : X_q \to P_q$. Suppose that $a_q$ has been constructed. The schemes
$X_{q + 1}$ and $P_{q + 1}$, together with the identifications induced by
$a_q$, are two flat deformations of $P_q$ over the first order thickening
$$
\Spec(A_q) \subset \Spec(A_{q + 1}).
$$
By Lemma \ref{lemma-deform}, the set of isomorphism classes of such
deformations is principal homogeneous under
$$
\Ext^1_{\mathcal{O}_{P_q}}
\left(
\NL_{P_q/A_q},
\mathcal{O}_{P_q} \otimes_{A_q}
\mathfrak m^q/\mathfrak m^{q + 1}
\right).
$$
The morphism $P_q \to \Spec(A_q)$ is smooth. Hence More on Morphisms,
Lemma \ref{more-morphisms-lemma-NL-smooth} identifies the first argument
with $\Omega_{P_q/A_q}$. Since this sheaf is finite locally free, the
displayed group is
$$
H^1\left(P_q,
\SheafHom_{\mathcal{O}_{P_q}}
\left(\Omega_{P_q/A_q},
\mathcal{O}_{P_q} \otimes_{A_q}
\mathfrak m^q/\mathfrak m^{q + 1}\right)\right).
$$
Put $V_q = \mathfrak m^q/\mathfrak m^{q + 1}$ and let
$i : P_1 \to P_q$ be the closed immersion. The vector space $V_q$ is finite
dimensional over $k$. Base change for differentials and the fact that
$\Omega_{P_q/A_q}$ is finite locally free show that the sheaf in the last
display is
$$
i_*\left(
\SheafHom_{\mathcal{O}_{P_1}}
(\Omega_{P_1/k}, \mathcal{O}_{P_1}) \otimes_k V_q
\right).
$$
Thus the displayed group is
$$
H^1\left(P_1,
\SheafHom_{\mathcal{O}_{P_1}}
(\Omega_{P_1/k}, \mathcal{O}_{P_1})\right) \otimes_k V_q,
$$
which vanishes by de Rham Cohomology, Lemma
\ref{derham-lemma-H1-tangent-projective-space}. Hence this principal
homogeneous space has at most one element. The two deformations are
isomorphic compatibly with $a_q$, proving the claim.

The compatible morphisms $a_q$ algebraize uniquely to a morphism
$a : X \to \mathbf{P}^n_A$ by Cohomology of Schemes, Lemma
\ref{coherent-lemma-algebraize-morphism}. Applying the same lemma to the
compatible inverses $a_q^{-1}$ gives a morphism
$b : \mathbf{P}^n_A \to X$. The uniqueness assertion of that lemma applied
to the two compositions shows that $a \circ b$ and $b \circ a$ are the
respective identity morphisms. Hence $a$ is an isomorphism.
\end{proof}

\begin{lemma}
\label{lemma-projective-lift-complete-local}
\begin{reference}
\cite[Expos\'e 182, Corollaire 3, p. 206]{FGA}
\end{reference}
Let $(A, \mathfrak m, k)$ be a complete Noetherian local ring. Let $Y$ be
a smooth projective scheme over $k$. Set
$$
\mathcal{T}_{Y/k} =
\SheafHom_{\mathcal{O}_Y}(\Omega_{Y/k}, \mathcal{O}_Y).
$$
If
$$
H^2(Y, \mathcal{O}_Y) = H^2(Y, \mathcal{T}_{Y/k}) = 0,
$$
then there exists a smooth projective morphism $X \to \Spec(A)$ and an
isomorphism $X \times_{\Spec(A)} \Spec(k) \cong Y$.
\end{lemma}

\begin{proof}
Choose an ample invertible sheaf $\mathcal{L}_1$ on $Y$. For $q \geq 1$,
set
$$
A_q = A/\mathfrak m^q, \qquad
S_q = \Spec(A_q), \qquad
V_q = \mathfrak m^q/\mathfrak m^{q + 1}.
$$
We will construct a compatible system of smooth proper morphisms
$X_q \to S_q$ and invertible sheaves $\mathcal{L}_q$ on $X_q$, starting
with $X_1 = Y$, such that $\mathcal{L}_{q + 1}|_{X_q} = \mathcal{L}_q$.

Suppose $X_q$ and $\mathcal{L}_q$ have been constructed. Apply Cotangent
Complex, Lemma \ref{cotangent-lemma-find-obstruction-ringed-spaces} to the
first order thickening $S_q \subset S_{q + 1}$ and the coefficient module
$\mathcal{O}_{X_q} \otimes_{A_q} V_q$, using the canonical map from the
ideal of $S_q$ in $S_{q + 1}$. The obstruction to lifting $X_q$ belongs to
$$
\Ext^2_{\mathcal{O}_{X_q}}
\left(L_{X_q/S_q}, \mathcal{O}_{X_q} \otimes_{A_q} V_q\right).
$$
The morphism $X_q \to S_q$ is smooth, so Cotangent Complex, Lemma
\ref{cotangent-lemma-when-projective} identifies $L_{X_q/S_q}$ with
$\Omega_{X_q/S_q}[0]$. The finite dimensional $k$-vector space $V_q$ is
annihilated by $\mathfrak m$, and the coefficient module is the pushforward
from $Y = X_1$ of $\mathcal{O}_Y \otimes_k V_q$. Hence the obstruction
group is
$$
H^2(Y, \mathcal{T}_{Y/k}) \otimes_k V_q = 0.
$$
We obtain a first order thickening $X_q \subset X_{q + 1}$ over
$S_q \subset S_{q + 1}$. It is a scheme by More on Morphisms, Lemma
\ref{more-morphisms-lemma-first-order-thickening}, and it is flat over
$S_{q + 1}$ by More on Morphisms, Lemma
\ref{more-morphisms-lemma-deform}. It is locally of finite presentation and
proper by More on Morphisms, Lemma
\ref{more-morphisms-lemma-thicken-property-morphisms-cartesian}. Since its
only fibre is $Y$, it is smooth by Morphisms, Lemma
\ref{morphisms-lemma-smooth-flat-smooth-fibres}.

Let $\mathcal{I}_q$ be the ideal sheaf of $X_q$ in $X_{q + 1}$. It is the
pushforward from $Y$ of $\mathcal{O}_Y \otimes_k V_q$. The exact sequence
of More on Morphisms, Lemma
\ref{more-morphisms-lemma-picard-group-first-order-thickening} shows that
the obstruction to lifting $\mathcal{L}_q$ belongs to
$$
H^2(X_q, \mathcal{I}_q) =
H^2(Y, \mathcal{O}_Y) \otimes_k V_q = 0.
$$
Thus $\mathcal{L}_q$ lifts to an invertible sheaf
$\mathcal{L}_{q + 1}$ on $X_{q + 1}$. It is ample by More on Morphisms,
Lemma \ref{more-morphisms-lemma-thicken-property-relatively-ample}.
This completes the induction.

Apply Grothendieck's algebraization theorem, Cohomology of Schemes,
Theorem \ref{coherent-theorem-algebraization}, to the systems $(X_q)$ and
$(\mathcal{L}_q)$. We obtain a proper morphism $X \to \Spec(A)$, an ample
invertible sheaf on $X$, and compatible identifications of the reductions
with the $X_q$. The morphism is projective by Morphisms, Lemma
\ref{morphisms-lemma-finite-type-over-affine-ample-very-ample}.

We finish by checking flatness and smoothness. Let $x$ be a point of the
closed fibre and choose an affine open neighbourhood $\Spec(B) \subset X$
of $x$. The ring $B$ is Noetherian and $B/\mathfrak m^qB$ is flat over
$A_q$ for every $q$. Algebra, Lemma
\ref{algebra-lemma-flat-module-powers} shows that $X \to \Spec(A)$ is flat
at $x$. Thus the flat locus is an open neighbourhood of the closed fibre by
More on Morphisms, Theorem
\ref{more-morphisms-theorem-openness-flatness}. Its closed complement has
closed image in $\Spec(A)$ because $X$ is proper. A nonempty closed subset
of the spectrum of a local ring contains the closed point, so this
complement is empty. Hence $X$ is flat over $A$.

The smooth locus is likewise an open neighbourhood of the closed fibre by
Morphisms, Lemma \ref{morphisms-lemma-set-points-where-fibres-smooth}.
Applying the same proper-image argument to its complement shows that
$X \to \Spec(A)$ is smooth.
\end{proof}

\begin{lemma}
\label{lemma-smooth-proper-curve-lift-complete-local}
\begin{reference}
\cite[Expos\'e 182, Corollaire 4, p. 206]{FGA}
\end{reference}
Let $(A, \mathfrak m, k)$ be a complete Noetherian local ring. If $Y$ is
a smooth proper scheme over $k$ of dimension at most $1$, then there exists
a smooth projective morphism $X \to \Spec(A)$ whose special fibre is
isomorphic to $Y$.
\end{lemma}

\begin{proof}
The scheme $Y$ is projective by Varieties, Lemma
\ref{varieties-lemma-dim-1-proper-projective}. Moreover,
$$
H^2(Y, \mathcal{O}_Y) = H^2(Y, \mathcal{T}_{Y/k}) = 0
$$
by Cohomology, Proposition
\ref{cohomology-proposition-vanishing-Noetherian}. Thus the result follows
from Lemma \ref{lemma-projective-lift-complete-local}.
\end{proof}
