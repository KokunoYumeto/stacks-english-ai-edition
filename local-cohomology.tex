...
\begin{reference}
\cite[Chapter III, \S 4, no. 69, definition of $B^q(M)$, the map
$\alpha$, and Remark, pp. 262--263]{FAC}
\cite[Chapter III, \S 4, no. 70, definition of $T^q(M)$ and the map
$\alpha^*$, pp. 263--264]{FAC}
\end{reference}
\begin{history}
In the cited case $A=K[t_0,\ldots,t_r]$, $I=A_+$, and
$U=\mathbb A_K^{r+1}\setminus\{0\}$. For a finite graded module $M$, the
source identifies its graded module $B^q(M)$ with
$H^q(U,\widetilde M|_U)$. The map $M\to B^0(M)$ is the restriction map in
the exact sequence below. For $q\geq1$, its cofinal Ext calculation using
the ideals $(t_0^k,\ldots,t_r^k)$ gives the same one-degree shift to local
cohomology. The grading records the homogeneous components of this affine-
complement calculation and is additional structure, not a different
cohomology theory.

No. 70 applies the exact degreewise $K$-dual to these graded modules. In
the polynomial-cone setting this identifies $T^q(M)$ with the graded dual
of $H^q(U,\widetilde M|_U)$, and $\alpha^*:T^0(M)\to M^*$ with the dual of
the restriction map in the five-term sequence below. Dualizing reverses
the associated long exact sequence; it does not change the one-degree
shift between positive open-complement cohomology and local cohomology.
\end{history}
