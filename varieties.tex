...
\end{reference}
\begin{history}
Serre's usage in \cite{FAC} requires finite affine-type charts and a closed
diagonal but does not require irreducibility; the source explicitly contrasts
this convention with Weil's irreducible varieties. Thus only the irreducible
objects in that convention correspond to varieties in the sense used here,
where integrality is part of the definition.
\end{history}
...
\begin{reference}
\cite[Chapter II, \S 1, no. 36, p. 230]{FAC}
\end{reference}
...
The projective definition is
\cite[Chapter III, \S 1, no. 51, p. 244]{FAC}.
\end{reference}
\begin{history}
The cited source calls a classical algebraic variety affine when it is
isomorphic to a closed subvariety of affine space. Under the passage to
reduced separated schemes of finite type over the ground field, this is the
affine-scheme condition in (1). The source's variety convention permits
reducible objects, whereas Definition \ref{definition-variety} requires
integrality; moreover, the scheme-theoretic closed-subscheme formulation can
retain nonreduced structure before the variety hypothesis is imposed.
The later cited passage defines a projective classical variety as one
isomorphic to a closed subvariety of projective space. Part (2) below uses
the intrinsic projective-morphism condition and records the equivalent
closed-subscheme formulation; this also makes the scheme structure and the
base field explicit.
\end{history}
...
\begin{reference}
\cite[Chapter III, \S 6, no. 79, definition of the
Euler--Poincar\'e characteristic, pp. 274--275]{FAC}
\end{reference}
\begin{history}
The cited source defines $h^q(V, \mathfrak F)$ and
$\chi(V, \mathfrak F)$ for a coherent algebraic sheaf on a projective
variety, using finiteness and vanishing of coherent cohomology. The
definition below retains the same alternating dimension sum for a coherent
module on any proper scheme over a field. It also makes explicit that the
ground field is part of the data.
\end{history}
...
\begin{reference}
\cite[Chapter III, \S 6, no. 79, Proposition 2, p. 275]{FAC}
\end{reference}
\begin{history}
The cited proposition gives the corresponding alternating formula for an
arbitrary finite exact sequence of coherent algebraic sheaves on a projective
variety. It splits that sequence at coherent kernels and applies the
three-term formula repeatedly. Thus the source result follows immediately
from the lemma below and the fact that coherent modules form an abelian
category. The Stacks statement uses the more general proper-scheme setting.
\end{history}
Let $k$ be a field. Let $X$ be a proper scheme over $k$.
...
\begin{lemma}
\label{lemma-euler-characteristic-additive-k-linear}
\begin{reference}
\cite[Chapter III, \S 6, no. 79, Lemma 1 and Proposition 1,
pp. 274--275]{FAC}
\end{reference}
\begin{history}
The cited lemma proves that the alternating sum of the dimensions in a finite
exact sequence of finite dimensional vector spaces is zero. The cited
proposition then applies this to coherent algebraic sheaves, but explicitly
requires the arrows to be only linear over the ground field, not morphisms of
modules. The statement below retains this distinction; it is strictly more
general in its arrows than Lemma
...
$$
be a short exact sequence of sheaves of $k$-vector spaces on $X$.
Assume that each $\mathcal{F}_i$ is a coherent $\mathcal{O}_X$-module and
that its $k$-vector space structure is the one induced by this module
structure; the arrows need not be $\mathcal{O}_X$-linear. Then
$$
\chi(X, \mathcal{F}_2) =
\chi(X, \mathcal{F}_1) + \chi(X, \mathcal{F}_3).
$$
\end{lemma}

\begin{proof}
By Cohomology on Sites, Lemma
\ref{sites-cohomology-lemma-cohomology-modules-abelian-agree}, the cohomology
of each $\mathcal{F}_i$ as a module agrees with the cohomology of its
underlying sheaf of abelian groups. Thus the displayed sequence gives a long
exact sequence of $k$-vector spaces. All of these vector spaces are finite
dimensional and only finitely many are nonzero by the discussion following
Definition \ref{definition-euler-characteristic}. The alternating sum of
their dimensions is zero, which gives the formula.
\end{proof}
...
Since the dimension of $Z$ is at most $0$, we see $Z$ is affine (Properties,
Lemma
...
\begin{reference}
\cite[Chapter III, \S 6, no. 81, proof of Proposition 6, p. 277]{FAC}
\end{reference}
\begin{history}
The cited proof chooses a homogeneous linear form outside every prime in an
irredundant primary decomposition except the irrelevant ideal. On each
standard open this makes multiplication a nonzerodivisor on the module, and
it gives the same short exact hyperplane sequence as below. The source also
identifies the quotient support with the hyperplane intersection and uses the
resulting dimension drop. The proof below replaces primary decomposition by
the finite set of associated primes on the standard affine cover.
\end{history}
...
Under this equivalence the cokernel corresponds to $i^*\mathcal{F}$ by
right exactness of pullback. Thus it is $i_*\mathcal{G}$ for the
$\mathcal{G} = i^*\mathcal{F}$ occurring in the statement.
\end{proof}
...
\end{reference}
\begin{history}
The cited source twists the hyperplane sequence by every integer and applies
Euler-characteristic additivity to obtain the first-difference formula for
the Hilbert polynomial. The notation there suppresses the closed-immersion
pushforward; the sequence below keeps $i_*\mathcal{G}(d)$ explicitly on
$\mathbf{P}^n_k$ while identifying its cohomology with that on $H$.
\end{history}
...
\begin{history}
The basis used in the proof must be chosen in
$H^0(\mathbf{P}^n_k, \mathcal{F}(m))$. An earlier version omitted the
twist from this group, although the multiplication map and the conclusion
both use sections of $\mathcal{F}(m)$.
\end{history}
Let $k$ be a field. Let $n \geq 0$.
Let $\mathcal{F}$ be a coherent sheaf on $\mathbf{P}^n_k$.
...
Choose a basis
$f_1, \ldots, f_r \in H^0(\mathbf{P}^n_k, \mathcal{F}(m))$.
...
\begin{reference}
\cite[Chapter III, \S 6, no. 80, Proposition 3 and proof, p. 275]{FAC}
\cite[Chapter III, \S 6, no. 81, Proposition 6 and Remark 2,
pp. 276--277]{FAC}
\end{reference}
\begin{history}
The cited proposition proves the projective-space assertion by resolving a
finite graded representative by finite graded free modules, sheafifying the
resolution, applying additivity of Euler characteristics, and using
$\chi(\mathcal{O}(d)) = \binom{d + n}{n}$ as an identity of numerical
polynomials for every integer $d$. The proof below instead uses hyperplane
induction. Both proofs give the degree bound $n$. The later Lemma
\ref{lemma-numerical-polynomial-from-euler} gives the stronger bound by the
dimension of the support on an arbitrary proper scheme.
The second citation gives an independent hyperplane proof, identifies the
first difference with the Hilbert polynomial of the hyperplane restriction,
and proves the exact support-dimension statement recorded separately below.
\end{history}
...
of degree at most $0$. Assume $n > 0$. By
...
by induction the right hand side is a polynomial of degree at most $n - 1$.
The lemma is finished by noting that any function
$f : \mathbf{Z} \to \mathbf{Z}$ with the property that the map
$d \mapsto f(d) - f(d - 1)$ is a polynomial of degree at most $n - 1$,
is itself a polynomial of degree at most $n$.
...
\begin{reference}
\cite[Chapter III, \S 6, no. 80, Proposition 4 and the definition of the
characteristic function, pp. 275--276]{FAC}
\end{reference}
\begin{history}
The cited source begins with a graded module $M$ satisfying its finite-tail
condition and calls the eventual polynomial $d \mapsto \dim_k M_d$ the
characteristic function of $M$. Recovery of high graded pieces and vanishing
of higher cohomology identify it, for all large $d$, with the Euler
characteristic of the associated sheaf. Equality of the two polynomials then
extends this identity to every integer $d$; it does not assert equality with
the actual graded dimensions in low degrees. The definition below takes the
intrinsic sheaf polynomial as primary and uses the modern term Hilbert
polynomial.
\end{history}
Let $k$ be a field. Let $n \geq 0$. Let $\mathcal{F}$ be a coherent sheaf
on $\mathbf{P}^n_k$. The function
...
\begin{reference}
\cite[Chapter III, \S 6, no. 80, Proposition 4, pp. 275--276]{FAC}
\end{reference}
\begin{history}
For an associated sheaf coming from a graded module satisfying the source's
finite-tail condition, the cited proposition combines eventual recovery of
$M_d$ with higher-cohomology vanishing. The lemma below states the intrinsic
sheaf part: the Hilbert polynomial agrees with the dimension of global
sections only in sufficiently high degree.
\end{history}
Let $k$ be a field. Let $n \geq 0$. Let $\mathcal{F}$ be a coherent sheaf
...
\begin{lemma}
\label{lemma-hilbert-polynomial-degree-support}
\begin{reference}
\cite[Chapter III, \S 6, no. 81, Proposition 6 and proof,
pp. 276--277]{FAC}
\end{reference}
\begin{history}
The cited source describes the support by an irredundant homogeneous primary
decomposition and proves the degree equality by choosing a hyperplane outside
the relevant primes. The resulting regular multiplication sequence identifies
the first difference with the Hilbert polynomial of the hyperplane
restriction, whose support has dimension one less. The source works over its
infinite ground field and leaves the zero polynomial convention implicit. The
statement below separates the zero sheaf and uses the modern leading-term and
ample-positivity results, so it applies over an arbitrary field.
\end{history}
Let $k$ be a field. Let $n \geq 0$. Let $\mathcal{F}$ be a coherent sheaf
on $\mathbf{P}^n_k$ with Hilbert polynomial $P$. If $\mathcal{F} = 0$, then
$P = 0$. If $\mathcal{F} \not = 0$, then
$$
...
and set $e = \dim(\operatorname{Supp}(\mathcal{F}))$. If $e = 0$, then
Lemma \ref{lemma-chi-tensor-finite}, applied to the rank one modules
$\mathcal{O}(d)$, shows that $P$ is constant with positive value
$\dim_k H^0(\mathbf{P}^n_k, \mathcal{F})$. Thus $P$ has degree $0$.

Assume $e > 0$. Let $Z_i$ be the irreducible components of
$\operatorname{Supp}(\mathcal{F})$ of dimension $e$, let $\xi_i$ be their
generic points, and set
$$
m_i = \operatorname{length}_{\mathcal{O}_{\mathbf{P}^n_k, \xi_i}}
(\mathcal{F}_{\xi_i}).
$$
These integers are positive. By
Lemma \ref{lemma-numerical-polynomial-leading-term}, the difference
$$
P(d) - \sum m_i\chi(Z_i, \mathcal{O}_{Z_i}(d))
$$
has degree less than $e$. The restriction of $\mathcal{O}(1)$ to every
$Z_i$ is ample by Properties, Lemma
\ref{properties-lemma-ample-on-closed}. The discussion following Definition
\ref{definition-degree} and Lemma \ref{lemma-ample-positive} show that the
coefficient of $d^e$ in each polynomial
$\chi(Z_i, \mathcal{O}_{Z_i}(d))$ is positive. Therefore the coefficient of
$d^e$ in $P$ is positive, and $P$ has degree exactly $e$.
\end{proof}

\begin{remark}
\label{remark-FAC-arithmetic-genus}
\begin{reference}
\cite[Chapter III, \S 6, no. 80, Proposition 5 and Remarks 1--3,
p. 276]{FAC}
\end{reference}
\begin{history}
The cited source applies the preceding polynomial identity to the homogeneous
coordinate ring of a classical closed projective subvariety, which may be
reducible, and calls the constant value at zero the arithmetic genus. The
scheme-theoretic statement below also allows nonreduced closed subschemes.
Only the value at zero is independent of the embedding; the remaining
Hilbert polynomial depends on the chosen polarization. The source explicitly
distinguishes its convention from the classical $p_a$ convention.
\end{history}
Let $i : V \to \mathbf{P}^n_k$ be a closed immersion and let $P$ be the
Hilbert polynomial of $i_*\mathcal{O}_V$. Then
$$
P(0) = \chi(\mathbf{P}^n_k, i_*\mathcal{O}_V)
= \chi(V, \mathcal{O}_V).
$$
The last equality follows from Cohomology of Schemes, Lemma
\ref{coherent-lemma-relative-affine-cohomology}. Thus, over the fixed ground
field, this value is intrinsic to $V$. The cited source calls it the
arithmetic genus. If all irreducible components have dimension $p$, its
comparison with the classical convention is
$\chi(V, \mathcal{O}_V) = 1 + (-1)^p p_a(V)$; compare Curves, Remark
\ref{curves-remark-genus-higher-dimension}.
\end{remark}
...
\begin{reference}
\cite[Chapter III, \S 6, no. 81, Proposition 6, pp. 276--277]{FAC}
\end{reference}
\begin{history}
For twists by $\mathcal{O}(1)$ on projective space, the cited proposition
proves the sharper equality between degree and support dimension for a
nonzero sheaf. The lemma below allows arbitrary invertible modules and hence
only gives an upper bound: the trivial invertible module on a
positive-dimensional proper scheme already shows why equality would fail.
The zero sheaf is separated explicitly because its support is empty.
\end{history}
Let $k$ be a field. Let $X$ be a proper scheme over $k$. Let $\mathcal{F}$
be a coherent $\mathcal{O}_X$-module. Let
$\mathcal{L}_1, \ldots, \mathcal{L}_r$ be invertible $\mathcal{O}_X$-modules.
If $\mathcal{F} = 0$, the map below is zero. If $\mathcal{F} \not = 0$, the map
$$
...
\begin{proof}
If $\mathcal{F} = 0$, the assertion is immediate. Thus we may assume
$\mathcal{F} \not = 0$.
...
\end{reference}
\begin{history}
The cited source obtains the degree from a regular hyperplane section. The
lemma below isolates the top-dimensional contribution by generic lengths;
it supplies a field-independent route to the same degree equality for
projective twists. The separate nonzero branch makes the support dimension and
the strict degree bound unambiguous.
\end{history}
Let $k$ be a field. Let $X$ be a proper scheme over $k$. Let
$\mathcal{F}$ be a coherent $\mathcal{O}_X$-module. Let
$\mathcal{L}_1, \ldots, \mathcal{L}_r$ be invertible $\mathcal{O}_X$-modules.
If $\mathcal{F} = 0$, then the corresponding Euler characteristic map is
zero. If $\mathcal{F} \not = 0$, let
$d = \dim(\text{Supp}(\mathcal{F}))$.
...
\begin{proof}
The assertion for $\mathcal{F} = 0$ is immediate. Thus we may assume
$\mathcal{F} \not = 0$.
...
\end{reference}
\begin{history}
The cited proof treats the structure sheaf of a connected reduced closed
subvariety whose irreducible components have positive dimension. It rules out
sections of every negative twist by multiplying with positive-degree forms.
The lemma below has broader sheaf and ample-invertible-sheaf hypotheses, but
only concludes vanishing for all sufficiently negative twists. Thus neither
form strictly contains the other.

The final asymptotic argument is carried out on the integral closed subscheme
$Z$. Earlier text inadvertently switched its global sections from $Z$ to the
ambient scheme $X$; that subscript is corrected below.
\end{history}
Let $k$ be a field. Let $X$ be a proper scheme over $k$. Let $\mathcal{L}$
...
nothing to show. If $\dim(Z) > 0$, then we will show that
$\Gamma(Z, \mathcal{L}^{\otimes n}|_Z) = 0$
for $n < 0$. Namely, suppose that
$s\in\Gamma(Z,\mathcal L^{\otimes -a}|_Z)$ is nonzero for some $a>0$.
Choose $b>0$ and a nonzero section
$t\in\Gamma(Z,\mathcal L^{\otimes b}|_Z)$ (this is possible as
$\mathcal{L}$ is ample, see Properties, Proposition
\ref{properties-proposition-characterize-ample}). Then
$s^{\otimes b}\otimes t^{\otimes a}$ is a nonzero global section of
$\mathcal{O}_Z$
...
of a positive power of $\mathcal{L}|_Z$.
Thus the function
$n \mapsto \dim_k \Gamma(Z, \mathcal{L}^{\otimes n}|_Z)$
is bounded on an infinite set of positive integers which contradicts
asymptotic Riemann-Roch
(Proposition \ref{proposition-asymptotic-riemann-roch}, applied to $Z$)
since $\dim(Z) > 0$.
\end{proof}
...
\begin{reference}
\cite[Chapter III, \S 5, no. 76, Theorem 4, Corollary, and Remarks,
pp. 270--271]{FAC}
\end{reference}
\begin{history}
The cited theorem treats an irreducible normal projective variety $X$ of
dimension at least $2$ and a coherent sheaf whose stalks are finite free. It
proves the asserted negative-twist vanishing by bounding the projective
dimension of every stalk over the ambient projective space. The local
argument successively divides by two module nonzerodivisors; the first uses
the domain property, while the second uses the absence of embedded primes in
a principal quotient of a normal domain and the associated-prime criterion
for the existence of a nonzerodivisor. Algebra, Lemma
\ref{algebra-lemma-projective-dimension-modulo-nonzerodivisor} retains the
resulting change of projective dimension.

The statement below is stronger and expresses the intrinsic input as depth.
The source variety is proper by Morphisms, Lemma
\ref{morphisms-lemma-locally-projective-proper}, and the restriction
$\mathcal{O}_X(1)$ is ample by Properties, Lemma
\ref{properties-lemma-ample-on-closed}.
Indeed, at a closed point of the source variety, Varieties, Lemma
\ref{lemma-dimension-locally-algebraic} identifies the local dimension with
the dimension of the irreducible variety. Normality implies $(S_2)$ by
Properties, Lemma \ref{properties-lemma-criterion-normal}; hence the local
ring has depth at least $2$, and the same is true for a finite free stalk.
Taking $\mathcal{L}=\mathcal{O}_X(1)$ and $m=-n$ recovers the source theorem,
and taking $\mathcal{F}=\mathcal{O}_X$ recovers its corollary.

The source attributes its classical syzygy proof to Gr\"obner. It also notes
that the ambient projective-dimension bound can be sharp even in dimension
greater than $2$, using cones over projectively normal irregular hyperplane
sections. This sharpness remark concerns the source's ambient resolution
method; the intrinsic depth statement below does not require that bound.

The proof below corrects three typographical slips in its prior form: the
space of linear forms is on $\mathbf{P}^n_k$, the two vertical arrows have
exponent $m-m'$, and the ambient local ring at $y=i(x)$ is subscripted by
$y$.
\end{history}
Let $k$ be a field. Let $X$ be a proper scheme over $k$. Let $\mathcal{L}$
be an ample invertible $\mathcal{O}_X$-module. Let $\mathcal{F}$ be a
...
$\mathcal{O}_{\mathbf{P}^n_k, y}$ and we can use for example
Algebra, Lemma \ref{algebra-lemma-depth-goes-down-finite}
to get the equality.

\medskip\noindent
Assume $X = \mathbf{P}^n_k$ and $\mathcal{L} = \mathcal{O}(1)$ and
$k$ is infinite. Choose $s \in H^0(\mathbf{P}^n_k, \mathcal{O}(1))$
