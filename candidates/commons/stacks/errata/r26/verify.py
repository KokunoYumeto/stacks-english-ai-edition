...
if tuple(CONFIG["stems"]) != (STEM,):
    raise AssertionError("R26 validation requires the single configured smoothing stem")
STRUCTURE_PATTERNS = {
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate the complete sealed R26 candidate evidence chain.")
    parser.parse_args()
    ...
    try:
        ...
        adjudication = json.loads((ROOT / "R26_SMOOTHING_ADJUDICATION_SPEC.json").read_text(encoding="utf-8"))
        prior_alias_rows = adjudication.get("prior_aliases", [])
        prior_aliases = [row.get("producer_id") for row in prior_alias_rows]
        prior_alias_stable_ids = [row.get("stable_id") for row in prior_alias_rows]
        if (
            prior_aliases != CONFIG["prior_alias_producer_ids"]
            or any(not isinstance(value, str) or not value for value in prior_aliases + prior_alias_stable_ids)
            or len(prior_aliases) != len(set(prior_aliases))
            or len(prior_alias_stable_ids) != len(set(prior_alias_stable_ids))
        ):
        ...
        if (
            ...
            or closure["producer_rows"] != len(all_producers) + len(rejected_producers) + len(prior_aliases)
            or not decisions
        ):
