...
note that $\kappa(\mathfrak p)$ is separable over $R/\pi R$
by our assumption on $R/\pi R \subset \Lambda/\pi \Lambda$.
Hence we conclude that we can find generators
$g_1, \ldots, g_t \in I$ such that $g_j \in \mathfrak p_B^2$
...
In particular, $K$ is separable over the fraction field of $R$
...
and some integer $N$. After replacing $a_k$ by $(a_k)^N + b_k$ we get
\begin{enumerate}
...
\medskip\noindent
Ad (\ref{item-strictly-standard}). Note that $A_{a_j}$ is smooth
...
$A \to B \to A^\wedge$ with $A \to B$ smooth and
