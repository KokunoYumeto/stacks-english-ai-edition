...
SPEC_INPUT_NAME = "R26_SMOOTHING_ADJUDICATION_SPEC.input.json"
SPEC_NAME = "R26_SMOOTHING_ADJUDICATION_SPEC.json"
REVIEW_NAME = "R26_SMOOTHING_REVIEW.md"
PRODUCER_LEDGER_INPUT_NAME = "SMOOTHING_SOURCE_DEFECT_LEDGER.input.csv"
PRODUCER_LEDGER_CANON_NAME = "SMOOTHING_SOURCE_DEFECT_LEDGER.csv"
PRODUCER_EMENDATIONS_INPUT_NAME = "SMOOTHING_SOURCE_EMENDATIONS.input.json"
PRODUCER_EMENDATIONS_CANON_NAME = "SMOOTHING_SOURCE_EMENDATIONS.json"
...
def generated_targets() -> list[Path]:
    return [
        ROOT / "authority/source/smoothing.tex",
        ROOT / "authority/COPYING",
        ROOT / "authority/upstream.lock.json",
        ROOT / f"authority/canon/{SPEC_NAME}",
        ROOT / "authority/canon/R26_SMOOTHING_INTAKE_VALIDATION.json",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Materialize the bounded R26 Smoothing source candidate.")
    ...
    if (
        ...
        or len(operation_spec["operations"]) != 39
        or stable_input["unit_count"] != 25
        or config_input["accepted"] != 25
        or config_input["rejected"] != 1
        or len(rejections) != 1
        ...
        raise AssertionError("R26 intake closure mismatch")
    if [row["stable_id"] for row in spec["accepted"]] != EXPECTED_IDS:
        raise AssertionError("R26 stable IDs are not contiguous 1177..1201")

    targets = generated_targets()
    existing = [path for path in targets if path.exists()]
    if existing and not args.overwrite_generated:
        raise FileExistsError("generated R26 material already exists; use --overwrite-generated for exact-file replay")
    ...
    upstream_lock = {
        ...
        "scope": "single_frozen_source_file_for_errata_r26",
    }

    write_bytes(ROOT / SPEC_NAME, spec_bytes)
    write_bytes(ROOT / "authority/source/smoothing.tex", authority)
    ...
    write_bytes(ROOT / "payload/smoothing.tex", payload)
    ...
    write_json(ROOT / "formula-diagram-inventory.json", {
        ...
        "classification": "Every accepted R26 unit is classified exactly once from its exact smoothing.tex operation; no accepted operation changes xymatrix source.",
    ...
    config["authority_evidence"] = [
        ...
        artifact(ROOT / "authority/canon/R26_SMOOTHING_INTAKE_VALIDATION.json", "authority/canon/R26_SMOOTHING_INTAKE_VALIDATION.json"),
        ...
        artifact(ROOT / "authority/source/smoothing.tex", "authority/source/smoothing.tex"),
    ...
    print(json.dumps({
        ...
        "units": 25,
        "operations": 39,
        "accepted_producer_ids": 31,
        "rejected_producer_ids": 1,
        "payload_bytes": len(payload),
        "payload_sha256": sha_bytes(payload),
