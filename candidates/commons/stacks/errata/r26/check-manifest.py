...
def validate_manifest_shape(manifest: dict[str, object]) -> None:
    ...
    for key in ("candidate_id", "lease_id", "namespace", "writer_task"):
    ...
    if closure["expected_units"] != CONFIG["accepted"] or closure["manifested_units"] != CONFIG["accepted"]:
        raise AssertionError("source_closure count differs from sealed R26 configuration")
    ...
    replay_exists = (ROOT / "replay" / "independent-review.json").is_file()
    if replay_exists != (manifest["review_state"] == "performed" and manifest["independent_replay"] == "passed"):
        raise AssertionError("manifest review state disagrees with replay-receipt presence")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check the R26 manifest shape, identity, hashes, and file closure.")
    parser.parse_args()
