...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the pre-review or final R26 candidate manifest.")
    parser.parse_args()
    ...
    if validation.get("passed") is not True or validation.get("candidate_id") != CONFIG["candidate_id"]:
        raise AssertionError("current R26 validation receipt is absent, stale, or failed")
    if (
        ...
        raise AssertionError("current R26 visual inspection receipt is absent, stale, or failed")
    ...
    first_producer = f"SMOOTHING-{min(packet_numbers):03d}"
    last_producer = f"SMOOTHING-{max(packet_numbers):03d}"
    ...
    manifest = {
        ...
        **{key: evidence(path) for key, path in singled.items()},
        ...
        "unresolved_defects": [
            replay_note,
            "The isolated smoothing.tex candidate retains cross-chapter undefined-reference targets; their multiset matches the frozen authority build exactly.",
