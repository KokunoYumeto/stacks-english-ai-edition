...
R29 source adjudication and materialization are bounded for `sites-modules.tex`: 30 semantic units and 31 exact operations produce the payload identified in `candidate.config.json`. Source materialization alone establishes no PDF, render, visual-inspection, manifest, or independent-review PASS.
...
The runner copies the frozen upstream tree, verifies authority and candidate source hashes, and builds candidate and authority versions of `sites-modules.tex` with `pdflatex`, `bibtex`, `pdflatex`, `pdflatex`. Both independent runs use `SOURCE_DATE_EPOCH=1788012044`, `FORCE_SOURCE_DATE=1`, and `TZ=UTC`.
