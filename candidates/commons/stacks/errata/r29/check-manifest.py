...
def validate_manifest_shape(manifest: dict[str, object]) -> None:
    ...
    if manifest.get("$schema") != "../../../../../schemas/candidate-manifest.schema.json":
        raise AssertionError("wrong manifest $schema path")
    patterns = {
    ...
    for key, pattern in patterns.items():
        ...
        if manifest[key] != CONFIG[key]:
            raise AssertionError(f"manifest/config {key} mismatch")
    ...
    if upstream["commit"] != CONFIG["authority_commit"] or upstream["tree"] != CONFIG["authority_tree"]:
        raise AssertionError("upstream identity mismatch")
    expected_closure = {
        "enumerated": True,
        "expected_units": CONFIG["accepted"],
        "manifested_units": CONFIG["accepted"],
        "complete": True,
    }
    if manifest["source_closure"] != expected_closure:
        raise AssertionError("source closure differs from sealed R29 configuration")
    ...
    replay_is_final = False
    if replay_path.is_file():
        replay_is_final = json.loads(replay_path.read_text(encoding="utf-8")).get("schema") == "mathematics-commons-stacks-errata-independent-review/v1"
    if replay_is_final != (manifest["review_state"] == "performed" and manifest["independent_replay"] == "passed"):
        raise AssertionError("manifest review state disagrees with final replay receipt")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check the R29 manifest shape, identity, hashes, and file closure.")
    parser.parse_args()
    ...
    actual = sorted(
        ...
        if path.is_file() and path.name != "candidate.manifest.json" and "__pycache__" not in path.parts
    )
