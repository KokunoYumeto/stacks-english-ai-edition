...
if tuple(CONFIG["stems"]) != (STEM,):
    raise AssertionError("R29 validation requires the single configured sites-modules stem")
STRUCTURE_PATTERNS = {
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate the complete sealed R29 candidate evidence chain.")
    parser.parse_args()
    ...
    try:
        ...
        raw_rows = [raw for row in stable["units"] for raw in row["raw_producer_ids"]]
        map_raw_rows = [raw for row in source_map for raw in row["producer_identity"]["raw_producer_ids"]]
        leases = [row["producer_identity"]["lease"] for row in source_map]
        if (
            ...
            or raw_rows != map_raw_rows
            or len(raw_rows) != len(expected_ids)
            or any(lease not in {"A", "B", "C", "D", "independent"} for lease in leases)
        ):
            raise AssertionError("stable-unit, alias, raw-ID, or source-map closure mismatch")
        report["checks"]["unit_closure"] = {
            "passed": True,
            "expected": len(expected_ids),
            "manifested": len(ids),
            "producer_aliases": len(stable_aliases),
            "raw_id_rows": len(raw_rows),
            "raw_id_strings_unique": len(set(raw_rows)),
        }
        ...
        page_binding = CONFIG["visual_qa"]["source_page_map"]
        page_path = ROOT / page_binding["path"]
        ...
        configured_review = CONFIG["visual_qa"]["correction_sensitive_pages"][STEM]
        configured_high = CONFIG["visual_qa"]["high_resolution_pages"][STEM]
        if (
            page_path.stat().st_size != page_binding["bytes"]
            or sha256(page_path) != page_binding["sha256"]
            ...
            or mapped_unique_pages != configured_review
            or configured_high != configured_review
