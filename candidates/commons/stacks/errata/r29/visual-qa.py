...
def configured_pages(stem: str, key: str, page_count: int) -> list[int]:
    """Return a frozen, explicitly configured PDF-page inventory.

    R29 source loci cannot be mapped safely to final reader pages until the
    payload has been built. The build operator must therefore record the Sites Modules
    chapter page inventory in candidate.config.json instead of this verifier
    guessing or inheriting prior-round page numbers.
    """
    visual = CONFIG.get("visual_qa")
    if not isinstance(visual, dict):
        raise AssertionError("candidate.config.json lacks visual_qa configuration")
    inventories = visual.get(key)
    if not isinstance(inventories, dict) or stem not in inventories:
        raise AssertionError(f"candidate.config.json lacks visual_qa.{key}.{stem}")
    pages = inventories[stem]
    if (
        ...
        raise AssertionError(f"invalid visual_qa.{key}.{stem} page inventory")
    return pages
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Record R29 structural and manual visual QA after the render inspection is complete."
    )
    ...
    report = {
        ...
        "render_protocol": {
            ...
            "high_resolution_review": "The manifest-bound high-resolution page inventory was generated at 180 dpi; every configured correction-sensitive page was manually inspected at that resolution",
    ...
    for stem in CONFIG["stems"]:
        ...
        if render_pdf.get("pdf_sha256") != sha256(pdf) or render_pdf.get("dpi") != 96:
        ...
        high_res_pages = configured_pages(stem, "high_resolution_pages", state["pages"])
        review_pages = configured_pages(stem, "correction_sensitive_pages", state["pages"])
        ...
        if high_res_manifest.get("dpi") != 180 or [row.get("page") for row in high_res_rows] != high_res_pages:
