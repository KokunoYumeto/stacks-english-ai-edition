...
def main() -> int:
    ...
    manifest = {
        ...
        **{key: evidence(path) for key, path in singled.items()},
        ...
        "unresolved_defects": [
            replay_note,
            "The isolated spaces-duality.tex candidate retains cross-chapter undefined-reference targets; their multiset matches the frozen authority build exactly.",
            ...
            f"The bounded intake from {first_producer} through {last_producer} assigns {closure['accepted']} semantic units to {len(CONFIG['expected_all_producer_ids'])} accepted producer identities; {closure['rejected']} proposal is rejected, {closure['intentionally_absent']} producer ID is intentionally absent, {closure['prior_aliases']} prior producer allegation aliases are retained, and none is unresolved.",
