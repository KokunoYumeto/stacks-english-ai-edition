...
def verify_payload(config: dict, source_map: list[dict]) -> dict:
    authority = (ROOT / "authority/source/spaces-duality.tex").read_bytes()
    payload = (ROOT / "payload/spaces-duality.tex").read_bytes()
    operations: list[tuple[int, int, bytes, bytes, str]] = []
    for unit in source_map:
        if unit["source"] != "spaces-duality.tex":
            raise AssertionError("R24 source map contains an unexpected source")
    ...
    if replay != payload:
        raise AssertionError("payload contains changes outside the 57 mapped operations")

    stem = config["stems"]["spaces-duality"]
...
def main() -> int:
    ...
    try:
        ...
        report["checks"]["authority_evidence"] = {
            "passed": True,
            "files": len(config["authority_evidence"]),
        }

        spec_input = ROOT / "R24_SPACES_DUALITY_ADJUDICATION_SPEC.input.json"
        spec_generated = ROOT / "R24_SPACES_DUALITY_ADJUDICATION_SPEC.json"
        ...
        expected_range = [f"MC-STK-ERR-{number:04d}" for number in range(1008, 1046)]
        if expected_ids != expected_range:
            raise AssertionError("configured stable IDs are not contiguous 1008..1045")
        ...
        if (
            ...
            or operation_spec["operation_count"] != 57
            or operation_spec["authority_sha256"] != config["stems"]["spaces-duality"]["authority_sha256"]
        ...
        if payload["operation_count"] != 57:
            raise AssertionError("exact payload operation-count mismatch")
        report["checks"]["exact_payload"] = {"passed": True, "spaces-duality": payload}
