...
def configured_pages(stem: str, key: str, page_count: int) -> list[int]:
    ...
    R21 source loci cannot be mapped safely to final reader pages until the
    payload has been built.  The materializer must therefore record the
    Simplicial chapter page inventory in candidate.config.json instead of this verifier
    ...
    for stem in CONFIG["stems"]:
        ...
        report["pdfs"].append({
            ...
            **state,
            ...
            "reflow_note": (
                f"Candidate and authority page counts are recorded above; every candidate page was inspected, including all pages directly affected by the {CONFIG['operation_count']} mapped Simplicial operations."
            ),
        })
