...
def replay_payload() -> dict:
    authority_path = ROOT / "authority" / "source" / "simplicial.tex"
    payload_path = ROOT / "payload" / "simplicial.tex"
    operation_path = ROOT / "operation-spec.json"
    authority = authority_path.read_bytes()
    if sha_bytes(authority) != CONFIG["stems"]["simplicial"]["authority_sha256"]:
...
def main() -> int:
    ...
    candidate_pdf, authority_pdf = ROOT / "builds" / "simplicial.pdf", ROOT / "builds" / "simplicial.authority.pdf"
    ...
    render_pdf = render_manifest.get("pdfs", {}).get("simplicial", {})
