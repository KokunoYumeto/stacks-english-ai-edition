...
SPEC_PATH = ROOT / "authority" / "canon" / "R19_DERIVED_021_063_ADJUDICATION_SPEC.json"
SPEC_SHA256 = "C24870546F3E65D18E699CFC2D442778B2296A9B5BE1C63D7B7F10AFE2A0143C"
REVIEW_PATH = ROOT / "authority" / "canon" / "R19_DERIVED_021_063_REVIEW.md"
REVIEW_SHA256 = "3E1BCA9ED0746FE39D0AB84C4CC6D95F53C255D97C459E237D6775D17644B882"
EXPECTED_PAYLOAD_BYTES = 449_674
EXPECTED_PAYLOAD_SHA256 = "64A9B8C5568B575457AE8E81DB54F39D9590260422EE4BD5710EB743CC6E7C31"
...
def main() -> int:
    ...
    if (
        spec["candidate_id"] != "stacks-errata-a04446e-r19"
    ...
    expected_producers = [f"DERIVED-{number:03d}" for number in range(21, 64)]
    ...
    if sorted(accepted_producers + rejected_producers) != expected_producers:
        raise AssertionError("adjudication partition is not DERIVED-021..063")
    expected_ids = [f"MC-STK-ERR-{number:04d}" for number in range(832, 875)]
    if [row["stable_id"] for row in accepted] != expected_ids:
        raise AssertionError("stable-ID assignment is not contiguous 0832..0874")
    if rejected_producers:
        raise AssertionError("R19 has no rejected producer rows")

    generated = [
        ROOT / "authority" / "source" / "derived.tex",
        ROOT / "authority" / "upstream.lock.json",
        ROOT / "authority" / "COPYING",
        ROOT / "authority" / "canon" / "R19_DERIVED_021_063_INTAKE.json",
    ...
    if len(accepted) != 43 or len(operations) != 57:
        raise AssertionError("R19 count contract mismatch")
    ...
    if len(payload) != EXPECTED_PAYLOAD_BYTES or sha_bytes(payload) != EXPECTED_PAYLOAD_SHA256:
        raise AssertionError("R19 deterministic payload identity mismatch")
    ...
    bounded_start_line = 4361
    bounded_end_line = 8603
    ...
    if len(bounded_operations) != len(operations):
        raise AssertionError("an R19 operation lies outside the declared bounded source")
    ...
    intake = {
        ...
        "closure": {
            ...
            "unresolved": 0,
            "stable_ids": "MC-STK-ERR-0832..MC-STK-ERR-0874",
        },
        "deduplication": {
            "registered_entries_checked": 18,
            "public_candidates_checked": "R1..R18",
            "duplicate_aliases": {},
    ...
    write_json(ROOT / "authority" / "canon" / "R19_DERIVED_021_063_INTAKE.json", intake)
    ...
    decisions = [
        ("ERR-R19-D0001", "Bind R19 exclusively to the active errata/r19 lease and frozen upstream commit/tree.", "Preserves the single-writer and immutable-authority boundary."),
        ("ERR-R19-D0002", "Assign MC-STK-ERR-0832..0874 in accepted DERIVED producer order after admitted R18.", "Creates a deterministic contiguous stable-ID sequence for 43 proved nonduplicate corrections."),
        ("ERR-R19-D0003", "Apply only exact nonoverlapping half-open UTF-8 spans in descending byte order.", "Prevents contextual or global replacement from changing unrelated source text."),
        ("ERR-R19-D0004", "Accept DERIVED-021..063 after independent source replay and deduplication against R1-R18.", "Each allegation is a distinct bounded defect proved directly from frozen derived.tex and surrounding project definitions."),
        ("ERR-R19-D0005", "Freeze R19 at 43 units and 57 operations, DERIVED-021..063.", "Prevents later producer growth beginning at DERIVED-064 from moving this round underneath its hashes."),
        ("ERR-R19-D0006", "Use producer evidence only as a bounded allegation packet, not as correction authority.", "Frozen source, independent exact-preimage replay, mathematical typing, and admitted-history deduplication bind the overlay."),
        ("ERR-R19-D0007", "Admit canon amendments for DERIVED-034 and DERIVED-037.", "The first restores the typed degree-n alpha index; the second absorbs the dangling conjunction with the deleted placeholder."),
        ("ERR-R19-D0008", "Keep canonical locale bytes and the literal upstream mirror outside the corrected English payload.", "Corrections belong only to the Unofficial AI-Integrated Stacks Project."),
        ("ERR-R19-D0009", "Do not contact or submit this AI-derived correction set to upstream Stacks maintainers.", "The maintained AI-open English edition is the authorized correction surface."),
    ]
    ...
    config = {
        ...
        "candidate_id": spec["candidate_id"],
        "lease_id": "stacks-lease-000022-errata-r19",
        ...
        "namespace": "commons/stacks/errata/r19",
        "private_render_logical_path": "canon/private_evidence/errata-r19-20260825T1430Z/render-final/render-manifest.json",
        "operation_count": len(operations),
        "visual_qa": {
            ...
            "derived": {
                ...
                "ordered_structure_exceptions": {
                    "references": [
                        {
                            "index_0based": 450,
                            "authority": "\\ref{lemma-filtered-acyclic}",
                            "payload": "\\ref{homology-lemma-filtered-acyclic}",
                        }
                    ]
                },
                "build_exceptions": {
                    "candidate_only_undefined_reference_targets": {
                        "homology-lemma-filtered-acyclic": 1,
                    },
