...
GENERATED_AT = "2026-08-25T01:10:00Z"
SPEC_PATH = ROOT / "authority" / "canon" / "R16_HOMOLOGY_001_031_ADJUDICATION_SPEC.json"
...
def main() -> int:
    ...
    if (
        spec["candidate_id"] != "stacks-errata-a04446e-r16"
    ...
    expected_producers = [f"HOMOLOGY-{number:03d}" for number in range(1, 32)]
    ...
    if sorted(accepted_producers + rejected_producers) != expected_producers:
        raise AssertionError("adjudication partition is not HOMOLOGY-001..031")
    expected_ids = [f"MC-STK-ERR-{number:04d}" for number in range(753, 784)]
    if [row["stable_id"] for row in accepted] != expected_ids:
        raise AssertionError("stable-ID assignment is not contiguous 0753..0783")
    if rejected_producers:
        raise AssertionError("R16 has no rejected producer rows")

    generated = [
        ROOT / "authority" / "source" / "homology.tex",
        ROOT / "authority" / "upstream.lock.json",
        ROOT / "authority" / "COPYING",
        ROOT / "authority" / "canon" / "R16_HOMOLOGY_001_031_INTAKE.json",
        ROOT / "authority" / "canon" / "R16_HOMOLOGY_001_031_REVIEW.md",
    ...
    bounded_start_line = 1
    bounded_end_line = 5155
    ...
    intake = {
        ...
        "allegation_locator_policy": {
            ...
            "adjudication_authorities": [
                ...
                "all fifteen admitted overlay source maps and rejection ledgers",
            ],
        },
        "allegation_locators": [
        ...
        "closure": {
            ...
            "unresolved": 0,
            "stable_ids": "MC-STK-ERR-0753..MC-STK-ERR-0783",
        },
        "deduplication": {
            "registered_entries_checked": 15,
            "public_candidates_checked": "R1..R15",
            "duplicate_aliases": {},
    ...
    write_json(ROOT / "authority" / "canon" / "R16_HOMOLOGY_001_031_INTAKE.json", intake)
    ...
    decisions = [
        ("ERR-R16-D0001", "Bind R16 exclusively to the active errata/r16 lease and frozen upstream commit/tree.", "Preserves the single-writer and immutable-authority boundary."),
        ("ERR-R16-D0002", "Assign MC-STK-ERR-0753..0783 in accepted HOMOLOGY producer order after admitted R15.", "Creates a deterministic contiguous stable-ID sequence for 31 proved nonduplicate corrections."),
        ("ERR-R16-D0003", "Apply only exact nonoverlapping half-open UTF-8 spans in descending byte order.", "Prevents contextual or global replacement from changing unrelated source text."),
        ("ERR-R16-D0004", "Accept HOMOLOGY-001..031 after independent source replay and deduplication against R1-R15.", "Each allegation is a distinct bounded defect proved directly from frozen homology.tex and surrounding project definitions."),
        ("ERR-R16-D0005", "Pin the delivered 31-row producer snapshot after proving it reserializes to 12845 bytes and its delivered SHA-256 despite later producer-ledger growth.", "Prevents concurrent later Homology rows from entering this bounded round."),
        ("ERR-R16-D0006", "Use producer evidence only as a bounded allegation packet, not as correction authority.", "Frozen source, independent exact-preimage replay, mathematical typing, and admitted-history deduplication bind the overlay."),
        ("ERR-R16-D0007", "Keep canonical locale bytes and the literal upstream mirror outside the corrected English payload.", "Corrections belong only to the Unofficial AI-Integrated Stacks Project."),
        ("ERR-R16-D0008", "Do not contact or submit this AI-derived correction set to upstream Stacks maintainers.", "The maintained AI-open English edition is the authorized correction surface."),
    ]
    ...
    review_lines = [
        "# Independent review: HOMOLOGY-001–031",
        "",
        f"Frozen authority: `{AUTHORITY_SHA256}` at `{spec['authority_commit']}`.",
        "",
        "The 31 bounded allegations were replayed directly against frozen `homology.tex` and",
        "deduplicated against every admitted R1–R15 source map and rejection ledger. All 31",
        "are accepted as `MC-STK-ERR-0753` through `MC-STK-ERR-0783`; the source map binds",
        "35 exact nonoverlapping operations because four allegations contain coupled edits.",
        "",
        "The mathematical subset repairs a cokernel projection label, malformed quotient",
        "syntax, extension and cohomology naming, total-differential scope, bidegree component",
        "types and shifts, a graded-quotient map and kernel, and a filtration induction",
        "inequality. The remaining units are exact bounded grammar or typography repairs.",
        "",
        ...
        "review accepted all twelve typed/index-sensitive units without disagreement.",
    ...
    (ROOT / "authority" / "canon" / "R16_HOMOLOGY_001_031_REVIEW.md").write_text(
        "\n".join(review_lines) + "\n",
        encoding="utf-8",
        newline="",
    )
    ...
    config = {
        ...
        "candidate_id": spec["candidate_id"],
        "lease_id": "stacks-lease-000019-errata-r16",
        ...
        "namespace": "commons/stacks/errata/r16",
        "private_render_logical_path": "canon/private_evidence/errata-r16-20260825T0110Z/render-final/render-manifest.json",
        "operation_count": len(operations),
        "stems": {
            "homology": {
                "authority_sha256": AUTHORITY_SHA256,
