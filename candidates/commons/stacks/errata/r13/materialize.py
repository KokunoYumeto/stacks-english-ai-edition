...
PRODUCER_SHA256 = "D580A489F44E9702A92C9C1FE510657B4ABB4FA1078A7A8014EDA95845A19F12"
PRODUCER_BYTES = 369_066
INTAKE_SHA256 = "DDB967CBD822A29DB067158A4421DB64F72B36082BA28DF936E063BF2FC20927"
INTAKE_BYTES = 5_618
...
SPEC_SHA256 = "BADFF50C2CBB2D0DA8AB4526FF1EE7278E2E296D368A38870DB3E5D5FA520BD7"
GENERATED_AT = "2026-08-24T21:45:43Z"
...
def main() -> int:
    ...
    spec_path = ROOT / "authority" / "canon" / "R13_ALG_392_394_ADJUDICATION_SPEC.json"
    ...
    expected_producers = [f"ALGEBRA-{number}" for number in range(392, 395)]
    ...
    if sorted(accepted_producers + rejected_producers) != sorted(expected_producers):
        raise AssertionError("adjudication partition is not ALGEBRA-392..394")
    expected_ids = [f"MC-STK-ERR-{number:04d}" for number in range(735, 738)]
    if [row["stable_id"] for row in accepted] != expected_ids:
        raise AssertionError("stable-ID assignment is not contiguous 0735..0737")
    ...
    generated = [
        ...
        ROOT / "authority" / "canon" / "R13_ALG_392_394_INTAKE.json",
        ROOT / "authority" / "canon" / "R13_ALG_392_394_REVIEW.md",
    ...
    write_json(ROOT / "authority" / "canon" / "R13_ALG_392_394_INTAKE.json", intake)
    ...
    decisions = [
        ("ERR-R13-D0001", "Bind R13 exclusively to the active errata/r13 lease and frozen upstream commit/tree.", "Preserves the single-writer and immutable-authority boundary."),
        ("ERR-R13-D0002", "Assign MC-STK-ERR-0735..0737 in producer order after admitted R12.", "Creates a deterministic contiguous stable-ID sequence for all three accepted rows."),
        ("ERR-R13-D0003", "Apply only exact nonoverlapping half-open UTF-8 spans in descending byte order.", "Prevents contextual or global replacement from changing unrelated source text."),
        ("ERR-R13-D0004", "Accept all three producer rows after independent source replay and deduplication against R1-R12.", "Every accepted row is a distinct bounded defect supported by the frozen source and queue checkpoint."),
        ("ERR-R13-D0005", "Keep canonical locale bytes and the literal upstream mirror outside the corrected English payload.", "Corrections belong only to Stacks — English AI Edition."),
        ("ERR-R13-D0006", "Do not contact or submit this AI-derived correction set to upstream Stacks maintainers.", "The maintained AI-open English edition is the authorized correction surface."),
    ]
    ...
    (ROOT / "rejections.jsonl").write_text(
        "".join(json.dumps({"schema": "mathematics-commons-stacks-candidate-rejection/v1",
            "id": f"ERR-R13-R{index:04d}", "timestamp_utc": GENERATED_AT,
    ...
    review_lines = [
        "# Independent review: ALGEBRA-392–394", "",
        f"Frozen authority: `{AUTHORITY_SHA256}` at `{spec['authority_commit']}`.", "",
        "The three producer rows were replayed against the frozen source and are accepted",
        "as `MC-STK-ERR-0735` through `MC-STK-ERR-0737`. No unit is rejected, aliased, or",
        ...
        "all six operations, exact source-line bounds, deduplication against R1–R12, and the",
    ...
    (ROOT / "authority" / "canon" / "R13_ALG_392_394_REVIEW.md").write_text(
        "\n".join(review_lines) + "\n", encoding="utf-8", newline="")
    ...
    config = {
        ...
        "candidate_id": spec["candidate_id"], "lease_id": "stacks-lease-000016-errata-r13",
        ...
        "expected_producer_ids": expected_producers, "namespace": "commons/stacks/errata/r13",
        "private_render_logical_path": "canon/private_evidence/errata-r13-20260824T2145Z/render-final/render-manifest.json",
