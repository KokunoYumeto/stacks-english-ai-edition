...
PDF = ROOT / "builds" / "more-algebra.pdf"
...
def main() -> int:
    ...
    page_root = render_root / "more-algebra"
    ...
    for sheet_index in range(math.ceil(page_count / 16)):
        ...
        for offset, page_path in enumerate(selected):
            ...
            draw.text((x + 4, y + 4), f"more-algebra.pdf page {page_number}", fill="black", font=font)
        output = contact_root / f"more-algebra_{sheet_index + 1:02d}.png"
    ...
    for page_number in sorted(set(args.high_res_pages)):
        ...
        output = highres_root / f"more-algebra_p{page_number}.png"
    ...
    manifest = {
        ...
        "published": False,
        "pdfs": {"more-algebra": {"pdf_sha256": sha256(PDF), "dpi": 96, "renders": render_rows}},
