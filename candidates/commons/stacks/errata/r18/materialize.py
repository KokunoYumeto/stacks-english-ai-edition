...
GENERATED_AT = "2026-08-25T02:29:00Z"
SPEC_PATH = ROOT / "authority" / "canon" / "R18_DERIVED_001_020_ADJUDICATION_SPEC.json"
...
def main() -> int:
    ...
    if (
        spec["candidate_id"] != "stacks-errata-a04446e-r18"
    ...
    expected_producers = [f"DERIVED-{number:03d}" for number in range(1, 21)]
    ...
    if sorted(accepted_producers + rejected_producers) != expected_producers:
        raise AssertionError("adjudication partition is not DERIVED-001..020")
    expected_ids = [f"MC-STK-ERR-{number:04d}" for number in range(812, 832)]
    if [row["stable_id"] for row in accepted] != expected_ids:
        raise AssertionError("stable-ID assignment is not contiguous 0812..0831")
    if rejected_producers:
        raise AssertionError("R18 has no rejected producer rows")

    generated = [
        ROOT / "authority" / "source" / "derived.tex",
        ROOT / "authority" / "upstream.lock.json",
        ROOT / "authority" / "COPYING",
        ROOT / "authority" / "canon" / "R18_DERIVED_001_020_INTAKE.json",
        ROOT / "authority" / "canon" / "R18_DERIVED_001_020_REVIEW.md",
    ...
    bounded_start_line = 1340
    bounded_end_line = 4062
    ...
    intake = {
        ...
        "allegation_locator_policy": {
            ...
            "adjudication_authorities": [
                ...
                "all seventeen admitted overlay source maps and rejection ledgers",
            ],
        },
        "allegation_locators": [
        ...
        "closure": {
            ...
            "unresolved": 0,
            "stable_ids": "MC-STK-ERR-0812..MC-STK-ERR-0831",
        },
        "deduplication": {
            "registered_entries_checked": 17,
            "public_candidates_checked": "R1..R17",
            "duplicate_aliases": {},
    ...
    write_json(ROOT / "authority" / "canon" / "R18_DERIVED_001_020_INTAKE.json", intake)
    ...
    decisions = [
        ("ERR-R18-D0001", "Bind R18 exclusively to the active errata/r18 lease and frozen upstream commit/tree.", "Preserves the single-writer and immutable-authority boundary."),
        ("ERR-R18-D0002", "Assign MC-STK-ERR-0812..0831 in accepted DERIVED producer order after admitted R17.", "Creates a deterministic contiguous stable-ID sequence for 20 proved nonduplicate corrections."),
        ("ERR-R18-D0003", "Apply only exact nonoverlapping half-open UTF-8 spans in descending byte order.", "Prevents contextual or global replacement from changing unrelated source text."),
        ("ERR-R18-D0004", "Accept DERIVED-001..020 after independent source replay and deduplication against R1-R17.", "Each allegation is a distinct bounded defect proved directly from frozen derived.tex and surrounding project definitions."),
        ("ERR-R18-D0005", "Freeze R18 at the 20-row and 20-operation DERIVED-001..020 subset.", "Prevents later producer growth beginning at DERIVED-021 from moving this round underneath its hashes."),
        ("ERR-R18-D0006", "Use producer evidence only as a bounded allegation packet, not as correction authority.", "Frozen source, independent exact-preimage replay, mathematical typing, and admitted-history deduplication bind the overlay."),
        ("ERR-R18-D0007", "Correct DERIVED-016 by deleting the placeholder together with its leading space.", "Avoids leaving an ungrammatical space before the sentence-ending period."),
        ("ERR-R18-D0008", "Keep canonical locale bytes and the literal upstream mirror outside the corrected English payload.", "Corrections belong only to the Unofficial AI-Integrated Stacks Project."),
        ("ERR-R18-D0009", "Do not contact or submit this AI-derived correction set to upstream Stacks maintainers.", "The maintained AI-open English edition is the authorized correction surface."),
    ]
    ...
    review_lines = [
        "# Independent review: DERIVED-001–020",
        "",
        f"Frozen authority: `{AUTHORITY_SHA256}` at `{spec['authority_commit']}`.",
        "",
        "The 20 bounded allegations were replayed directly against frozen `derived.tex` and",
        "deduplicated against every admitted R1–R17 source map and rejection ledger. All 20",
        "are accepted as `MC-STK-ERR-0812` through `MC-STK-ERR-0831`; the source map binds",
        "20 exact nonoverlapping operations.",
        "",
        "The mathematical subset repairs two undefined or wrong triangle arrows, three wrong",
        "category annotations in a Yoneda argument, and two incomplete cone-triangle tuples.",
        "The remaining units are exact bounded grammar, number, or editorial-placeholder repairs.",
        "",
        ...
        "review accepted all seven typed or tuple-sensitive units without disagreement.",
    ...
    (ROOT / "authority" / "canon" / "R18_DERIVED_001_020_REVIEW.md").write_text(
        "\n".join(review_lines) + "\n",
        encoding="utf-8",
        newline="",
    )
    ...
    config = {
        ...
        "candidate_id": spec["candidate_id"],
        "lease_id": "stacks-lease-000021-errata-r18",
        ...
        "namespace": "commons/stacks/errata/r18",
        "private_render_logical_path": "canon/private_evidence/errata-r18-20260825T0229Z/render-final/render-manifest.json",
        "operation_count": len(operations),
        "stems": {
            "derived": {
                "authority_sha256": AUTHORITY_SHA256,
