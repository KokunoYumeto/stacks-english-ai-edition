...
def replay_payload() -> dict:
    authority_path = ROOT / "authority" / "source" / "more-algebra.tex"
    payload_path = ROOT / "payload" / "more-algebra.tex"
    operation_path = ROOT / "operation-spec.json"
    authority = authority_path.read_bytes()
    if sha_bytes(authority) != CONFIG["stems"]["more-algebra"]["authority_sha256"]:
    ...
    if (
        ...
        or spec["authority_sha256"] != CONFIG["stems"]["more-algebra"]["authority_sha256"]
    ):
...
def main() -> int:
    ...
    candidate_pdf, authority_pdf = ROOT / "builds" / "more-algebra.pdf", ROOT / "builds" / "more-algebra.authority.pdf"
    ...
    render_pdf = render_manifest.get("pdfs", {}).get("more-algebra", {})
