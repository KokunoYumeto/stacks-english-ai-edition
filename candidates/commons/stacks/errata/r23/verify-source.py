...
def exact_payload(stem: str, rows: list[dict]) -> dict:
    ...
    if replay != payload:
        raise AssertionError(f"{stem}: payload contains changes outside mapped operations")
...
def main() -> int:
    report = {
        ...
        "generated_at_utc": "2026-08-26T03:45:08Z",
        "verifier_sha256": sha256(Path(__file__)),
        "scope": "source_only_no_build_render_admission_git_or_publication",
        "passed": False,
        "checks": {},
    }
    try:
        ...
        accepted_producers = [producer for row in source_map for producer in row["producer_ids"]]
        rejected_producers = [producer for row in rejections for producer in row["producer_ids"]]
        if (
            ...
            or sorted(accepted_producers) != sorted(CONFIG["expected_producer_ids"])
            or sorted(accepted_producers + rejected_producers) != sorted(CONFIG["expected_all_producer_ids"])
            or len(set(accepted_producers + rejected_producers)) != len(CONFIG["expected_all_producer_ids"])
        ):
            raise AssertionError("stable-unit/source-map/producer closure mismatch")
        report["checks"]["unit_closure"] = {
            "passed": True,
            "units": len(expected_ids),
            "accepted_producer_ids": len(accepted_producers),
            "rejected_producer_ids": len(rejected_producers),
        }
        ...
        if (
            sorted(classified) != sorted(expected_ids)
            or len(classified) != len(set(classified))
            or len(rejections) != CONFIG["proof_closure"]["rejected"]
            or any(not row["result"].startswith("rejected_") for row in rejections)
            or len(decisions) != 6
        ):
            raise AssertionError("decision/inventory closure mismatch")
        if CONFIG["lease_id"] != "stacks-lease-000027-errata-r23" or not CONFIG["lease_status"].startswith("active;"):
            raise AssertionError("R23 must bind the active registrar-issued lease")
        report["checks"]["lease_ledgers_inventory"] = {
            "passed": True,
            "lease_registry_mutated": False,
        ...
        spec_pairs = [
            (row["stable_id"], row["operation_id"], row["start_byte"], row["end_byte_exclusive"])
            for row in operation_spec["operations"]
        ]
        map_pairs = [
            (row["unit_id"], operation["operation_id"], operation["start_byte"], operation["end_byte_exclusive"])
            for row in source_map for operation in row["operations"]
        ]
        if operation_spec["operation_count"] != CONFIG["operation_count"] or len(flattened) != CONFIG["operation_count"] or spec_pairs != map_pairs:
