...
def main() -> int:
    ...
    try:
        ...
        prior_p02_aliases = [alias for row in source_map for alias in row.get("prior_p02_aliases", [])]
        if len(prior_p02_aliases) != len(set(prior_p02_aliases)):
            raise AssertionError("prior P02 alias identities are duplicated")
        closure = CONFIG["proof_closure"]
        if (
            len(rejections) != closure["rejected"]
            or len(absent) != closure["intentionally_absent"]
            or len(prior_p02_aliases) != closure["prior_p02_aliases"]
        ...
        report["checks"]["lease_ledgers_inventory"] = {
            ...
            "prior_p02_aliases": len(prior_p02_aliases),
        ...
        if (
            ...
            or operation_spec.get("authority_sha256") != CONFIG["stems"]["more-algebra"]["authority_sha256"]
