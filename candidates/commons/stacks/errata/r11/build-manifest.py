...
def main() -> int:
    ...
    first_producer = units[0]["producer_id"]
    last_producer = units[-1]["producer_id"]
    if replay:
        replay_note = (
    ...
    manifest = {
        ...
        **{key: evidence(path) for key, path in singled.items()},
        ...
        "unresolved_defects": [
            ...
            f"The sealed chapter readers have page counts {page_note}. Every candidate page and the complete correction-sensitive tail were inspected without clipping, overlap, missing glyph, broken diagram, blank page, or unreadable content.",
            ...
            f"The frozen producer snapshot closes {first_producer} through {last_producer}: all {len(units)} rows are accepted and none is rejected, aliased, or unresolved.",
