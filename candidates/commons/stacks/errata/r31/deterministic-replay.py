...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Compare the first private R31 PDF build with the current candidate replay outputs."
    )
    ...
    for stem in CONFIG["stems"]:
        for phase, public_name in (
            ("candidate", f"{stem}.pdf"),
            ("authority", f"{stem}.authority.pdf"),
        ):
            ...
            rows.append({
                ...
                "second_path": f"builds/{public_name}",
