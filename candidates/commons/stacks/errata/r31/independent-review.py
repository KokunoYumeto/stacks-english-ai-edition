...
PRIVATE_RENDER_LOGICAL_PATH = "canon/private_evidence/errata-r31-20260829/render"
if tuple(CONFIG["stems"]) != (STEM,):
    raise AssertionError("independent replay requires the single configured stem")
...
def replay_payload() -> dict:
    ...
    for row in descending:
        ...
        if (
            sha_bytes(old) != row["old_sha256"]
            or sha_bytes(replacement) != row["replacement_sha256"]
            or payload[start:end] != old
        ):
    ...
    return {
        ...
        "operation_count": len(descending),
        "descending_intervals": [[row["start_byte"], row["end_byte_exclusive"]] for row in descending],
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Perform the independent adverse replay for sealed R31 evidence.")
    ...
    if (
        ...
        raise AssertionError("stable-unit or producer-alias mapping mismatch")

    candidate_pdf = ROOT / "builds" / f"{STEM}.pdf"
    authority_pdf = ROOT / "builds" / f"{STEM}.authority.pdf"
    ...
    review = {
        ...
        "stable_id_recomputation": {
            "passed": True,
            "derivation_rule": (
                "Accepted semantic units map in first physical-source-locus order to consecutive stable IDs. "
                "Collision-free chapter aliases preserve each raw producer identity together with its lease."
            ),
            "explicit_mapping": [
                {
        ...
        "sealed_evidence_verification": {
            ...
            "candidate_pdf": {
                "path": f"builds/{STEM}.pdf", "bytes": candidate_pdf.stat().st_size,
                "sha256": sha256(candidate_pdf), "pages": len(candidate_reader.pages),
            },
            "authority_pdf": {
                "path": f"builds/{STEM}.authority.pdf", "bytes": authority_pdf.stat().st_size,
                "sha256": sha256(authority_pdf), "pages": len(authority_reader.pages),
            },
            "private_render_manifest": {
                "bytes": args.private_render_manifest.stat().st_size,
                "sha256": sha256(args.private_render_manifest),
