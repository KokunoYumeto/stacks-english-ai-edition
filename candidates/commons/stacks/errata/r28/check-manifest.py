...
def validate_manifest_shape(manifest: dict[str, object]) -> None:
    ...
    if closure["expected_units"] != CONFIG["accepted"] or closure["manifested_units"] != CONFIG["accepted"]:
        raise AssertionError("source_closure count differs from sealed R28 configuration")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check the R28 manifest shape, identity, hashes, and file closure.")
    parser.parse_args()
