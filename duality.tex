...
\begin{reference}
\cite[Chapter III, \S 4, no. 71, Proposition 2 and proof,
pp. 264--265]{FAC}
\end{reference}
\begin{history}
Over a field and for a trivial bundle of rank $r+1$, the cited source sets
$\Omega=S(-r-1)$ and proves that top positive {\v C}ech cohomology pairs
perfectly with internal graded Hom into $\Omega$ for every graded module.
Under Proj the twist $\Omega$ gives $\mathcal{O}(-r-1)$. The lemma below
upgrades that field-level graded calculation to the relative derived
right-adjoint statement $\mathcal{O}(-n-1)[n]$; it does not identify
arbitrary graded modules with quasi-coherent sheaves.
\end{history}
Let $Y$ be a quasi-compact and quasi-separated scheme.
...
\begin{history}
The value $r=0$ is allowed: it occurs for the zero ideal and the identity
closed immersion, whose Koszul complex is concentrated in degree zero.
Thus the codomain of the rank function below includes zero. In the final
parenthetical display of the proof, the derived sheaf Hom is taken over
$\mathcal O_Y$, as in the statement; using $\mathcal O_X$ there would make
the second argument $\mathcal O_Y$ ill-typed.
\end{history}
Let $Y$ be a ringed space. Let $\mathcal{I} \subset \mathcal{O}_Y$
be a sheaf of ideals. Set $\mathcal{O}_X = \mathcal{O}_Y/\mathcal{I}$ and
$\mathcal{N} =
...
\wedge^r \mathcal{N}[-r]
$$
where $r : Y \to \{0, 1, 2, \ldots \}$ sends $y$ to
...
\begin{reference}
\cite[Chapter III, \S 4, no. 72, Corollary 1, p. 267]{FAC}
\end{reference}
\begin{history}
The cited corollary proves the projective-space case in terms of a finite
graded representative: positive cohomology is the vector-space dual of the
degree-zero part of internal graded Ext into $S(-r-1)$. More on Algebra,
Lemma \ref{more-algebra-lemma-graded-local-duality-polynomial-ring}
preserves the full internal grading and the exceptional low-degree exact
sequence. The remark below is the stronger proper-scheme formulation in
terms of a dualizing complex; the two statements agree on the coherent
projective-space surface, but are not identified outside it.
\end{history}
...
\begin{lemma}
\label{lemma-duality-proper-over-field-in-range}
Let $X$ be a nonempty projective scheme over a field $k$.
Let $\mathcal{L}$ be an ample invertible $\mathcal{O}_X$-module.
Let $\omega_X^\bullet$ and $\omega_X$ be as in
Lemma \ref{lemma-duality-proper-over-field}, and set $d = \dim(X)$.
For a coherent $\mathcal{O}_X$-module $\mathcal{F}$ and $p \in \mathbf{Z}$,
consider the map
$$
\begin{aligned}
a_{\mathcal{F}, p} :
\Ext_X^{d - p}(\mathcal{F}, \omega_X)
& = \Ext_X^{-p}(\mathcal{F}, \omega_X[d]) \\
& \longrightarrow \Ext_X^{-p}(\mathcal{F}, \omega_X^\bullet) \\
& = \Hom_k(H^p(X, \mathcal{F}), k).
\end{aligned}
$$
Here the arrow is induced by the canonical map
$\omega_X[d] \to \omega_X^\bullet$. For an integer $c \geq 0$, the
following are equivalent.
\begin{enumerate}
\item The map $a_{\mathcal{F}, p}$ is an isomorphism for every coherent
$\mathcal{F}$ and $d - c \leq p \leq d$.
\item We have
$H^p(X, \mathcal{L}^{\otimes -m}) = 0$ for $m \gg 0$ and
$d - c \leq p < d$.
\item For every coherent $\mathcal{F}$ and $d - c \leq p < d$, there
is a surjection $\mathcal{E} \to \mathcal{F}$ of coherent modules such that
$$
H^p(X, \mathcal{E}) \longrightarrow H^p(X, \mathcal{F})
$$
is zero.
\item We have $H^{-d + i}(\omega_X^\bullet) = 0$ for $0 < i \leq c$.
\end{enumerate}
\end{lemma}

\begin{proof}
The canonical map in the statement exists because
$\omega_X^\bullet$ has no cohomology in degrees less than $-d$.
The final equality in the definition of $a_{\mathcal{F},p}$ is
Lemma \ref{lemma-duality-proper-over-field} part (5).

\medskip\noindent
Assume (1). For $d - c \leq p < d$, apply (1) to
$\mathcal{F} = \mathcal{L}^{\otimes -m}$. Since this module is invertible,
the source of $a_{\mathcal{F},p}$ is
$$
H^{d - p}(X, \omega_X \otimes \mathcal{L}^{\otimes m}),
$$
which is zero for $m \gg 0$ by
Cohomology of Schemes, Lemma \ref{coherent-lemma-coherent-proper-ample}.
Thus (2) holds.

\medskip\noindent
Assume (2). Given $\mathcal{F}$ and $p$ as in (3), choose $m$ sufficiently
large that $\mathcal{F} \otimes \mathcal{L}^{\otimes m}$ is globally
generated and $H^p(X, \mathcal{L}^{\otimes -m}) = 0$. Such an $m$ exists by
Properties, Proposition \ref{properties-proposition-characterize-ample}
and (2). We obtain a surjection
$$
(\mathcal{L}^{\otimes -m})^{\oplus r} \longrightarrow \mathcal{F}
$$
for some $r$, and this surjection has the property required in (3).

\medskip\noindent
Assume (3). For $q \geq 0$, set
$$
T^q(\mathcal{F}) = \Ext_X^q(\mathcal{F}, \omega_X)
\quad\text{and}\quad
U^q(\mathcal{F}) = \Hom_k(H^{d - q}(X, \mathcal{F}), k).
$$
These are contravariant delta functors on coherent modules, and the maps
$a_{\mathcal{F},d-q}$ give a morphism between them. The map
$T^0(\mathcal{F}) \to U^0(\mathcal{F})$ is an isomorphism by
Lemma \ref{lemma-duality-proper-over-field} part (6).

For $q > 0$, the functor $T^q$ is co-effaceable with respect to
surjections. Indeed, choose $m \gg 0$ and a surjection
$$
(\mathcal{L}^{\otimes -m})^{\oplus r} \longrightarrow \mathcal{F}.
$$
By Cohomology of Schemes, Lemma
\ref{coherent-lemma-coherent-proper-ample}, we may choose $m$ such that
$$
\Ext_X^q(\mathcal{L}^{\otimes -m}, \omega_X) =
H^q(X, \omega_X \otimes \mathcal{L}^{\otimes m}) = 0.
$$
Condition (3), after taking $k$-linear duals, says that $U^q$ is
co-effaceable for $1 \leq q \leq c$.

We finish by dimension shifting. Suppose by induction that
$T^{q - 1} \to U^{q - 1}$ is an isomorphism on every coherent module.
Given $\mathcal{F}$, take a surjection $\mathcal{E}_1 \to \mathcal{F}$
which kills the map from $U^q(\mathcal{F})$, and then a surjection
$\mathcal{E} \to \mathcal{E}_1$ with $T^q(\mathcal{E}) = 0$.
The composite $\mathcal{E} \to \mathcal{F}$ kills the maps from both
$T^q(\mathcal{F})$ and $U^q(\mathcal{F})$. If $\mathcal{K}$ is its kernel,
the two long exact sequences identify $T^q(\mathcal{F})$ and
$U^q(\mathcal{F})$ with the respective cokernels of
$$
T^{q - 1}(\mathcal{E}) \longrightarrow T^{q - 1}(\mathcal{K})
\quad\text{and}\quad
U^{q - 1}(\mathcal{E}) \longrightarrow U^{q - 1}(\mathcal{K}).
$$
The induction hypothesis identifies these cokernels. This proves (1).

\medskip\noindent
It remains to compare (2) and (4). By duality and invertibility of
$\mathcal{L}$, we have
$$
\Hom_k(H^p(X, \mathcal{L}^{\otimes -m}), k) =
H^{-p}(X, \omega_X^\bullet \otimes \mathcal{L}^{\otimes m}).
$$
The spectral sequence of Cohomology, Example
\ref{cohomology-example-spectral-sequence} has terms
$$
H^a(X, H^b(\omega_X^\bullet) \otimes \mathcal{L}^{\otimes m})
$$
and converges to the group on the right. There are only finitely many
nonzero coherent modules $H^b(\omega_X^\bullet)$. Hence, for $m \gg 0$,
Serre vanishing makes the spectral sequence degenerate and gives
$$
\Hom_k(H^p(X, \mathcal{L}^{\otimes -m}), k) =
H^0(X, H^{-p}(\omega_X^\bullet) \otimes \mathcal{L}^{\otimes m}).
$$
A coherent module $\mathcal{G}$ is zero if and only if
$H^0(X, \mathcal{G} \otimes \mathcal{L}^{\otimes m})$ is zero for all
sufficiently large $m$: use global generation for the nontrivial direction.
Taking $p = d - i$ proves the equivalence of (2) and (4).

This is the intrinsic dualizing-complex form of
\cite[Expos\'e 149, Th\'eor\`eme 3 bis and Proposition 6,
pp. 186--187]{FGA}.
\end{proof}
...
\begin{reference}
\cite[Chapter III, \S 4, no. 69, Proposition 1, Corollary 2, and
Remark, pp. 262--263]{FAC}
\end{reference}
\begin{history}
The cited source gives the polynomial-cone instance. Put
$X=\operatorname{Spec}(K[t_0,\ldots,t_r])$ and
$U=X\setminus\{0\}$. Its ideals
$J_k=(t_0^k,\ldots,t_r^k)$ are cofinal with powers of the ideal of the
origin. A truncated Koszul resolution identifies its finite-stage module
$B_k^q(M)$ with internal graded $\operatorname{Ext}^q(J_k,M)$; because
$J_k$ is finite, forgetting the grading gives ordinary Ext. Taking
$K_n=\mathcal I^n$, $K=\mathcal O_U$, and
$L=\widetilde M[q]$ below yields
$\varinjlim\operatorname{Ext}^q(\mathcal I^n,\widetilde M)
=H^q(U,\widetilde M|_U)$, which is the source's final identification after
retaining homogeneous components. The lemma removes the polynomial and
grading restrictions while keeping the Noetherian coherent hypotheses.
\end{history}
